package AICluster::SegmentGenerator;

use strict;
use AICluster::Functions;

my $pid_file = "\${workspace_root}/run/SegmentGenerator.pid";

sub start
{
  my ($host, $description) = @_;
  my $config_file = "\${config_root}/$host/SegmentGeneratorConfig.json";
  my $command =
    ...
    "fi; " .
    "setsid -f SegmentGenerator.py --config=\"$config_file\" " .
      "> \${workspace_root}/run/SegmentGenerator.out 2>&1 < /dev/null; " .
