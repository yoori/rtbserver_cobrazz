...
sub start
{
  ...
  my $command =
    ...
    AdServer::Functions::pidfile_start_guard($pid_file, "ExpressionMatcher") . " && " .
    "ulimit -n 64000 && " .
...
{
  my ($host, $descr) = @_;
  return AdServer::Functions::stop_by_pidfile($host, $descr, $pid_file, "ExpressionMatcher");
}
...
sub is_alive
{
  ...
  return AdServer::Functions::execute_command(
    $host, $descr, AdServer::Functions::pidfile_is_alive($pid_file, "ExpressionMatcher"));
}
