...
<xsl:template name="UserBindServerConfigGenerator">
  ...
  <cfg:UserBindServerConfig partition_index="0" partitions_number="1">
    ...
    <xsl:variable name="user-bind-server-grpc-threads-per-cq">
      <xsl:value-of select="$user-bind-server-config/cfg:networkParams/@grpc_threads_per_cq"/>
      <xsl:if test="not($user-bind-server-config/cfg:networkParams/@grpc_threads_per_cq)">
    ...
    <cfg:GrpcConfig process_threads="{$user-bind-server-grpc-process-threads}"
      ...
      threads_per_cq="{$user-bind-server-grpc-threads-per-cq}"
    ...
    <cfg:HttpConfig>
      <cfg:Endpoint host="*">
        <xsl:attribute name="port"><xsl:value-of select="$user-bind-server-monitoring-port"/></xsl:attribute>
      </cfg:Endpoint>
    </cfg:HttpConfig>
    ...
    <xsl:variable name="user-bind-keep-mode">
      <xsl:value-of select="$colo-config/cfg:userProfiling/@user_bind_keep_mode"/>
      <xsl:if test="count($colo-config/cfg:userProfiling/@user_bind_keep_mode)
        = 0">keep slave</xsl:if>
    </xsl:variable>
