...
<xsl:stylesheet
  ...
  xsi:schemaLocation="http://www.adintelligence.net/xsd/AdServer/Configuration ../AdServer/UserInfoSvcs/UserBindController2Config.xsd">
...
<xsl:template name="UserBindController2ConfigGenerator">
  ...
  <cfg:UserBindController2Config>
    ...
    <xsl:call-template name="ConvertLogger">
      ...
      <xsl:with-param name="log-file" select="concat($workspace-root, $user-bind-controller2-log-path)"/>
...
<xsl:template match="/">
  ...
  <xsl:choose>
    <xsl:when test="count($xpath) = 0">
       <xsl:message terminate="yes"> UserBindController2: Can't find XPATH element </xsl:message>
    </xsl:when>

    <xsl:when test="count($full-cluster-path) = 0">
       <xsl:message terminate="yes"> UserBindController2: Can't find full cluster group </xsl:message>
    </xsl:when>

    <xsl:when test="count($be-cluster-path) = 0">
       <xsl:message terminate="yes"> UserBindController2: Can't find be-cluster group </xsl:message>
    </xsl:when>

    <xsl:when test="count($fe-cluster-path) = 0">
       <xsl:message terminate="yes"> UserBindController2: Can't find fe-cluster group </xsl:message>
    </xsl:when>
  </xsl:choose>
  ...
  <xsl:variable
    ...
    select="$xpath/configuration/cfg:userBindController2"/>
  ...
  <cfg:AdConfiguration>
    <xsl:attribute name="xsi:schemaLocation"><xsl:value-of select="concat('http://www.adintelligence.net/xsd/AdServer/Configuration ', $server-root, '/xsd/UserInfoSvcs/UserBindController2Config.xsd')"/></xsl:attribute>
    <xsl:call-template name="UserBindController2ConfigGenerator">
