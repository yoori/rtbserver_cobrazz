...
<xsl:stylesheet
  ...
  xsi:schemaLocation="http://www.adintelligence.net/xsd/AdServer/Configuration ../AdServer/UserInfoSvcs/UserInfoController2Config.xsd">
...
<xsl:template name="UserInfoController2ConfigGenerator">
  ...
  <cfg:UserInfoController2Config>
    ...
    <xsl:variable name="user-info-controller-grpc-port">
      <xsl:choose>
        <xsl:when test="count($user-info-controller-config/cfg:networkParams/@port) > 0">
          <xsl:value-of select="$user-info-controller-config/cfg:networkParams/@port"/>
        </xsl:when>
        <xsl:when test="count($user-info-controller-config/cfg:networkParams/@grpc_port) > 0">
          <xsl:value-of select="$user-info-controller-config/cfg:networkParams/@grpc_port"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$def-user-info-controller2-grpc-port"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>

    <xsl:variable name="status-check-period"><xsl:value-of select="$user-info-controller-config/cfg:controlParams/@status_check_period"/>
      <xsl:if test="count($user-info-controller-config/cfg:controlParams/@status_check_period) = 0">
    ...
    <xsl:call-template name="ConvertLogger">
      <xsl:with-param name="logger-node" select="$user-info-controller-config/cfg:logging"/>
      <xsl:with-param name="log-file" select="concat($workspace-root, $user-info-controller2-log-path)"/>
...
<xsl:template match="/">
  ...
  <xsl:choose>
    <xsl:when test="count($xpath) = 0">
       <xsl:message terminate="yes"> UserInfoController2: Can't find XPATH element </xsl:message>
    </xsl:when>

    <xsl:when test="count($full-cluster-path) = 0">
       <xsl:message terminate="yes"> UserInfoController2: Can't find full cluster group </xsl:message>
    </xsl:when>

    <xsl:when test="count($be-cluster-path) = 0">
       <xsl:message terminate="yes"> UserInfoController2: Can't find be-cluster group </xsl:message>
    </xsl:when>

    <xsl:when test="count($fe-cluster-path) = 0">
       <xsl:message terminate="yes"> UserInfoController2: Can't find fe-cluster group </xsl:message>
    </xsl:when>
  </xsl:choose>
  ...
  <xsl:variable
    ...
    select="$xpath/configuration/cfg:userInfoController2"/>
  ...
  <cfg:AdConfiguration>
    <xsl:attribute name="xsi:schemaLocation"><xsl:value-of select="concat('http://www.adintelligence.net/xsd/AdServer/Configuration ', $server-root, '/xsd/UserInfoSvcs/UserInfoController2Config.xsd')"/></xsl:attribute>
    <xsl:call-template name="UserInfoController2ConfigGenerator">
