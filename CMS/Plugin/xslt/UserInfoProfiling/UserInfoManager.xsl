...
<xsl:template name="UserInfoManagerConfigGenerator">
  ...
  <cfg:UserInfoManagerConfig
    ...
    <xsl:variable name="user-info-manager-grpc-threads-per-cq">
      <xsl:value-of select="$user-info-manager-config/cfg:networkParams/@grpc_threads_per_cq"/>
      <xsl:if test="not($user-info-manager-config/cfg:networkParams/@grpc_threads_per_cq)">
        <xsl:value-of select="1"/>
      </xsl:if>
    </xsl:variable>
    <xsl:variable name="user-info-manager-grpc-max-sequential-ops">
      <xsl:value-of select="$user-info-manager-config/cfg:networkParams/@grpc_max_sequential_ops"/>
      <xsl:if test="count($user-info-manager-config/cfg:networkParams/@grpc_max_sequential_ops) = 0">4</xsl:if>
    </xsl:variable>
    <xsl:variable name="user-info-manager-monitoring-port">
      ...
      <xsl:if test="count($user-info-manager-config/cfg:networkParams/@monitoring_port) = 0">
        <xsl:value-of select="$user-info-manager-port + 600"/>
      </xsl:if>
    </xsl:variable>
    ...
    <xsl:variable name="use-add-profile-on-match"><xsl:value-of select="$user-info-manager-config/cfg:matchParams/@use_add_profile_on_match"/>
      <xsl:if test="count($user-info-manager-config/cfg:matchParams/@use_add_profile_on_match) = 0">false</xsl:if>
    </xsl:variable>
   <xsl:variable name="history-optimization-period"><xsl:value-of select="$user-info-manager-config/cfg:matchParams/@history_optimization_period"/>
    ...
    <cfg:GrpcConfig process_threads="{$user-info-manager-grpc-process-threads}"
      ...
      threads_per_cq="{$user-info-manager-grpc-threads-per-cq}"
      max_sequential_ops="{$user-info-manager-grpc-max-sequential-ops}">
      <cfg:Endpoint host="*">
    ...
    <cfg:HttpConfig>
      <cfg:Endpoint host="*">
        <xsl:attribute name="port"><xsl:value-of select="$user-info-manager-monitoring-port"/></xsl:attribute>
      </cfg:Endpoint>
    </cfg:HttpConfig>
