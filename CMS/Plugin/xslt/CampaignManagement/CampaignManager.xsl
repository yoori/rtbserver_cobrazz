...
<xsl:template name="CampaignManagerConfigGenerator">
  ...
  <xsl:param name="full-cluster-path"/>
    ...
    <xsl:variable name="campaign-manager-grpc-threads-per-cq">
      <xsl:value-of select="$campaign-manager-config/cfg:networkParams/@grpc_threads_per_cq"/>
      <xsl:if test="not($campaign-manager-config/cfg:networkParams/@grpc_threads_per_cq)">
  ...
  <cfg:CampaignManager
    ...
    <cfg:GrpcConfig
      ...
      cq_threads="4"
      threads_per_cq="{$campaign-manager-grpc-threads-per-cq}"
    ...
    <cfg:Creative
      ...
      <cfg:CreativeRule name="unsecure" secure="false"
        ...
        notice_url="http://{$adserving-domain}/track.png"
        ...
        nonsecure_direct_instantiate_url="http://{$adserving-domain}/inst?"
        video_instantiate_url="http://{$video-domain}/inst?"
        nonsecure_video_instantiate_url="http://{$video-domain}/inst?"
        >
      ...
      <cfg:CreativeRule name="secure" secure="true"
        ...
        notice_url="https://{$adserving-domain}/track.png"
        ...
        nonsecure_direct_instantiate_url="http://{$adserving-domain}/inst?secure=1&amp;"
        video_instantiate_url="https://{$video-domain}/inst?"
        nonsecure_video_instantiate_url="http://{$video-domain}/inst?secure=1&amp;"
        >
        <xsl:for-each select="$colo-config/cfg:coloParams/cfg:bindUrl">
          <cfg:Token name="{concat('BINDURL', position())}" value="{@template}"/>
        </xsl:for-each>
      ...
      <xsl:for-each select="$colo-config/cfg:coloParams/cfg:RTB/cfg:source">
        <xsl:if test="$colo-config/cfg:coloParams/cfg:RTB/cfg:source/@custom_click = 'true' or
          ...
          <cfg:SourceRule name="{@id}">
            ...
            <xsl:if test="count(@vast_preclick) > 0">
              <xsl:attribute name="vast_preclick">
                <xsl:value-of select="@vast_preclick"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:if test="count(@mime_encoded_vast_preclick) > 0">
              <xsl:attribute name="mime_encoded_vast_preclick">
                <xsl:value-of select="@mime_encoded_vast_preclick"/>
              </xsl:attribute>
            </xsl:if>
          </cfg:SourceRule>
        </xsl:if>
    ...
    <cfg:ConvRateConfig root="{concat($log-root,'/In/ConvRateConfig')}"
      ...
      check_period="60"
      expire_timeout="0"/>
    <cfg:BidCostConfig root="{concat($log-root,'/In/BidCostConfig')}"
      capture_root="{concat($log-root,'/In/CapturedBidCostConfig')}"
      check_period="60"
      expire_timeout="2592000"/>
