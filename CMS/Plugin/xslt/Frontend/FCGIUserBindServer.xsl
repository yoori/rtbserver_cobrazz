...
<xsl:template name="FCGIUserBindServerConfigGenerator">
  ...
  <cfg:FCGIServerConfig>
    ...
    <xsl:attribute name="grpc_coalesce_threads">
      <xsl:value-of select="$fcgi-adserver-config/cfg:userBindConfig/@threads"/>
      <xsl:if test="count($fcgi-adserver-config/cfg:userBindConfig/@threads) = 0">64</xsl:if>
    </xsl:attribute>
    ...
    <xsl:attribute name="pid_file">
      <xsl:value-of select="concat($workspace-root, '/run/FCGIUserBindServer.pid')"/>
    </xsl:attribute>
