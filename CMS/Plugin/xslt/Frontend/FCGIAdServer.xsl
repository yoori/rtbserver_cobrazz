...
<xsl:template name="FCGIServerConfigGenerator">
  ...
  <cfg:FCGIServerConfig>
    ...
    <xsl:attribute name="grpc_coalesce_threads">
      <xsl:value-of select="$fcgi-adserver-config/cfg:adConfig/@threads"/>
      <xsl:if test="count($fcgi-adserver-config/cfg:adConfig/@threads) = 0">64</xsl:if>
    </xsl:attribute>
    ...
    <xsl:for-each select="exsl:node-set($socket_arr)/i">
      <cfg:BindSocket>
        ...
        <xsl:attribute name="accept_threads">
          ...
          <xsl:if test="count($fcgi-adserver-config/cfg:adFCGINetworkParams/@accept_threads) = 0">10</xsl:if>
        </xsl:attribute>
    ...
    <cfg:Monitoring port="{$fcgi-adserver-mon-port}"/>
