...
<xsl:variable name="nginx-index" select="$NGINX_INDEX"/>
...
<xsl:template name="httpdConfigGenerator">
  ...
  <xsl:variable name="virtual-servers-raw">
    <xsl:for-each select="$colo-config/cfg:coloParams/cfg:virtualServer[
      ((count(@nginx_index) = 0 and $nginx-index = 1) or (@nginx_index = $nginx-index)) and (count(@enable) = 0 or @enable='true')]">
       <xsl:copy-of select="."/>
    </xsl:for-each>
  </xsl:variable>
...
pid "<xsl:value-of select="$workspace-root"/>/run/nginx<xsl:value-of select="$nginx-index"/>.pid";
...
http {
    ...
    upstream fastcgi_rtbbackend {
      ...
      keepalive <xsl:value-of select="$frontend-config/cfg:rtbFCGINetworkParams/@keep_connections"/><xsl:if
        test="count($frontend-config/cfg:rtbFCGINetworkParams/@keep_connections)
        = 0">800</xsl:if>;
    ...
    <xsl:variable name="locations">
      ...
      location =/crossdomain.xml {
        alias <xsl:value-of select="$config-root"/>/conf<xsl:value-of select="$nginx-index"/>/crossdomain.xml;
      }
    ...
    server {
      <xsl:choose>
        <xsl:when test="@proxy_protocol= 'true' or @proxy_protocol = '1'">
           listen <xsl:value-of select="$port"/> proxy_protocol so_keepalive=10:5:2;
           set_real_ip_from 0.0.0.0/24;
           real_ip_header proxy_protocol;
        </xsl:when>
        <xsl:otherwise>
           listen <xsl:value-of select="$port"/> default_server so_keepalive=10:5:2;
        </xsl:otherwise>
      </xsl:choose>
      ...
      location = / {
        ...
        if ( $args ~ "id=" ) { set $adblast_processing A; }
        if ( $host ~* "ad-blast.ru" ) { set $adblast_processing "${adblast_processing}B"; }
        if ( $adblast_processing = AB ) { return 420; }
      }
    ...
    server {
      <xsl:choose>
        <xsl:when test="@proxy_protocol= 'true' or @proxy_protocol = '1'">
           listen <xsl:value-of select="$port"/> ssl proxy_protocol so_keepalive=10:5:2;
           set_real_ip_from 0.0.0.0/24;
           real_ip_header proxy_protocol;
        </xsl:when>
        <xsl:otherwise>
           listen <xsl:value-of select="$port"/> ssl so_keepalive=10:5:2;
        </xsl:otherwise>
      </xsl:choose>
