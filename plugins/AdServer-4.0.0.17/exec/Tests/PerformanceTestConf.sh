...
PERF_OUT_DIR=$OUT_DIR/PerformanceTest-$CONFIGNAME
...
$EXEC/XsltTransformer.sh \
  ...
  --out-file $ROOT_OUT_DIR/performance-test-$CONFIGNAME.sh

let "EXIT_CODE |= $?"

chmod +x $ROOT_OUT_DIR/performance-test-$CONFIGNAME.sh

if [ $EXIT_CODE -eq 0 ]
then
  echo config for PerformanceTest \"$CONFIGNAME\" completed successfully
else
  echo config for PerformanceTest \"$CONFIGNAME\" contains errors >2
fi
