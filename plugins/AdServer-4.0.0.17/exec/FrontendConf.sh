...
do
  ...
  </cross-domain-policy>" > $OUT_DIR/$host/"$CONF_SUFFIX"conf/crossdomain.xml
  done
done
...
$EXEC/ProcessHostFiles.sh \
   ...
   --cmd "mkdir -p \$HOST_DIR/\"$CONF_SUFFIX\"conf" \
   --cmd "touch \$HOST_DIR/\"$CONF_SUFFIX\"conf/empty" \
