...
<xsl:template name="AdFrontendConfigGenerator">
  ...
  <cfg:AdOpConfiguration nouid_cache_control="max-age=432000, max-stale=432000" optin_cache_control="no-store, no-cache, must-revalidate">
    <xsl:attribute name="nouid_script"><xsl:value-of select="concat($data-root, '/PageSense/http/tag/pb.js')"/></xsl:attribute>
    <xsl:attribute name="optin_script"><xsl:value-of select="concat($data-root, '/PageSense/http/tag/1.js')"/></xsl:attribute>
    <cfg:UriList>
      <cfg:Uri path="/services/adop"/>
      <cfg:Uri path="/adop"/>
    </cfg:UriList>
    <xsl:call-template name="ConvertFrontendLogger">
      <xsl:with-param name="logger-node" select="$adop-module/cfg:logging"/>
      <xsl:with-param name="default-log-level" select="$adop-module-log-level"/>
    </xsl:call-template>
  </cfg:AdOpConfiguration>
    ...
    <cfg:BidFeConfiguration max_pending_tasks="10">
      ...
      <xsl:for-each select="$colo-config/cfg:coloParams/cfg:RTB/cfg:source">
        <cfg:Source id="{@id}">
          ...
          <xsl:attribute name="enable_notice"><xsl:value-of
             select="@enable_notice"/><xsl:if test="count(@enable_notice)
          ...
          <xsl:choose>
            <xsl:when test="count(@vast_instantiate_type) > 0">
              ...
              <xsl:attribute name="vast_enable_notice">
                <xsl:value-of select="@vast_enable_notice"/>
              </xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
              <xsl:attribute name="vast_instantiate_type">
              ...
              <xsl:attribute name="vast_enable_notice">
                <xsl:value-of select="@enable_notice"/>
              </xsl:attribute>
            </xsl:otherwise>
          </xsl:choose>
