...
<xsl:template name="FCGIServerConfigGenerator">
  ...
  <cfg:FCGIServerConfig>
    ...
    <xsl:for-each select="exsl:node-set($socket_arr)/i">
      <cfg:BindSocket>
        <xsl:attribute name="backlog">
          <xsl:value-of select="$fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@backlog"/>
          <xsl:if test="count($fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@backlog) = 0">8000</xsl:if>
        </xsl:attribute>
        <xsl:attribute name="accept_threads">
          <xsl:value-of select="$fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@accept_threads"/>
          <xsl:if test="count($fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@accept_threads) = 0">5</xsl:if>
        </xsl:attribute>
        <xsl:attribute name="bind">
          <xsl:value-of select="concat($workspace-root,'/run/fcgi_rtbserver',., '.sock')"/>
        </xsl:attribute>
      </cfg:BindSocket>
    </xsl:for-each>
    ...
    <xsl:variable name="fcgi-rtbserver-port">
      <xsl:value-of select="$fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@port"/>
      <xsl:if test="count($fcgi-rtbserver-config/cfg:rtbFCGINetworkParams/@port) = 0">
        <xsl:value-of select="$def-fcgi-rtbserver-port"/>
      </xsl:if>
    </xsl:variable>

    <exsl:document href="FCGIServer.port"
      method="text" omit-xml-declaration="yes"
      >  ['FCGIServer', <xsl:copy-of select="$fcgi-rtbserver-port"/>],</exsl:document>
    ...
    <cfg:CorbaConfig>
      ...
      <cfg:Endpoint host="*">
        <xsl:attribute name="port"><xsl:value-of select="$fcgi-rtbserver-port"/></xsl:attribute>
