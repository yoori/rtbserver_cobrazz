  ...
  <cfg:FCGIServerConfig>
    ...
    <cfg:BindSocket>
      <xsl:attribute name="backlog">
        <xsl:value-of select="$fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@backlog"/>
        <xsl:if test="count($fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@backlog) = 0">1000</xsl:if>
      </xsl:attribute>
      <xsl:attribute name="accept_threads">
        <xsl:value-of select="$fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@accept_threads"/>
        <xsl:if test="count($fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@accept_threads) = 0">5</xsl:if>
      </xsl:attribute>
      <xsl:attribute name="bind">
        <xsl:value-of select="concat($workspace-root,'/run/fcgi_userbindintserver.sock')"/>
      </xsl:attribute>
    </cfg:BindSocket>

    <xsl:variable name="colo-id" select="$colo-config/cfg:coloParams/@colo_id"/>

    <xsl:variable name="def-fcgi-userbindintserver-port">
      <xsl:value-of select="$fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@port"/>
      <xsl:if test="count($fcgi-adserver-config/cfg:userBindIntFCGINetworkParams/@port) = 0">
        <xsl:value-of select="$def-fcgi-userbindintserver-port"/>
      </xsl:if>
    </xsl:variable>

    <exsl:document href="FCGIUserBindIntServer.port"
      method="text" omit-xml-declaration="yes"
      >  ['FCGIServer', <xsl:copy-of select="$def-fcgi-userbindintserver-port"/>],</exsl:document>
    ...
    <cfg:CorbaConfig>
      ...
      <cfg:Endpoint host="*">
        <xsl:attribute name="port"><xsl:value-of select="$def-fcgi-userbindintserver-port"/></xsl:attribute>
    ...
    <xsl:call-template name="ConvertLogger">
      ...
      <xsl:with-param name="log-file" select="concat($workspace-root, $fcgi-userbindintserver-log-path)"/>
