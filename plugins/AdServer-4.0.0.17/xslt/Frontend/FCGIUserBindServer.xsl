  ...
  <cfg:FCGIServerConfig>
    ...
    <cfg:BindSocket>
      <xsl:attribute name="backlog">
        <xsl:value-of select="$fcgi-adserver-config/cfg:userBindFCGINetworkParams/@backlog"/>
        <xsl:if test="count($fcgi-adserver-config/cfg:userBindFCGINetworkParams/@backlog) = 0">1000</xsl:if>
      </xsl:attribute>
      <xsl:attribute name="accept_threads">
        <xsl:value-of select="$fcgi-adserver-config/cfg:userBindFCGINetworkParams/@accept_threads"/>
        <xsl:if test="count($fcgi-adserver-config/cfg:userBindFCGINetworkParams/@accept_threads) = 0">5</xsl:if>
      </xsl:attribute>
      <xsl:attribute name="bind">
        <xsl:value-of select="concat($workspace-root,'/run/fcgi_userbindserver.sock')"/>
      </xsl:attribute>
    </cfg:BindSocket>
    ...
    <xsl:variable name="def-fcgi-userbindserver-port">
      <xsl:value-of select="$fcgi-adserver-config/cfg:userBindFCGINetworkParams/@port"/>
      <xsl:if test="count($fcgi-adserver-config/cfg:userBindFCGINetworkParams/@port) = 0">
