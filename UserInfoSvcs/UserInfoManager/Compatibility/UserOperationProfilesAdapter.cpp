...
namespace AdServer::UserInfoSvcs
{
  ...
  UserFreqCapUpdateOperationProfilesAdapter::operator()(Generics::SmartMemBuf* smart_mem_buf)
    /*throw(Exception)*/
  {
    ...
    if (version_reader.version() == 1)
    {
      AdServer::UserInfoSvcs_v300::
        UserFreqCapUpdateOperationReader old_reader(membuf.data(), membuf.size());
    ...
    if (current_version == 310)
    {
      ...
      for (auto it = old_reader.seq_orders().begin(); it != old_reader.seq_orders().end(); ++it)
      {
    ...
    return Generics::SmartMemBuf_var(ReferenceCounting::add_ref(smart_mem_buf));
  }

  Generics::SmartMemBuf_var
  UserFreqCapConfirmOperationProfilesAdapter::operator()(Generics::SmartMemBuf* smart_mem_buf)
    /*throw(Exception)*/
  {
    static const char* FUN =
    ...
    if (version_reader.version() != FC_CONFIRM_OPERATION_PROFILE_VERSION)
    {
      ...
      if (version_reader.version() == 1)
      {
        AdServer::UserInfoSvcs_v340::
          UserFreqCapConfirmOperationReader old_reader(membuf.data(), membuf.size());
  ...
  Generics::SmartMemBuf_var
  UserMatchOperationProfilesAdapter::operator()(Generics::SmartMemBuf* smart_mem_buf)
    /*throw(Exception)*/
  {
    static const char* FUN = "UserMatchOperationProfilesAdapter::adapt_match_operation_profile()";
    ...
    if (version_reader.version() != MATCH_OPERATION_PROFILE_VERSION)
    {
      ...
      if (version_reader.version() == 1)
      {
        ...
        match_operation_writer.change_last_request() = ((old_reader.ad_request() & 0x80) == 0);
      ...
      else if (version_reader.version() == 320)
      {
        ...
        match_operation_writer.household() = old_reader.household();
        match_operation_writer.cohort() = old_reader.cohort();
        match_operation_writer.repeat_trigger_timeout() = old_reader.repeat_trigger_timeout();
  ...
  Generics::SmartMemBuf_var
  UserMergeOperationProfilesAdapter::operator()(Generics::SmartMemBuf* smart_mem_buf)
    /*throw(Exception)*/
  {
    static const char* FUN = "UserMergeOperationProfilesAdapter::adapt_merge_operation_profile()";
    ...
    if (version_reader.version() != MERGE_OPERATION_PROFILE_VERSION)
    {
      ...
      if (version_reader.version() == 1)
      {
        ...
        merge_operation_writer.exchange_merge() = old_reader.exchange_merge() & 0x7FFFFFFF;
