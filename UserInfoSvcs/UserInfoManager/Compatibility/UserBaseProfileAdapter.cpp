...
namespace AdServer::UserInfoSvcs
{
  namespace
  {
    ...
    static void copy_base_section(WriterType& ciw, const ReaderType& cir) noexcept
    {
      std::copy(
    ...
    template <typename WriterType, typename ReaderType>
    static void copy_section(WriterType& ciw, const ReaderType& cir) noexcept
    {
...
namespace AdServer::UserInfoSvcs
{
  ...
  BaseProfileAdapter::operator()(Generics::SmartMemBuf* mem_buf, bool ignore_future_versions)
    /*throw(Exception)*/
  {
    ...
    ChannelsProfileVersionReader version_reader(mem_buf->membuf().data(), version_head_size);

    unsigned long current_version = version_reader.version();

    if (current_version > CURRENT_BASE_PROFILE_VERSION && ignore_future_versions)
    {
    ...
    if (current_version != CURRENT_BASE_PROFILE_VERSION)
    {
      ...
      if (current_version == 1)
      {
        ...
        copy_base_section(profile_writer.search_channels(), profile_reader.search_channels());

        copy_base_section(profile_writer.page_channels(), profile_reader.page_channels());

        copy_base_section(profile_writer.url_channels(), profile_reader.url_channels());
      ...
      else if (version_reader.version() == 4)
      {
        ...
        copy_section(profile_writer.search_channels(), profile_reader.search_channels());

        copy_section(profile_writer.page_channels(), profile_reader.page_channels());

        copy_section(profile_writer.url_channels(), profile_reader.url_channels());
      ...
      if (current_version == 6)
      {
        ...
        copy_section(writer.search_channels(), profile_reader.search_channels());

        copy_section(writer.page_channels(), profile_reader.page_channels());

        copy_section(writer.url_channels(), profile_reader.url_channels());
      ...
      if (current_version == 320)
      {
        ...
        copy_section(writer.url_keyword_channels(), profile_reader.url_keyword_channels());
      ...
      if (current_version != CURRENT_BASE_PROFILE_VERSION)
      {
        ...
        ostr << FUN << ": incorrect version after adaptation = " << current_version <<
          "\n Profile size = " << membuf.size() << ", the contents of the profile (36 bytes) : ";
