...
namespace AdServer::UserInfoSvcs
{
  ...
  : callback_(ReferenceCounting::add_ref(callback)),
    ...
    check_operations_callback_(
      new Logging::ActiveObjectCallbackImpl(
        logger_,
        "UserInfoManagerCore::check_operations_()",
  ...
  try
  {
    UserInfoManagerCoreState::CampaignServerPoolConfig
      pool_config(state_->corba_client_adapter.in());
    ...
    state_->campaign_servers.reset(new UserInfoManagerCoreState::CampaignServerPool(
      pool_config,
  ...
  void UserInfoManagerCore::calc_user_daily_stat_(
  ...
  void UserInfoManagerCore::confirm_user_freq_caps(
  const AdServer::Commons::UserId& user_id,
  const Generics::Time& time,
  const Generics::Uuid& request_id,
  ...
  try
  {
    ...
    matched_channels.page_channels.reserve(match_params.page_channel_ids.size());
    for (const auto& channel : match_params.page_channel_ids)
    {
      matched_channels.page_channels.push_back(
        ChannelMatch(
          channel.channel_id,
          channel.channel_trigger_id));
    }

    matched_channels.search_channels.reserve(match_params.search_channel_ids.size());
    for (const auto& channel : match_params.search_channel_ids)
    {
    ...
    matched_channels.url_channels.reserve(match_params.url_channel_ids.size());
    for (const auto& channel : match_params.url_channel_ids)
    {
    ...
    matched_channels.persistent_channels.reserve(match_params.persistent_channel_ids.size());
    for (std::size_t i = 0; i < match_params.persistent_channel_ids.size(); ++i)
    {
    ...
    if (!household)
    {
      ...
      {
        match_result.geo_data_seq.push_back(
          GeoData{geo_data.latitude, geo_data.longitude, geo_data.accuracy});
      }
    ...
    else
    {
      ...
      for (const auto& channel : result_channels)
      {
        match_result.hid_channels.push_back(ChannelWeight{channel.first, channel.second});
      }
    }
  ...
  void
  UserInfoManagerCore::clear_expired(
  bool synch,
      ...
      if(user_info_manager_config_.UserProfilesCleanup().present())
      {
        Task_var all_users_processing_msg =
          new AllUsersProcessingTask(
            0, this, false, 0, cleanup_time, portion);
  ...
  else
  {
    ...
    all_users_process_step_(
      false, 0, true, cleanup_time, portion);
  }
  }

  void
  UserInfoManagerCore::all_users_process_step_(
  bool reschedule,
  ...
  noexcept
  {
  static const char* FUN = "UserInfoManagerCore::all_users_process_step_()";
  ...
  void UserInfoManagerCore::flush_logs_() noexcept
  {
  ...
  try
  {
    for (;;)
    {
      UserInfoManagerCoreState::CampaignServerPool::ObjectHandlerType campaign_server =
  ...
  void
  UserInfoManagerCore::update_config_()
  noexcept
  {
  ...
  void UserInfoManagerCore::push_channel_interval_(
