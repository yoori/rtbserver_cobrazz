...
namespace AdServer::UserInfoSvcs
{
  ...
  using ChannelIdArray = Generics::MonoVector<unsigned long>;
  ...
  {
    explicit ChannelIdPack(std::shared_ptr<Generics::MonoAllocatorArena> arena = nullptr)
  ...
  public:
    ...
    static void copy_history_section(WriterType& ciw, const ReaderType& cir) noexcept
    {
      std::copy(
        cir.begin(), cir.end(),
    ...
    static void copy_base_section(ChannelsInfoWriter& ciw, const ReaderType& cir) noexcept
    {
      std::copy(
    ...
    template <typename ReaderType>
    static void copy_section(ChannelsInfoWriter& ciw, const ReaderType& cir) noexcept
    {
  ...
  private:
    ...
    void
    write_geo_data_(GeoDataWriter& gdw, const CoordData& coord_data, const Generics::Time& now);

    AdServer::CampaignSvcs::AccuracyDecimal
    read_accuracy_(const void* buf);

    AdServer::CampaignSvcs::CoordDecimal
    read_coord_(const void* buf);
    ...
    void add_weight_(ChannelMatchMap& result_channels, uint32_t index, uint32_t weight);
