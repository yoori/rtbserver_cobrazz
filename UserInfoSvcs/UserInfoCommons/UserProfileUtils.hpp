  ...
  public:
      ...
      noexcept
    {
      if (size != 0)
      {
        ...
        {
          table.column(i, expand ? PROFILE_EXPANDED_TABLE_COLUMNS[i] : PROFILE_TABLE_COLUMNS[i]);
        }
...

        {
          ...
          row.add_field(Generics::Time(reader.create_time()).gm_ft());
          row.add_field(Generics::Time(reader.history_time()).gm_ft());
          row.add_field(Generics::Time(reader.ignore_fraud_time()).gm_ft());
          row.add_field(Generics::Time(reader.last_request_time()).gm_ft());
          row.add_field(Generics::Time(reader.session_start()).gm_ft());
          ...
          add_channel_info_row(row, reader.page_channels(), space_align.str().c_str(), expand);
          add_channel_info_row(row, reader.search_channels(), space_align.str().c_str(), expand);
          add_channel_info_row(row, reader.url_channels(), space_align.str().c_str(), expand);
      ...
      noexcept
    {
      if (size != 0)
      {
...

        {
          ...
          add_history_info_row(row, reader.page_channels(), space_align.str().c_str());
          add_history_info_row(row, reader.search_channels(), space_align.str().c_str());
          add_history_info_row(row, reader.url_channels(), space_align.str().c_str());
          add_history_info_row(row, reader.url_keyword_channels(), space_align.str().c_str());
        }
      ...
      const char* space_align)
    {
            ...
            rdr.begin(); it != rdr.end(); ++it)
      {
        ostr << (it == rdr.begin() ? "" : space_align) <<
          "[ channel_id = " << (*it).channel_id() << ", days_visits_pairs =";
      ...
      bool /*expand*/)
    {
          ...
          it != rdr.ht_candidates().end(); ++it)
      {
        htc_ostr << (it == rdr.ht_candidates().begin() ? "" : space_align) <<
          ...
          ", visits = " << (*it).visits() << ", weight = " << (*it).weight() << " ]";
      }
          ...
          it != rdr.history_matches().end(); ++it)
      {
        hm_ostr << (it == rdr.history_matches().begin() ? "" : space_align) <<
          "[ channel_id = " << (*it).channel_id() << ", weight = " << (*it).weight() << " ]";
      }
          ...
          it != rdr.history_visits().end(); ++it)
      {
        hv_ostr << (it == rdr.history_visits().begin() ? "" : space_align) <<
          "[ channel_id = " << (*it).channel_id() << ", visits = " << (*it).visits() << " ]";
      }
