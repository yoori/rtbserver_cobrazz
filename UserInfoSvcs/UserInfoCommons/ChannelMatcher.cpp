...
namespace AdServer::UserInfoSvcs
{
  ...
  {
    explicit RequestChannelsDelta(Generics::MonoAllocatorArena* resource);
  ...
  namespace
  {
    ...
    struct ExistingHTCandidate
    {
      ...
      {
        return channels.find(HTCandidateLess::channel_id(candidate)) != channels.end();
      }
    ...
    struct HTCandidateWriterAdapter
    {
      ...
      const HTCandidatesWriter& operator()(const HTCandidatesWriter& writer) const
      {
        return writer;
      }
      ...
      HTCandidatesWriter operator()(const RequestHTCandidate& candidate) const
      {
    ...
    RequestHistoryVisit::RequestHistoryVisit(uint32_t channel_id, uint32_t visits) noexcept
      : channel_id(channel_id),
        visits(visits)
    {}
    ...
    struct ExistingHistoryVisit
    {
      ...
      {
        return channels.find(HistoryVisitLess::channel_id(visit)) != channels.end();
      }
    ...
    struct HistoryVisitWriterAdapter
    {
      ...
      const HistoryVisitsWriter& operator()(const HistoryVisitsWriter& writer) const
      {
        return writer;
      }
      ...
      HistoryVisitsWriter operator()(const RequestHistoryVisit& visit) const
      {
    ...
    RequestSessionMatch::RequestSessionMatch(uint32_t channel_id, uint32_t timestamp) noexcept
      : channel_id(channel_id),
        timestamp(timestamp)
    {}
  ...
  {
    bool operator()(const DaysVisitsReader& left, const DaysVisitsReader& right) const
      /*throw(eh::Exception)*/
    {
  ...
  {
    DaysVisitsWriter operator()(const DaysVisitsReader& left, const DaysVisitsReader& right) const
      /*throw(eh::Exception)*/
    {
  ...
  {
    template<typename LeftType, typename RightType>
    LastTriggerWriter operator()(const LeftType& left, const RightType& right) const
      /*throw(eh::Exception)*/
    {
  ...
  struct HistoryChannelInfoFilter
  {
    ...
    bool operator()(const HistoryChannelInfoWriter& obj) const
      /*throw(eh::Exception)*/
    {
      return channels_.find(obj.channel_id()) != channels_.end() && !obj.days_visits().empty();
    }
  ...
  {
    template<typename LeftType, typename RightType>
    HistoryVisitsWriter operator()(const LeftType& left, const RightType& right) const
      /*throw(eh::Exception)*/
    {
  ...
  {
    template<typename LeftType, typename RightType>
    SessionMatchesWriter operator()(const LeftType& left, const RightType& right) const
      /*throw(eh::Exception)*/
    {
  ...
  {
  public:
    SessionMatchesCleaner(const ChannelsHashMap& channels, const Generics::Time& now) noexcept
      : channels_(channels),
        now_(now.tv_sec)
    {}
  ...
  {
  public:
    SessionMatchesCleanAdapter(const ChannelsHashMap& channels, const Generics::Time& now) noexcept
  ...
  public:
    ...
    HistoryChannelInfoWriter
    operator()(const HistoryChannelInfoReader& left, const HistoryVisitsReader& right)
      const noexcept
    {
      HistoryChannelInfoWriter ret;
      ret.channel_id() = left.channel_id();
  ...
  public:
    ...
    HistoryChannelInfoWriter
    operator()(const HistoryVisitsReader& right)
      const noexcept
    {
      HistoryChannelInfoWriter ret;
      ...
      dvw.days() = days_offset_;
      dvw.visits() = right.visits();
      ret.days_visits().push_back(std::move(dvw));
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << FUN << ": " << "Attempt to get unique channels from invalid profile: " << ex.what();
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    {
      ChannelsProfileReader rdr(base_profile_->membuf().data(), base_profile_->membuf().size());

      if ((now + gmt_offset).get_gm_time().get_date() >
          (Generics::Time(rdr.last_request_time()) + gmt_offset).get_gm_time().get_date())
        {
          return true;
        }
    }
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << FUN << ": " << "Attempt to get last request time from invalid profile: " << ex.what();
      throw InvalidProfileException(ostr);
    }
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << FUN << ": " << "Attempt to get create time from invalid profile: " << ex.what();
      throw InvalidProfileException(ostr);
    }
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    try
    {
      ...
      {
        ChannelsProfileReader rdr(add_profile_->membuf().data(), add_profile_->membuf().size());
    ...
    catch (const eh::Exception& ex)
    {
      Stream::Error ostr;
      ostr << FUN << ": " <<
        "Attempt to get session start time from invalid profile: " << ex.what();
      throw InvalidProfileException(ostr);
    }
  ...
  bool ChannelsMatcher::fraud_user(const Generics::Time& now)
  {
    ...
    {
      ChannelsProfileReader reader(base_profile_->membuf().data(), base_profile_->membuf().size());

      // fraud detection requests isn't serialized - use greatest
      if (now.tv_sec > reader.ignore_fraud_time())
      {
        writer.init(base_profile_->membuf().data(), base_profile_->membuf().size());
    ...
    const Generics::Time& now)
  {
    ...
    {
      gdw.latitude().alloc(AdServer::CampaignSvcs::CoordDecimal::PACK_SIZE);
      coord_data.latitude.pack(gdw.latitude().data());

      gdw.longitude().alloc(AdServer::CampaignSvcs::CoordDecimal::PACK_SIZE);
      coord_data.longitude.pack(gdw.longitude().data());

      gdw.accuracy().alloc(AdServer::CampaignSvcs::AccuracyDecimal::PACK_SIZE);
  ...
  ChannelsMatcher::read_accuracy_(const void* buf)
  {
  ...
  ChannelsMatcher::read_coord_(const void* buf)
  {
    ...
    const Generics::Time& now)
  {
           ...
           geo_data_count < MAX_GEO_DATA)
    {
      ...
      {
        if ((*base_it).timestamp() / SEC_IN_DAY < now.tv_sec / SEC_IN_DAY - 15)
        {
      ...
      {
        if ((*add_it).timestamp() / SEC_IN_DAY < now.tv_sec / SEC_IN_DAY - 15)
        {
          base_it = base_geo_data.end();
          add_it = add_geo_data.end();
    ...
    const _T2& new_geo_data)
  {
    ...
    {
      if (!new_geo_data_inserted && (*g_it).timestamp() <= new_geo_data.timestamp())
      {
      ...
      {
        if ((*g_it).timestamp() / SEC_IN_DAY < new_geo_data.timestamp() / SEC_IN_DAY - 15)
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    res_upw.household() =
      (base_profile_->membuf().size() != 0) ? base_household : profile_match_params.household;

    ChannelsProfileReader base_rdr(base_profile_->membuf().data(), base_profile_->membuf().size());

    res_upw.create_time() = base_create_time == 0 ? current_time.tv_sec : base_create_time;

    res_upw.last_request_time() =
      profile_match_params.change_last_request ? current_time.tv_sec : base_last_request_time;
    ...
    set_cohort_(res_upw.cohort(), profile_match_params.cohort, profile_match_params.cohort2);

    split_cohort_(properties.cohort, properties.cohort2, res_upw.cohort());

    res_upw.last_colo_id() = profile_match_params.request_colo_id;

    res_upw.history_time() = base_create_time == 0 ? current_time.tv_sec : base_history_time;
    ...
    /*throw(Exception)*/
  {
    ...
    {
      ChannelsProfileReader rdr(base_profile.data(), base_profile.size());
    ...
    /*throw(Exception)*/
  {
    ...
    {
      HistoryUserProfileReader rdr(history_profile.data(), history_profile.size());

      process_channels_sequence_(rdr.page_channels(), dictionary, HISTORY, auc);
      process_channels_sequence_(rdr.search_channels(), dictionary, HISTORY, auc);
      process_channels_sequence_(rdr.url_channels(), dictionary, HISTORY, auc);
      process_channels_sequence_(rdr.url_keyword_channels(), dictionary, HISTORY, auc);
    }
  }
  ...
  void ChannelsMatcher::fill_unique_channels_(
    ...
    /*throw(Exception)*/
  {
    process_channels_sequence_(section.ht_candidates(), dictionary, HISTORY, auc);
    process_channels_sequence_(section.history_matches(), dictionary, HISTORY, auc);
    process_channels_sequence_(section.history_visits(), dictionary, HISTORY, auc);
  }
    ...
    /*throw(Exception)*/
  {
    ...
    if (channels_type == SESSION)
    {
      ...
      {
        if (auc.found_session_channels.find((*it).channel_id()) == auc.found_session_channels.end())
        {
          ChannelsHashMap::const_iterator ch_it = page.find((*it).channel_id());
    ...
    else
    {
      for (; it != sequence.end(); ++it)
      {
        if (auc.found_history_channels.find((*it).channel_id()) == auc.found_history_channels.end())
        {
          ChannelsHashMap::const_iterator ch_it = page.find((*it).channel_id());
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      {
        std::copy(channels.begin(), channels.end(), std::back_inserter((*out_pmw).channel_ids()));
      }
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      {
        add_channel_visit_(now, *it, dictionary, request_delta, result_channels);
      }

      merge_request_channels_delta_(*out_ciw, dictionary, now, base, request_delta, household);

      fill_channels_results_(result_channels, *out_ciw, now, dictionary);
    }
    catch (const eh::Exception& ex)
    {
      Stream::Error ostr;
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    try
    {
      ...
      if (other_base_profile.size() != 0)
      {
        ...
        ChannelsProfileReader temp_base_rdr(other_base_profile.data(), other_base_profile.size());
      ...
      result_profile_writer.history_time() = std::min(base_history_time, other_base_history_time);
      ...
      result_profile_writer.last_colo_id() =
        base_last_colo_id != UNKNOWN_COLO_ID ? base_last_colo_id : match_params.request_colo_id;
      ...
      result_profile_writer.last_request_time() =
        match_params.change_last_request ? merge_time.tv_sec : base_last_request_time;

      base_profile_->membuf().alloc(result_profile_writer.size());
      result_profile_writer.save(base_profile_->membuf().data(), result_profile_writer.size());
      ...
      if (history_profile && (!history_profile->membuf().empty() || !other_history_profile.empty()))
      {
    ...
    /*throw(InvalidProfileException)*/
  {
    ...
    try
    {
      if (!base_profile_->membuf().empty() && add_profile_->membuf().empty())
      {
        return need_history_optimization_(base_profile_.in(), now, period, gmt_offset);
      }
    }
    catch (const eh::Exception& ex)
    {
    ...
    bool /*household*/)
  {
    ...
    fill_history_candidates_(channels_info_writer, history_channels_writer, channels);
  }
    ...
    bool* first_today_history_optimization)
  {
    ...
    std::copy(rdr.geo_data().begin(), rdr.geo_data().end(), std::back_inserter(bwr.geo_data()));
    ...
    {
      ChannelsHashMap::const_iterator c_it = channels.audience_channels.find((*it).channel_id());
      if (c_it != channels.audience_channels.end())
      {
    ...
    const ChannelsHashMap& channels)
  {
    ...
    for (auto it = hciw.begin(); it != hciw.end(); ++it)
    {
      ...
      if (weight != 0)
      {
        ...
        hmw.weight() = weight;
        ciw.history_matches().push_back(std::move(hmw));
      }

      fill_history_candidate_visits_(ciw.ht_candidates(), *it, channels);
    }
  }
      ...
      if (!profile->membuf().empty())
      {
        ...
        if (rdr.history_time() != 0 &&
            ((now + gmt_offset).get_gm_time().get_date() >
             (Generics::Time(rdr.history_time()) + gmt_offset).get_gm_time().get_date() ||
    ...
    const ChannelsHashMap& channels)
  {
    ...
    for (ChannelIntervalList::const_iterator it = cit->second->long_intervals.begin();
    ...
    const ChannelsHashMap& channels)
  {
    ...
    for (auto it = cit->second->today_long_intervals.begin();
      it != cit->second->today_long_intervals.end(); ++it)
    {
      ...
      if (found_visits != 0 || insert_all_ints)
      {
        if (!insert_all_ints)
        {
          for (ChannelIntervalList::const_iterator lit = cit->second->today_long_intervals.begin();
            lit != it; ++lit)
          {
            ...
            ht_candidates.push_back(std::move(new_ht_candidate));
          }
        }
        ...
        new_ht_candidate.weight() = new_ht_candidate.req_visits() != 0 ? 0 : it->weight;
    ...
    ChannelMatchMap& result_channels) noexcept
  {
    ...
    if (c_it != channels.end())
    {
      ...
      if (!c_it->second->long_intervals.empty() || !c_it->second->today_long_intervals.empty())
      {
      ...
      {
        delta.ht_candidates.emplace_back(channel_id, it->min_visits, it->req_visits, 1, it->weight);
      }
    }
  }

  template <
    typename HistoryChannelInfoListWriter,
    ...
    const ChannelsHashMap& channels)
  {
    ...
    Algs::merge_unique(
      base.begin(), base.end(),
      add.begin(), add.end(),
      Algs::filter_inserter(std::back_inserter(hw), HistoryChannelInfoFilter(channels)),
      ChannelIdLess(),
    ...
    bool /*household*/)
  {
    ...
    if (base)
    {
      Algs::merge_unique(
        ...
        Algs::modify_inserter(
          Algs::filter_inserter(std::back_inserter(ciw.session_matches()), SessionMatchesFilter()),
    ...
    bool /*household*/)
  {
    if (base && add)
    {
      ...
      merge_htc_(ciw.ht_candidates(), channels, base->ht_candidates(), add->ht_candidates());
    ...
    else if (base)
    {
      ...
      merge_htc_(ciw.ht_candidates(), channels, base->ht_candidates(), empty_ht);

      ciw.history_matches().reserve(ciw.history_matches().size() + base->history_matches().size());
    ...
    else if (add)
    {
      ...
      merge_htc_(ciw.ht_candidates(), channels, add->ht_candidates(), empty_ht);
    ...
    const AddReaderType& add)
  {
    ...
    while (base_it != base.end() && add_it != add.end())
    {
      ...
      else
      {
        ...
        if (c_it != channels.end())
        {
          ChannelIntervalList::const_iterator ci_it = c_it->second->today_long_intervals.begin();
    ...
    std::copy(
      base_it, base.end(),
      Algs::filter_inserter(std::back_inserter(htcw), ExistChannelFilter(channels)));
  ...
  void ChannelsMatcher::history_print(
    ...
    noexcept
  {
    ColumnProfilePrint::print_history_profile(mb.data(), mb.size(), ostr, print_align);
  }
  ...
  void ChannelsMatcher::print(
    ...
    noexcept
  {
    ColumnProfilePrint::print_profile(mb.data(), mb.size(), ostr, print_expand, print_align);
  }
    ...
    noexcept
  {
    ...
    try
    {
      ...
      {
        ColumnProfilePrint::print_history_profile(profile, size, ostr, print_align);
      }
    }
    catch (const eh::Exception& ex)
    {
    ...
    noexcept
  {
    ...
    try
    {
      ...
      {
        ColumnProfilePrint::print_profile(profile, size, ostr, print_expand, print_align);
      }
    }
    catch (const eh::Exception& ex)
    {
    ...
    noexcept
  {
    if (!cohort_part1.empty() || !cohort_part2.empty())
    {
      ...
      split_cohort_(res_cohort_part1, res_cohort_part2, res_cohort);
