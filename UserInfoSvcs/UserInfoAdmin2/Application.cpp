...
namespace
{
  ...
  const char USAGE[] =
    "\nUsage: \nUserInfoAdmin2 <command> <command arguments>\n\n"
    "UserInfoAdmin2 match --uid=<user id> --page-channels=<ids> "
      "-r[--reference=]<user_info_controller2_grpc_endpoint>|"
      "<user_info_manager_grpc_endpoint> --time=<DD-MM-YYYY:HH-MM-SS>\n\n"
    "UserInfoAdmin2 [print|print-freq-cap|print-base|print-add|print-history] "
      "--uid=<user id> [-e|--expand] [-a|--align] [-p|--plain] "
      "-r[--reference=]<grpc endpoint>\n\n"
    "UserInfoAdmin2 remove --uid=<user id> -r[--reference=]<grpc endpoint>\n\n"
    "UserInfoAdmin2 delete-old-profiles -r[--reference=]<grpc endpoint> "
      "--sync --time=<HH-MM> --portion=<number>\n\n"
    "UserInfoAdmin2 get-config-timestamp -r[--reference=]<grpc endpoint>\n\n"
    "UserInfoAdmin2 generate --keys-dir=<path to key directory> --persistent\n";
  ...
  UserInfoClientHolder create_user_info_client_(const std::string& reference)
  {
    ...
    result.coalesce_runner =
      std::make_shared<AdServer::Commons::BoostAsioContextRunActiveObject>(
        new Logging::ActiveObjectCallbackImpl(logger, "UserInfoAdmin2", "gRPC"),
