#include "UserBindController2Main.hpp"
...
namespace
{
  const char ASPECT[] = "UserBindController2";
...
void
UserBindController2App_::stop_() noexcept
{
  if (grpc_adapter_.in())
  {
...
void
UserBindController2App_::main(int& argc, char** argv) noexcept
{
  static const char* FUN = "UserBindController2App_::main()";

  try
  {
    const char* usage = "usage: UserBindController2 <config_file>";
    if (argc < 2)
    {
      Stream::Error ostr;
    ...
    try
    {
      ...
      configuration_.reset(
        new UserBindController2ConfigType(
          ad_configuration->UserBindController2Config()));
    }
    ...
    Logging::ActiveObjectCallbackImpl_var callback(
      new Logging::ActiveObjectCallbackImpl(
        logger(),
        "UserBindController2App",
        ASPECT));

    controller_ = new AdServer::UserInfoSvcs::UserBindController2Impl(
      callback,
      logger(),
      config());

    grpc_adapter_ = new AdServer::UserInfoSvcs::UserBindController2Grpc(
      controller_,
      logger(),
  ...
  catch (const Exception& ex)
  {
    if (logger())
    {
      logger()->sstream(Logging::Logger::CRITICAL, ASPECT, "ADS-IMPL-73") <<
        FUN << ": caught UserBindController2App_::Exception: " << ex.what();
    }
    else
    {
      std::cerr << FUN << ": caught UserBindController2App_::Exception: " <<
        ex.what() << '\n';
    }
  }
  catch (const eh::Exception& ex)
