...
#define CALL_MATCHER_WITH_RESULT(USER_ID, CALL_NAME, ...) \
  ...
  UserInfoManagerSession* UserInfoCorbaClient::user_info_session() noexcept
  {
    UserInfoManagerSession* session =
      static_cast<UserInfoManagerSession*>(distributor_.in());
    CORBA::add_ref(session);
    return session;
  }
