#include "UserInfoControllerMain.hpp"
...
namespace
{
  const char ASPECT[] = "UserInfoController";
...
void
UserInfoControllerApp_::stop_() noexcept
{
  if (grpc_adapter_.in())
  {
...
UserInfoControllerApp_::main(int& argc, char** argv) noexcept
{
  ...
  try
  {
    const char* usage = "usage: UserInfoController <config_file>";
    if (argc < 2)
    {
      Stream::Error ostr;
  ...
  catch (const Exception& ex)
  {
    if (logger())
    {
      logger()->sstream(Logging::Logger::CRITICAL, ASPECT, "ADS-IMPL-73") <<
        FUN << ": caught UserInfoControllerApp_::Exception: " << ex.what();
    }
    else
    {
