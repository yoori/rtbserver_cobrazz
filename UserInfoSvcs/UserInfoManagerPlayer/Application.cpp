...
namespace
{
  ...
  bool parse_args(int argc, char** argv, Config& config)
  {
    ...
    args.add(
      ...
      opt_send_endpoint);
    args.add(
      Generics::AppUtils::equal_name("dump"),
      opt_dump);
  ...
  grpc::Status replay_record(
    uim::UserInfoManagerGrpc::Stub* stub,
    const dump::DumpRecord& record)
  {
    ...
    return grpc::Status(
      ...
      "unknown UserInfoManager method: " + record.method);
  }
  ...
  int print_dump(const std::string& dump_path)
  {
    ...
    {
      std::cout << format_time_us(record.request_time_us) <<
        ' ' << record.method << '\n';
    }
