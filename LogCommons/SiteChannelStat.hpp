...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    SiteChannelStatInnerKey(std::uint32_t tag_id, std::uint32_t channel_id)
    :
      tag_id_(tag_id),
  ...
  public:
    ...
    bool operator==(const SiteChannelStatInnerData &rhs) const
    {
      ...
      return imps_ == rhs.imps_ &&
        adv_revenue_ == rhs.adv_revenue_ && pub_revenue_ == rhs.pub_revenue_;
    }
  ...
  struct SiteChannelStatKey
  {
    ...
    SiteChannelStatKey(const DayTimestamp &sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
  ...
  public:
    ...
    friend std::istream& operator>>(std::istream &is, SiteChannelStatKey &key);
    friend std::ostream& operator<<(std::ostream &os,
