        ...
        for (OldCollectorT::DataT::const_iterator it = old_data.begin(); it != old_data.end(); ++it)
        {
          ...
          if (reach_diff)
          {
            CollectorT::DataT::KeyT
              inner_key(it->first.site_id(), old_key.isp_sdate().time() -= ONE_DAY);
          ...
          {
            CollectorT::DataT::KeyT
              inner_key(it->first.site_id(), old_key.isp_sdate().time() -= ONE_MONTH);
            data.add(inner_key, CollectorT::DataT::DataT(it->second.monthly_reach()));
          }
        }
