...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    SearchEngineStatInnerKey(std::uint32_t search_engine_id, const std::string& host_name)
    :
    ...
    bool operator==(const SearchEngineStatInnerKey& rhs) const
    {
      ...
      return search_engine_id_ == rhs.search_engine_id_ && host_name_ == rhs.host_name_;
    }
  ...
  public:
    ...
    SearchEngineStatInnerData(unsigned long hits, unsigned long hits_empty_page)
    :
      hits_(hits),
  ...
  struct SearchEngineStatKey
  {
    ...
    SearchEngineStatKey(const DayTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
