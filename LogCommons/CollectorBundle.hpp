...
namespace AdServer::LogProcessing::CollectorBundleImplDefs_
{
  ...
  struct SizeCalculator<COLLECTOR_TYPE_, true>
  {
    static std::size_t calc_size(const COLLECTOR_TYPE_& collector)
    {
      return std::accumulate(collector.begin(), collector.end(), 0, add_size_);
    }
...
namespace AdServer::LogProcessing
{
    ...
    public:
      ...
      typedef SpilloverImpl<typename BundleType::CollectorT>
        SpilloverType;
      typedef typename SpilloverImpl<typename BundleType::CollectorT>::SmartPtr Spillover_var;
  ...
  public:
    ...
    CollectorBundle(const CollectorBundleParams& params, const ThreadLogSaver_var& saver)
      noexcept;

    void
    merge(CollectorT& file_collector, const CollectorBundleFileGuard_var& file_handle)
      /*throw(eh::Exception)*/;
