...
namespace AdServer::LogProcessing
{
  ...
  public:
    explicit
    ChannelInventoryInnerKey(std::uint32_t channel_id = 0)
    :
  ...
  public:
    ...
    bool operator==(const ChannelInventoryInnerData& rhs) const
    {
      ...
      return meaningful_data_ == rhs.meaningful_data_ &&
        ...
        active_user_count_ == rhs.active_user_count_ && total_user_count_ == rhs.total_user_count_;
    }
  ...
  struct ChannelInventoryKey
  {
    ...
    ChannelInventoryKey(const DayTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
