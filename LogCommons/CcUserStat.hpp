...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    CcUserStatInnerKey(std::uint32_t cc_id, const OptionalDayTimestamp& last_appearance_date)
    :
      cc_id_(cc_id),
    ...
    bool operator==(const CcUserStatInnerKey& rhs) const
    {
      ...
      return cc_id_ == rhs.cc_id_ && last_appearance_date_ == rhs.last_appearance_date_;
    }
  ...
  private:
    ...
    void invariant() const /*throw(eh::Exception)*/
    {
      if (!cc_id_)
      {
        throw ConstraintViolation("CcUserStatInnerKey::invariant(): " "cc_id_ must be > 0");
      }
    }
  ...
  public:
    ...
    CcUserStatInnerData(long unique_users)
    :
  ...
  struct CcUserStatKey
  {
    ...
    CcUserStatKey(const DayTimestamp& adv_sdate, std::uint32_t colo_id)
    :
  ...
  public:
    ...
    explicit
    CcStatToCcUserStatLoader(const CollectorBundlePtrT& bundle)
    :
      bundle_(bundle)
    {
    }
