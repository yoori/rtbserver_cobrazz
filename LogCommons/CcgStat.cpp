...
namespace AdServer::LogProcessing
{
  ...
  template <> const char* CampaignStatTraits::B::base_name_ = "CampaignStat";
  template <> const char* CampaignStatTraits::B::signature_ = "CampaignStat";
  template <> const char* CampaignStatTraits::B::current_version_ =
    "1.1"; // Last change: AdServer v2.1

  template <> const char* AdvertiserUserStatTraits::B::base_name_ = "AdvertiserUserStat";
  template <> const char* AdvertiserUserStatTraits::B::signature_ = "AdvertiserUserStat";
  template <> const char* AdvertiserUserStatTraits::B::current_version_ = "2.5";
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      if (is.eof())
      {
        ...
        for (OldCollectorT::DataT::const_iterator it = old_data.begin(); it != old_data.end(); ++it)
        {
          // Reach conversion
          {
            const OldCollectorT::DataT::DataT::ReachRecord& reach = it->second.reach();
            ...
            unsigned long reach_diff = reach.daily_reach() - reach.monthly_reach();
            if (reach_diff)
            {
            ...
            if (reach.total_reach())
            {
              ...
              data.add(inner_key, CollectorT::DataT::DataT(reach.total_reach(), 0, 0));
            }
          }
          // Text reach conversion
          {
            const OldCollectorT::DataT::DataT::ReachRecord& reach = it->second.text_reach();
            if (reach.daily_reach() < reach.monthly_reach())
            {
              Stream::Error es;
              es << __PRETTY_FUNCTION__ << ": AdvertiserUserStat_V_1_0::"
            ...
            if (reach.total_reach())
            {
              CollectorT::DataT::KeyT
                inner_key(it->first.adv_account_id(), OptionalDayTimestamp());
              data.add(inner_key, CollectorT::DataT::DataT(0, reach.total_reach(), 0));
            }
          }
          // Display reach conversion
          {
            const OldCollectorT::DataT::DataT::ReachRecord& reach = it->second.display_reach();
            if (reach.daily_reach() < reach.monthly_reach())
            {
              Stream::Error es;
              es << __PRETTY_FUNCTION__ << ": AdvertiserUserStat_V_1_0::"
            ...
            if (reach.total_reach())
            {
              CollectorT::DataT::KeyT
                inner_key(it->first.adv_account_id(), OptionalDayTimestamp());
              data.add(inner_key, CollectorT::DataT::DataT(0, 0, reach.total_reach()));
            }
          }
        }
    ...
    {
      Stream::Error es;
      es << __PRETTY_FUNCTION__ << ": Error: Malformed log file " << "(extra data at the of file)";
      throw Exception(es);
    }
  }
