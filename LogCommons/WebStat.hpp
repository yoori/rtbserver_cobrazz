...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    bool operator==(const WebStatInnerKey_V_3_1& rhs) const
    {
      ...
      return ct_ == rhs.ct_ &&
        ...
        test_ == rhs.test_ && tag_id_ == rhs.tag_id_ && cc_id_ == rhs.cc_id_;
    }
  ...
  public:
    ...
    WebStatInnerKey(const WebStatInnerKey_V_3_1& key)
      /*throw(eh::Exception)*/
    :
      ct_(key.ct()),
      curct_(key.curct()),
  ...
  struct WebStatKey
  {
    ...
    WebStatKey(const DayHourTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
  ...
  private:
    ...
    void invariant_() const /*throw(ConstraintViolation)*/
    {
      if (!colo_id_)
      {
        throw ConstraintViolation("WebStatKey::invariant_(): " "colo_id_ must be > 0");
      }
    }
