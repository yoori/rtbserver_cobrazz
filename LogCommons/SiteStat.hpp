...
namespace AdServer::LogProcessing
{
  ...
  public:
    explicit
    SiteStatInnerKey(std::uint32_t site_id = 0)
    :
      site_id_(site_id)
    {
    }
  ...
  private:
    void invariant() const /*throw(eh::Exception)*/
    {
      if (!site_id_)
      {
        throw ConstraintViolation("SiteStatInnerKey::invariant(): " "site_id_ must be > 0");
      }
    }
  ...
  public:
    ...
    SiteStatInnerData(unsigned long daily_reach, unsigned long monthly_reach)
    :
    ...
    bool operator==(const SiteStatInnerData& rhs) const
    {
      ...
      return daily_reach_ == rhs.daily_reach_ && monthly_reach_ == rhs.monthly_reach_;
    }
  ...
  struct SiteStatKey
  {
    ...
    SiteStatKey(const DayTimestamp& isp_sdate, std::uint32_t colo_id)
    :
  ...
  private:
    ...
    void invariant() const /*throw(eh::Exception)*/
    {
      if (!colo_id_)
      {
        throw ConstraintViolation("SiteStatKey::invariant(): " "colo_id_ must be > 0");
      }
    }
