...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    CampaignUserStatInnerKey(std::uint32_t cmp_id, const OptionalDayTimestamp& last_appearance_date)
    :
      cmp_id_(cmp_id),
    ...
    bool operator==(const CampaignUserStatInnerKey& rhs) const
    {
      ...
      return cmp_id_ == rhs.cmp_id_ && last_appearance_date_ == rhs.last_appearance_date_;
    }
  ...
  private:
    ...
    void invariant() const
    {
      if (!cmp_id_)
      {
        throw ConstraintViolation("CampaignUserStatInnerKey::invariant(): " "cmp_id_ must be > 0");
      }
    }
  ...
  public:
    ...
    CampaignUserStatInnerData(long unique_users)
    :
  ...
  struct CampaignUserStatKey
  {
    ...
    CampaignUserStatKey(const DayTimestamp& adv_sdate, std::uint32_t colo_id)
    :
  ...
  public:
    ...
    explicit
    CampaignStatToCampaignUserStatLoader(const CollectorBundlePtrT& bundle)
    :
      bundle_(bundle)
    {
    }
