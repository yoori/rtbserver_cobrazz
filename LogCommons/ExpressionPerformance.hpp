...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    ExpressionPerformanceInnerKey(std::uint32_t cc_id, const std::string& expression)
    :
      cc_id_(cc_id),
  ...
  public:
    ...
    bool operator==(const ExpressionPerformanceInnerData& rhs) const
    {
      ...
      return imps_verified_ == rhs.imps_verified_ &&
        clicks_ == rhs.clicks_ && actions_ == rhs.actions_;
    }
  ...
  struct ExpressionPerformanceKey
  {
    ...
    ExpressionPerformanceKey(const DayTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
