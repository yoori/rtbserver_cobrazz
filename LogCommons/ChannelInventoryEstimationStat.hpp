...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    ChannelInventoryEstimationStatInnerKey(std::uint32_t channel_id, const LevelT& level)
    :
    ...
    void invariant() const /*throw(eh::Exception)*/
    {
      if (level_ < LevelT::ZERO || level_ > max_level_value_)
      {
        ...
        es << "ChannelInventoryEstimationStatInnerKey::invariant(): "
           << "level_ = " << level_ << " (must be 0.0 <= level_ <= " << max_level_value_ << ")";
  ...
  public:
    ...
    bool operator==(const ChannelInventoryEstimationStatInnerData& rhs) const
    {
      ...
      return users_regular_ == rhs.users_regular_ && users_from_now_ == rhs.users_from_now_;
    }
  ...
  struct ChannelInventoryEstimationStatKey
  {
    ...
    ChannelInventoryEstimationStatKey(const DayTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
