...
namespace AdServer::LogProcessing
{
  namespace
  {
    ...
    void
    parse_log_file_name(const std::string& log_file_name, FileNameInfo& info)
      /*throw(eh::Exception)*/
    {
      if (log_file_name[log_file_name.size() - 1] == REGULAR_FILE_IN_COMMIT_MODE_SUFFIX)
      {
  ...
  //
  FileReceiver::FileGuard::FileGuard(const char* full_path)
    /*throw(eh::Exception)*/
    : file_name_(Generics::DirSelect::file_name(full_path)),
      full_path_(full_path)
  {}

  FileReceiver::FileGuard::FileGuard(FileReceiver* file_receiver, const char* file_name)
    /*throw(eh::Exception)*/
  ...
  bool
  FileReceiver::fetch_files(const char* input_dir, const char* prefix)
    /*throw(Interrupted, eh::Exception)*/
  {
  ...
  const std::string&
  FileReceiver::add_file_i_(const std::string& file_name)
    /*throw(eh::Exception)*/
  {
  ...
  FileReceiver::fetch_intermediate_dir_() /*throw(Interrupted, eh::Exception)*/
  {
    ...
    catch (const eh::Exception&)
    {
      ...
      if (oldest_not_in_files_.empty() || oldest_not_in_files_ > oldest_not_in_files_backup)
      {
  ...
  void
  FileReceiver::fetch_files_handler_(const char* full_path, const std::string& prefix)
    /*throw(eh::Exception)*/
  {
    ...
    if (file_info.type == FileNameInfo::COMMIT_FILE)
    {
      ...
      {
        fetch_file_(file_info.regular_file_name, regular_full_path);
      }
      else if (logger_)
      {
  ...
  void
  FileReceiver::fetch_file_(const std::string& name, const std::string& full_path)
    /*throw(eh::Exception)*/
  {
  ...
  void
  FileReceiver::fetch_intermediate_dir_handler_(const char* full_path, const std::string&)
    /*throw(eh::Exception)*/
  {
  ...
  void
  FileReceiver::dir_select_external_(const char* input_dir, const char* prefix)
    /*throw(Interrupted, eh::Exception)*/
  {
  ...
  void
  FileReceiver::rename_(const char* old_name, const char* new_name)
    /*throw(eh::Exception)*/
  {
    ...
    /*throw(eh::Exception)*/
  {
    if (::unlink(name))
    {
      eh::throw_errno_exception<Exception>(FNE, "failed to unlink file '", name, "'");
    }
  }
  ...
  void
  FileReceiver::check_top_changed_(const std::string& top, const std::string& file_name)
    noexcept
  {
    if (top == file_name)
    {
      ...
      logger_(ReferenceCounting::add_ref(logger))
  {
    ...
    file_receiver_ = new FileReceiver(intermediate_dir, max_files_to_store, interrupter, logger);

    add_child_object(Generics::ActiveObject_var(
      new FileThreadProcessorPool(file_processor, file_receiver_, callback, threads_number)));
