...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    static
    Key
    key(LogType, const std::string* log_file_name = nullptr)
      noexcept
    {
      if (!log_file_name)
      {
  ...
  private:
    ...
    public:
      FileReceiverHolder(LogType t, FileReceiver* fr, FileReceiverFacade& f)
        noexcept;
  ...
  protected:
    ...
    find_holder_i_(FileReceivers& file_receivers, LogType log_type)
      noexcept;

    friend bool operator<(const FileReceiverHolder_var& arg1, const FileReceiverHolder_var& arg2)
    {
...
namespace AdServer::LogProcessing
{
  ...
  template<typename OrderStrategy>
  void
  FileReceiverFacade<OrderStrategy>::FileReceiverHolder::on_top_changed(const std::string& new_top)
    noexcept
  {
  ...
  FileReceiverFacade<OrderStrategy>::FileReceiverFacade(
    ...
    : strategy_(strategy)
  {
    for (auto it = file_receivers.begin(); it != file_receivers.end(); ++it)
    {
      file_receivers_.emplace_back(
        FileReceiverHolder_var(new FileReceiverHolder(it->first, it->second, *this)));
  ...
  FileReceiverFacade<OrderStrategy>::find_holder_i_(FileReceivers& file_receivers, LogType log_type)
    noexcept
  {
    auto it = file_receivers.begin();

    while (it != file_receivers.end() && (*it)->type != log_type)
    {
      ++it;
    }
  ...
  FileReceiverFacade<OrderStrategy>::get_eldest(unsigned long timeout)
    /*throw(eh::Exception, Interrupted)*/
  {
    FileEntity res;
    ...
    while (active() && !res.file_guard && !interrupt_time.time_is_up())
    {
      ...
      if (holder)
      {
        ...
        else
        {
          ...
          file_receivers_.sort(FileReceiverFacade<OrderStrategy>::compare_file_receiver_holder_var);
        }
      }
    }
