...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    friend
    std::istream& operator>>(std::istream &is, GenericLogHeader<TRAITS_> &header);

    template <class TRAITS_>
    friend
    std::ostream& operator<<(std::ostream &os, const GenericLogHeader<TRAITS_> &header);
  ...
  public:
      ...
      /*throw(Exception)*/
    {
      ...
      {
        Stream::Error es;
        es << __PRETTY_FUNCTION__ << ": Error: Malformed log file (extra data at the end of file)";
        throw Exception(es);
      }
    }
      ...
      {
        {
          ...
          for (typename CollectorT::DataT::iterator it = data.begin(); it != data.end();)
          {
  ...
  public:
    ...
    :
      collector_(collector)
    {
      add_loader_(Traits::current_version(), new typename Traits::LoaderType(collector_));
  ...
  private:
    ...
    struct PreviousVersionSupportAdder_
    {
      ...
      void support_current(const char* version) /*throw(eh::Exception)*/
      {
  ...
  public:
    GenericLogSaverImpl(typename Base::CollectorT& collector, const std::string& path)
      : Base(path),
        collector_(collector)
    {}
  ...
  public:
    ...
    GenericLogSaverImpl(const std::string& path, CollectorFilterT* collector_filter)
