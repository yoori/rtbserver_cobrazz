...
namespace AdServer::LogProcessing
{
  ...
  struct PageLoadsDailyStatKey_V_1_0
  {
    ...
    PageLoadsDailyStatKey_V_1_0(const DayTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
  ...
  private:
    ...
    public:
      ...
      bool operator==(const DataHolder& rhs) const
      {
        ...
        return site_id == rhs.site_id && country == rhs.country && tags == rhs.tags;
      }
  ...
  public:
    ...
    PageLoadsDailyStatInnerData(long page_loads, long utilized_page_loads)
    :
    ...
    bool operator==(const PageLoadsDailyStatInnerData& rhs) const
    {
      ...
      return page_loads_ == rhs.page_loads_ && utilized_page_loads_ == rhs.utilized_page_loads_;
    }
  ...
  struct PageLoadsDailyStatKey
  {
    ...
    PageLoadsDailyStatKey(const DayHourTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
    ...
    PageLoadsDailyStatKey(const PageLoadsDailyStatKey_V_1_0& key)
    :
  ...
  private:
    ...
    void invariant() const /*throw(eh::Exception)*/
    {
      if (!colo_id_)
      {
        throw ConstraintViolation("PageLoadsDailyStatKey::invariant(): " "colo_id_ must be > 0");
      }
    }
