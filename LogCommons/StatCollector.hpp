...
namespace AdServer::LogProcessing
{
  ...
  namespace StatCollectorImplDefs_
  {
    ...
    struct ValueReader<STAT_COLLECTOR_, false>
    {
      ...
      )
      {
        ...
        if (!(is >> static_cast<KeyT&>(value.first)))
        {
          Stream::Error es;
    ...
    struct ValueReader<STAT_COLLECTOR_, true>
    {
        ...
        unsigned long& line_num)
      {
        ...
        if (is.good())
        {
          ...
          if (!(fbs >> static_cast<KeyT&>(value.first)))
          {
            Stream::Error es;
            es << __PRETTY_FUNCTION__
               << ": Failed to read key from istream (line number = " << line_num
  ...
  class StatCollector
  {
    ...
    typedef class StatCollectorImplDefs_::CopyImpl<StatCollector, IsCollector<DATA_>::value>
      CopyImpl_;
  ...
  public:
    ...
    typedef KEY_ KeyT;
    typedef DATA_ DataT;
    using LoadValueT = std::pair<KeyT, DataT>;
    ...
    StatCollector&
    add(const KeyT& key, const Mediator& data)
    {
  ...
  private:
    ...
    struct MapImplTypedefHelper<M_KEY_, M_DATA_, false>
    {
      ...
      struct Type: private ArenaHolder_, public Base_
      {
        Type()
          : ArenaHolder_(),
            Base_(
  ...
  private:
    ...
    static
    auto
    merge_data_i_(Data& dest, Data& src, int) -> decltype(dest.merge(std::move(src)), void())
    {
    ...
    static
    auto
    merge_data_i_(Data& dest, Data& src, long) -> decltype(dest.merge(src), void())
    {
      dest.merge(src);
    }
    ...
    /*throw(eh::Exception, Exception)*/
  {
    ...
    for (; !is.eof(); ++line_num)
    {
      ...
      auto ins_res = dest.map_impl_->try_emplace(std::move(value.first), std::move(value.second));
    ...
    /*throw(eh::Exception, Exception)*/
  {
    ...
    for (; is.peek(), !is.eof(); ++line_num)
    {
      DataT& value = tmp.append_default_();
      value_reader.read(is, value, line_num);
