...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    :
      holder_(new DataHolder(action_id, country_code, action_referrer_url, user_status))
    {
    }
    ...
    public:
      ...
      bool operator==(const DataHolder& key) const
      {
        return action_id == key.action_id &&
          ...
          action_referrer_url == key.action_referrer_url && user_status == key.user_status;
      }
  ...
  public:
    ...
    ActionRequestInnerData(unsigned long action_request_count, const FixedNum& cur_value)
    :
    ...
    bool operator==(const ActionRequestInnerData& rhs) const
    {
      ...
      return action_request_count_ == rhs.action_request_count_ && cur_value_ == rhs.cur_value_;
    }
  ...
  struct ActionRequestKey
  {
    ...
    ActionRequestKey(const DayHourTimestamp& sdate, std::uint32_t colo_id)
    :
      sdate_(sdate),
      colo_id_(colo_id),
      hash_()
