...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    CcgUserStatInnerKey(std::uint32_t ccg_id, const OptionalDayTimestamp& last_appearance_date)
    :
      ccg_id_(ccg_id),
    ...
    bool operator==(const CcgUserStatInnerKey& rhs) const
    {
      ...
      return ccg_id_ == rhs.ccg_id_ && last_appearance_date_ == rhs.last_appearance_date_;
    }
  ...
  private:
    ...
    void invariant() const /*throw(eh::Exception)*/
    {
      if (!ccg_id_)
      {
        throw ConstraintViolation("CcgUserStatInnerKey::invariant(): " "ccg_id_ must be > 0");
      }
    }
  ...
  public:
    ...
    explicit
    CcgUserStatInnerData(long unique_users)
    :
    ...
    friend
    BufferWriter& operator<<(BufferWriter& out, const CcgUserStatInnerData& data)
      /*throw(eh::Exception)*/;
  ...
  struct CcgUserStatKey
  {
    ...
    CcgUserStatKey(const DayTimestamp& adv_sdate, std::uint32_t colo_id)
    :
    ...
    bool operator==(const CcgUserStatKey& rhs) const
    {
      ...
      return adv_sdate_ == rhs.adv_sdate_ && colo_id_ == rhs.colo_id_;
    }
  ...
  public:
    ...
    friend
    BufferWriter&
    operator<<(BufferWriter& out, const CcgUserStatKey& key) /*throw(eh::Exception)*/;
  ...
  public:
    ...
    explicit
    CcgStatToCcgUserStatLoader(const CollectorBundlePtrT& bundle)
    :
      bundle_(bundle)
    {
    }
      ...
      try
      {
        ...
        if (is.eof())
        {
          ...
          )
          {
            ...
            unsigned long reach_diff = it->second.daily_reach() - it->second.monthly_reach();
            if (reach_diff)
            {
            ...
            if (it->second.total_reach())
            {
              ...
              data.add(inner_key, CollectorT::DataT::DataT(it->second.total_reach()));
            }
          }
