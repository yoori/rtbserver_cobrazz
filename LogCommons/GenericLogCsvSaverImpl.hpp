...
namespace AdServer::LogProcessing
{
  ...
  struct CsvWrite<LogExtTraits, false, false>
  {
    static
    void
    impl(std::ofstream& ofs, typename LogExtTraits::CollectorType::const_iterator it)
      /*throw(eh::Exception)*/
    {
  ...
  public:
    ...
    SimpleLogCsvSaverImpl(CollectorT& collector, const std::string& path)
    :
    ...
    try
    {
      ...
      if (!ofs)
      {
        ...
        es << __PRETTY_FUNCTION__ <<
          ": Error: Failed to write log header to file '" << filenames.second << '\'';
        throw CsvException(es);
      }
      ...
      if (!ofs)
      {
        Stream::Error es;
        es << __PRETTY_FUNCTION__ <<
          ": Error: Failed to write log data to file '" << filenames.second << '\'';
        throw CsvException(es);
      }
      ofs.close();
  ...
  public:
    ...
    GenericLogCsvSaverImpl(const std::string& path, CollectorFilterT* collector_filter)
      /*throw(Exception)*/
    :
