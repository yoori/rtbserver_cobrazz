...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    ChannelCountStatInnerKey(char channel_type, unsigned long channel_count)
    :
    ...
    bool operator==(const ChannelCountStatInnerKey& rhs) const
    {
      ...
      return channel_type_ == rhs.channel_type_ && channel_count_ == rhs.channel_count_;
    }
  ...
  public:
    explicit
    ChannelCountStatInnerData(unsigned long users_count = 0)
    :
  ...
  struct ChannelCountStatKey
  {
    ...
    ChannelCountStatKey(const DayTimestamp& isp_sdate, std::uint32_t colo_id)
    :
