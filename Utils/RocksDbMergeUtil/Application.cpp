...
namespace
{
  ...
  open_input_db(const std::filesystem::path& path)
  {
    ...
    if(!status.ok())
    {
      throw std::runtime_error(
        "can't open input DB " + path.string() + ": " + status.ToString());
    }
  ...
  open_output_db(const std::filesystem::path& path)
  {
    ...
    {
      throw std::runtime_error(
        "can't open output DB " + path.string() + ": " + status.ToString());
    }
    ...
    MergeStats& stats)
  {
    ...
    std::unique_ptr<rocksdb::Iterator> iterator(
      input_db->NewIterator(read_options));
    ...
    for(iterator->SeekToFirst(); iterator->Valid(); iterator->Next())
    {
      ...
      if(batch_records >= WRITE_BATCH_RECORDS_LIMIT ||
        batch_bytes >= WRITE_BATCH_BYTES_LIMIT)
      {
...
main(int argc, char** argv)
{
  try
  {
    ...
    {
      throw std::runtime_error(
        "can't close output DB: " + close_status.ToString());
    }
