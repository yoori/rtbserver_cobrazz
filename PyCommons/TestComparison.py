  ...
  def assertHasItem( self, item, gotList, desc = 'item' ):
    ...
    for r in gotList:
      try:
        ...
        pass  # try another
      i += 1
    self.fail('No %s (%s) found in %s' % (desc, self.__v2str(item), map(self.__v2str, gotList)))

  def assertHasItems( self, items, gotList, desc = 'item' ):
    for item in items:
      self.assertHasItem(item, gotList, desc)

  def assertHasNoItems( self, items, gotList, desc = 'item' ):
    index = 0
    for item in items:
      try:
        index = self.assertHasItem(item, gotList, desc)
      except:
        pass
      if index:
        self.fail('Unexpected#%d %s (%s) found' % (index, desc, self.__v2str(item)))
