...
namespace AdServer::CampaignSvcs
{
  ...
  using ChannelIdHashSet = Generics::MonoUnorderedSet<ChannelId>;
  ...
  struct ChannelParams
  {
    ...
    {
    public:
      DiscoverParams(const char* query_val, const char* annotation_val) noexcept;
    ...
    public:
      ...
      {
        return name == right.name && parent_channel_id == right.parent_channel_id;
      }
  ...
  public:
    ...
    virtual bool
    triggered_expression(std::string& responded_expr, const ChannelIdHashSet& triggered_channels)
      const
      /*throw(Exception, eh::Exception)*/ = 0;

    virtual void
    get_cmp_channels(ExpressionChannelList& cmp_channels, const ChannelIdHashSet& simple_channels)
      /*throw(Exception)*/ = 0;

    virtual void get_all_cmp_channels(ExpressionChannelList& cmp_channels)
      /*throw(Exception)*/ = 0;

    virtual void get_all_channels(ChannelIdSet& channels)
      noexcept = 0;
  ...
  public:
    ...
    ExpressionChannel(const ChannelParams& channel_params, const Expression& expression_val)
      noexcept
      : ExpressionChannelBaseWithParams(channel_params),
        expr_(expression_val)
    {};

    ExpressionChannel(const ChannelParams& channel_params, Expression&& expression_val)
      noexcept
      : ExpressionChannelBaseWithParams(channel_params),
        expr_(std::move(expression_val))
    {};
  ...
  private:
    ...
    static void
    get_all_cmp_channels_(ExpressionChannelList& cmp_channels, const Expression& expr)
      /*throw(Exception)*/;

    static void get_all_channels_(ChannelIdSet& channels, const Expression& expr)
      noexcept;
  ...
  public:
    ...
    virtual void
    triggered_named_channels(ChannelIdSet&, const ChannelIdHashSet&) const
      /*throw(Exception)*/
    {}

    virtual bool
    triggered_expression(std::string&, const ChannelIdHashSet&) const
      /*throw(Exception, eh::Exception)*/
    {
      return false;
    }
    ...
    virtual void
    get_cmp_channels(ExpressionChannelList&, const ChannelIdHashSet&)
      /*throw(Exception)*/
    {}

    virtual void
    get_all_cmp_channels(ExpressionChannelList&)
      /*throw(Exception)*/
    {}

    virtual void
    get_all_channels(ChannelIdSet& channels)
      noexcept;
  ...
  protected:
    ...
    static bool
    match_cell_(const Expression& expr, const ChannelIdHashSet& channels);

    static void
    get_all_channels_(ChannelIdSet& channels, const Expression& expr)
      /*throw(eh::Exception)*/;
    ...
    static std::string&
    print_(std::string& out, const Expression& expr)
      /*throw(eh::Exception)*/;

    static
    void
    reordering_(Expression& expr, ChannelIdSet& basis)
      /*throw(eh::Exception)*/;
...
namespace AdServer::CampaignSvcs
{
  ...
  ChannelParams::CommonParams::operator==(const CommonParams& right) const noexcept
  {
    return account_id == right.account_id &&
      ...
      flags == right.flags && is_public == right.is_public && freq_cap_id == right.freq_cap_id;
  }
  ...
  inline
  bool
  ChannelParams::DiscoverParams::operator==(const DiscoverParams& right) const noexcept
  {
    return query == right.query &&
      annotation == right.annotation && create_time == right.create_time;
  }
  ...
  ChannelParams::CMPParams::operator==(const CMPParams& right) const noexcept
  {
    return channel_rate_id == right.channel_rate_id &&
      imp_revenue == right.imp_revenue && click_revenue == right.click_revenue;
  }

  inline
  bool ChannelParams::equal(const ChannelParams& right, bool status_check) const noexcept
  {
    return channel_id == right.channel_id &&
      type == right.type &&
      country == right.country &&
      (!status_check || status == right.status) &&
      ((common_params.in() && right.common_params.in() && *common_params == *right.common_params) ||
      ...
      ((discover_params.in() && right.discover_params.in() &&
        *discover_params == *right.discover_params) ||
        (!discover_params.in() && !right.discover_params.in())) &&
      ((cmp_params.in() && right.cmp_params.in() && *cmp_params == *right.cmp_params) ||
        (!cmp_params.in() && !right.cmp_params.in()));
  }

  inline
  void print_expression(std::string& out, const ExpressionChannel::Expression& expr, bool expand)
    noexcept
  {
    ...
    noexcept
  {
    ...
    {
      return left_simple_channel->params().equal(right_simple_channel->params());
    }
  ...
  inline
  ExpressionChannelHolder::ExpressionChannelHolder(ExpressionChannelBase* channel_val) noexcept
    ...
    noexcept
  {
    return op == right.op &&
      (op == NOP ?
       ...
       (!channel.in() && !right.channel.in()) :
      std::equal(sub_channels.begin(), sub_channels.end(), right.sub_channels.begin()));
  }
}
