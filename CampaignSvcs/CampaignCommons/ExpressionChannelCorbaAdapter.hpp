...
namespace AdServer::CampaignSvcs
{
  ...
  inline
  void pack_channel_params(ExpressionChannelInfo& channel_info, const ChannelParams& channel_params)
  {
    ...
    if (channel_params.cmp_params.in())
    {
      ...
      channel_info.imp_revenue = CorbaAlgs::pack_decimal(channel_params.cmp_params->imp_revenue);
    ...
    const ExpressionChannelInfo& channel_info)
  {
    ...
    ChannelParams::CommonParams_var common_params(new ChannelParams::CommonParams());
  ...
  inline
  void pack_simple_channel(ExpressionChannelInfo& channel_info, const SimpleChannel* simple_channel)
    /*throw(eh::Exception)*/
  {
...
//    channel_info.threshold = simple_channel->threshold();
  ...
  {
    expression.op = static_cast<ExpressionChannel::Operation>(expression_info.operation);

    if (expression.op == ExpressionChannel::NOP)
    {
      if (expression_info.channel_id)
      {
        typename ChannelContainerType::iterator ch_it = channels.find(expression_info.channel_id);
        ...
        {
          ExpressionChannelHolder_var channel_holder(new ExpressionChannelHolder());
          channels.insert(std::make_pair(expression_info.channel_id, channel_holder));
  ...
  inline
  void pack_channel(ExpressionChannelInfo& channel_info, const ExpressionChannelBase* channel)
    noexcept
  {
