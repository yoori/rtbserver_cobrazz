...
namespace AdServer::CampaignSvcs
{
  ...
  struct ChannelIdSortedArray: protected std::vector<unsigned long>
  {
    ...
    {
      return std::find(std::vector<unsigned long>::begin(), std::vector<unsigned long>::end(), val);
    }
  ...
  using AllowedDurationSet = Generics::MonoSet<unsigned long>;
  ...
  inline
  void
  narrow_decimal(ExtRevenueDecimal& result, const RevenueDecimal& source)
  {
  ...
  inline
  void
  narrow_decimal(RevenueDecimal& result, const ExtRevenueDecimal& source)
  {
    result = RevenueDecimal(ExtRevenueDecimal(source).floor(RevenueDecimal::FRACTION_RANK).str());
  }
...
{
  inline
  bool CampaignDeliveryLimits::operator==(const CampaignDeliveryLimits& right) const
    noexcept
  {
    return date_start == right.date_start &&
      ...
      clicks == right.clicks && daily_clicks == right.daily_clicks;
  }
  ...
  {
    return size() == right.size() && std::equal(begin(), end(), right.begin(), Algs::PairEqual());
  }
  ...
  {
    return option_id == right.option_id && value == right.value;
  }
