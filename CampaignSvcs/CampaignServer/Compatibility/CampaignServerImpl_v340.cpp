...
#include "CampaignServerImpl_v340.hpp"

namespace
{
  void
  copy_price_range_seq(
    AdServer::CampaignSvcs_v340::PriceRangeSeq& tgt,
    const AdServer::CampaignSvcs_v350::PriceRangeSeq& src)
    noexcept
  {
  ...
  void
  copy_option_values(
    AdServer::CampaignSvcs_v340::OptionValueSeq& tgt,
    const AdServer::CampaignSvcs_v350::OptionValueSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  copy_creative_size_info(
    AdServer::CampaignSvcs_v340::CreativeSizeSeq& tgt,
    const AdServer::CampaignSvcs_v350::CreativeSizeSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  void
  copy_tag_size_info(
    AdServer::CampaignSvcs_v340::TagSizeSeq& tgt,
    const AdServer::CampaignSvcs_v350::TagSizeSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  void
  copy_channel_expr(
    AdServer::CampaignSvcs_v340::ExpressionInfo& result_expr,
    const AdServer::CampaignSvcs_v350::ExpressionInfo& expr)
  {
  ...
  void copy_channel(
    AdServer::CampaignSvcs_v340::ExpressionChannelInfo& result_channel_info,
    const AdServer::CampaignSvcs_v350::ExpressionChannelInfo& channel_info)
  {
  ...
  copy_simple_channels(
    const AdServer::CampaignSvcs_v350::SimpleChannelKeySeq& simple_channel_seq,
    AdServer::CampaignSvcs_v340::SimpleChannelKeySeq& result_simple_channel_seq,
    const Generics::Time& request_timestamp)
  {
  ...
  void convert_bp_seq(
    AdServer::CampaignSvcs_v340::BehavParameterSeq& result,
    const AdServer::CampaignSvcs_v350::BehavParameterSeq& src)
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::BehavParameter& bp_el = src[bp_el_i];
      AdServer::CampaignSvcs_v340::BehavParameter& result_bp_el = result[res_el_i++];
  ...
  void convert_ext_categories(
    AdServer::CampaignSvcs_v340::ExternalCategorySeq& result,
    const AdServer::CampaignSvcs_v350::ExternalCategorySeq& src)
  {
    result.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
      const AdServer::CampaignSvcs_v350::ExternalCategoryInfo& cat = src[i];
      AdServer::CampaignSvcs_v340::ExternalCategoryInfo& res = result[i];
...
namespace CampaignSvcs
{
  namespace Aspect
  {
    const char CAMPAIGN_SERVER_v340[] = "CampaignServer_v340";
  }

  CampaignServerImpl_v340::CampaignServerImpl_v340(
    Logging::Logger* logger,
    POA_AdServer::CampaignSvcs_v350::CampaignServer* campaign_server)
  ...
  CampaignServerImpl_v340::~CampaignServerImpl_v340() noexcept
  {}

  void
  CampaignServerImpl_v340::convert_config_(
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const Generics::Time& request_timestamp)
  {
    /* fill global params */
    ...
    result_update_info.tanx_publisher_account_id = update_info.tanx_publisher_account_id;
  ...
  void
  CampaignServerImpl_v340::convert_get_config_settings_(
    const AdServer::CampaignSvcs_v340::CampaignGetConfigSettings& settings,
    AdServer::CampaignSvcs_v350::CampaignGetConfigSettings& result_settings)
  {
  ...
  AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo*
  CampaignServerImpl_v340::get_config(
    const AdServer::CampaignSvcs_v340::CampaignGetConfigSettings& settings)
  {
    static const char* FUN = "CampaignServerImpl_v340::get_config()";

    try
    {
      AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo_var campaign_config;
      AdServer::CampaignSvcs_v350::CampaignGetConfigSettings conv_settings;

      convert_get_config_settings_(settings, conv_settings);

      campaign_config = campaign_server_->get_config(conv_settings);

      AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo_var result_campaign_config =
        new AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo();
    ...
    catch(const AdServer::CampaignSvcs_v350::CampaignServer::ImplementationException& ex)
    {
      Stream::Error ostr;
      ostr << FUN << ": AdServer::CampaignSvcs_v350::CampaignServer::"
        "ImplementationException caught: " <<
        ex.description;

      CORBACommons::throw_desc<
        CampaignSvcs_v340::CampaignServer::ImplementationException>(
          ostr.str());
    }
    catch(const AdServer::CampaignSvcs_v350::CampaignServer::NotReady& ex)
    {
      throw AdServer::CampaignSvcs_v340::CampaignServer::NotReady(ex.description);
    }
  ...
  AdServer::CampaignSvcs_v340::ColocationFlagsSeq*
  CampaignServerImpl_v340::get_colocation_flags()
  {
    try
    {
      AdServer::CampaignSvcs_v350::ColocationFlagsSeq_var flags_src =
        campaign_server_->get_colocation_flags();

      AdServer::CampaignSvcs_v340::ColocationFlagsSeq_var flags_dest =
        new AdServer::CampaignSvcs_v340::ColocationFlagsSeq;

      flags_dest->length(flags_src->length());
      for(CORBA::ULong i = 0; i < flags_dest->length(); ++i)
      {
        const AdServer::CampaignSvcs_v350::ColocationFlags& flag_src =
          flags_src[i];
        AdServer::CampaignSvcs_v340::ColocationFlags& flag_dst = 
          flags_dest[i];
    ...
    {
      Stream::Error ostr;
      ostr << __func__ << ": AdServer::CampaignSvcs_v350::CampaignServer::"
        "ImplementationException caught: " <<
        ex.description;
  ...
  AdServer::CampaignSvcs_v340::EcpmSeq*
  CampaignServerImpl_v340::get_ecpms(
    const AdServer::CampaignSvcs_v340::TimestampInfo& request_timestamp)
  {
    static const char* FUN = "CampaignServerImpl_v340::get_ecpms()";
      
    try
    {
      AdServer::CampaignSvcs_v350::EcpmSeq_var ecpms  =
        campaign_server_->get_ecpms(request_timestamp);
      AdServer::CampaignSvcs_v340::EcpmSeq_var result_ecpms =
        new AdServer::CampaignSvcs_v340::EcpmSeq();
    ...
    catch(const AdServer::CampaignSvcs_v350::CampaignServer::ImplementationException& e)
    {
      Stream::Error ostr;
      ostr << FUN << ": CampaignSvcs_v350::CampaignServer::"
        "ImplementationException caught: " << e.description;
      CORBACommons::throw_desc<
        CampaignSvcs_v340::CampaignServer::ImplementationException>(
          ostr.str());
    }
    catch(const AdServer::CampaignSvcs_v350::CampaignServer::NotReady& )
    {
      throw AdServer::CampaignSvcs_v340::CampaignServer::NotReady();
    }
    return 0; // never reach
  }

  AdServer::CampaignSvcs_v340::BriefSimpleChannelAnswer*
  CampaignServerImpl_v340::brief_simple_channels(
    const AdServer::CampaignSvcs_v340::CampaignServer::
    GetSimpleChannelsInfo& /*settings*/)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::brief_simple_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::SimpleChannelAnswer*
  CampaignServerImpl_v340::simple_channels(
    const AdServer::CampaignSvcs_v340::CampaignServer::GetSimpleChannelsInfo& /*settings*/)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::simple_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::ExpressionChannelsInfo*
  CampaignServerImpl_v340::get_expression_channels(
    const AdServer::CampaignSvcs_v340::CampaignServer::GetExpressionChannelsInfo&)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::get_expression_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::DetectorsConfig*
  CampaignServerImpl_v340::detectors(
    const AdServer::CampaignSvcs_v340::TimestampInfo& /*request_timestamp*/)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::search_engines(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::FreqCapConfigInfo*
  CampaignServerImpl_v340::freq_caps()
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::freq_caps(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::StatInfo*
  CampaignServerImpl_v340::get_stat()
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::NotSupport(
      "CampaignServerImpl_v340::get_stat(): Not supported.");
  }

  void
  CampaignServerImpl_v340::update_stat()
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::NotSupport(
      "CampaignServerImpl_v340::update_stat(): Not supported.");
  }

  CORBA::Boolean
  CampaignServerImpl_v340::need_config(
    const CORBACommons::TimestampInfo& master_stamp)
  {
    static const char* FUN = "CampaignServerImpl_v340::need_config()";

    try
    {
      return campaign_server_->need_config(master_stamp);
    }
    catch(const AdServer::CampaignSvcs_v340::
  ...
  AdServer::CampaignSvcs_v340::DiscoverSourceInfo*
  CampaignServerImpl_v340::get_discover_channels(
    CORBA::ULong, CORBA::ULong)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::get_discover_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::CampaignServer::PassbackInfo*
  CampaignServerImpl_v340::get_tag_passback(CORBA::ULong, const char*)
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::get_tag_passback(): Not supported.");
  }

  AdServer::CampaignSvcs_v340::FraudConditionConfig*
  CampaignServerImpl_v340::fraud_conditions()
  {
    throw AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException(
      "CampaignServerImpl_v340::fraud_conditions(): Not supported.");
  }

  void
  CampaignServerImpl_v340::convert_campaigns_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    const
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CampaignInfo& campaign_info =
        update_info.campaigns[i];

      if(CorbaAlgs::unpack_time(campaign_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CampaignInfo& result_campaign_info =
        ...
        {
          const AdServer::CampaignSvcs_v350::CreativeInfo& creative_info =
            campaign_info.creatives[j];

          AdServer::CampaignSvcs_v340::CreativeInfo& result_creative_info =
  ...
  void
  CampaignServerImpl_v340::convert_accounts_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::AccountInfo& account_info =
        update_info.accounts[i];

      if(CorbaAlgs::unpack_time(account_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::AccountInfo& result_account_info =
  ...
  void
  CampaignServerImpl_v340::convert_creative_options_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CreativeOptionInfo& option_info =
        update_info.activate_creative_options[i];

      if(CorbaAlgs::unpack_time(option_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CreativeOptionInfo& result_option_info =
  ...
  void
  CampaignServerImpl_v340::convert_expression_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v340::convert_geo_coord_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::GeoCoordChannelInfo& channel_info =
        update_info.activate_geo_coord_channels[ch_i];

      if(CorbaAlgs::unpack_time(channel_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::GeoCoordChannelInfo& result_channel_info =
  ...
  void
  CampaignServerImpl_v340::convert_block_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::BlockChannelInfo& channel_info =
        update_info.activate_block_channels[ch_i];

      if(CorbaAlgs::unpack_time(channel_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::BlockChannelInfo& result_channel_info =
  ...
  void
  CampaignServerImpl_v340::convert_geo_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::GeoChannelInfo& channel_info =
        update_info.activate_geo_channels[ch_i];

      if(CorbaAlgs::unpack_time(channel_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::GeoChannelInfo& result_channel_info =
  ...
  void
  CampaignServerImpl_v340::convert_ecpm_seq_(
    AdServer::CampaignSvcs_v340::EcpmSeq& result_ecpms,
    const AdServer::CampaignSvcs_v350::EcpmSeq& ecpms,
    const Generics::Time& request_timestamp)
    noexcept
  {
    /* fill ecpms */
    ...
    {
      const AdServer::CampaignSvcs_v350::CampaignEcpmInfo& ecpm_info = ecpms[i];

      if(CorbaAlgs::unpack_time(ecpm_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CampaignEcpmInfo& res_ecpm_info = result_ecpms[res_i++];
  ...
  void
  CampaignServerImpl_v340::convert_ecpms_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v340::convert_sites_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::SiteInfo& site_info = update_info.sites[i];

      if(CorbaAlgs::unpack_time(site_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::SiteInfo& result_site_info =
  ...
  void
  CampaignServerImpl_v340::convert_sizes_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::SizeInfo& size_info =
        update_info.sizes[i];
      if(CorbaAlgs::unpack_time(size_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::SizeInfo& result_size_info =
  ...
  void
  CampaignServerImpl_v340::convert_tags_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::TagInfo& tag_info =
         ...
         tag_info.sizes.length() > 0)
      {
        ...
        AdServer::CampaignSvcs_v340::TagInfo& result_tag_info =
        ...
        {
          const AdServer::CampaignSvcs_v350::TagPricingInfo& tag_pricing_info =
            tag_info.tag_pricings[j];
          AdServer::CampaignSvcs_v340::TagPricingInfo& result_tag_pricing_info =
  ...
  void
  CampaignServerImpl_v340::convert_frequency_caps_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::FreqCapInfo& freq_cap =
        update_info.frequency_caps[i];

      if(CorbaAlgs::unpack_time(freq_cap.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::FreqCapInfo& result_freq_cap =
  ...
  void
  CampaignServerImpl_v340::convert_simple_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v340::convert_behav_params_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    {
      ...
      {
        const AdServer::CampaignSvcs_v350::BehavParamInfo& bp =
          update_info.behav_params[i];

        if(CorbaAlgs::unpack_time(bp.timestamp) > request_timestamp)
        {
          AdServer::CampaignSvcs_v340::BehavParamInfo& result_bp =
    ...
    {
      const AdServer::CampaignSvcs_v350::KeyBehavParamInfo& bp =
        update_info.key_behav_params[i];
      if(CorbaAlgs::unpack_time(bp.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::KeyBehavParamInfo& result_bp =
  ...
  void
  CampaignServerImpl_v340::convert_colocations_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::ColocationInfo& colo_info =
        update_info.colocations[i];
      if(CorbaAlgs::unpack_time(colo_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::ColocationInfo& result_colo_info =
  ...
  void
  CampaignServerImpl_v340::convert_creative_templates_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CreativeTemplateInfo& cr_templ_info =
        update_info.creative_templates[i];
      if(CorbaAlgs::unpack_time(cr_templ_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CreativeTemplateInfo& result_cr_templ_info =
        ...
        {
          const AdServer::CampaignSvcs_v350::CreativeTemplateFileInfo& ctf_info =
            cr_templ_info.files[ctf_i];
          AdServer::CampaignSvcs_v340::CreativeTemplateFileInfo& result_ctf_info =
          ...
          result_ctf_info.type = static_cast<
            AdServer::CampaignSvcs_v340::CreativeTemplateType>(
              ctf_info.type);
        }
  ...
  void
  CampaignServerImpl_v340::convert_currencies_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CurrencyInfo& currency_info =
        update_info.currencies[i];

      if(CorbaAlgs::unpack_time(currency_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CurrencyInfo& result_currency_info =
  ...
  void
  CampaignServerImpl_v340::convert_campaign_keywords_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CampaignKeywordInfo& keyword_info =
        update_info.campaign_keywords[i];

      if(CorbaAlgs::unpack_time(keyword_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CampaignKeywordInfo& res_keyword_info =
  ...
  void
  CampaignServerImpl_v340::convert_creative_categories_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CreativeCategoryInfo& category_info =
        update_info.creative_categories[i];
      if(CorbaAlgs::unpack_time(category_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CreativeCategoryInfo& res_category_info =
  ...
  void
  CampaignServerImpl_v340::convert_adv_actions_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v340::convert_category_channels_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::CategoryChannelInfo& channel_category_info =
        update_info.category_channels[cat_i];
      if(CorbaAlgs::unpack_time(channel_category_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::CategoryChannelInfo& result_channel_category_info =
  ...
  void
  CampaignServerImpl_v340::convert_search_engines_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::SearchEngineInfo& s_info =
        update_info.search_engines[i];
      if(CorbaAlgs::unpack_time(s_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::SearchEngineInfo& result_s_info =
  ...
  void
  CampaignServerImpl_v340::convert_web_browsers_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::WebBrowserInfo& b_info =
        update_info.web_browsers[i];
      if(CorbaAlgs::unpack_time(b_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::WebBrowserInfo& result_b_info =
        ...
        {
          const AdServer::CampaignSvcs_v350::WebBrowserDetectorInfo& d_info =
            b_info.detectors[j];
          AdServer::CampaignSvcs_v340::WebBrowserDetectorInfo& result_d_info =
  ...
  void
  CampaignServerImpl_v340::convert_platforms_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::PlatformInfo& p_info =
        update_info.platforms[i];
      if(CorbaAlgs::unpack_time(p_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::PlatformInfo& result_p_info =
        ...
        {
          const AdServer::CampaignSvcs_v350::PlatformDetectorInfo& pd_info =
            p_info.detectors[j];
          AdServer::CampaignSvcs_v340::PlatformDetectorInfo& result_pd_info =
  ...
  void
  CampaignServerImpl_v340::convert_fraud_conditions_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::FraudConditionInfo& fraud_cond_info =
        update_info.fraud_conditions[i];
      if(CorbaAlgs::unpack_time(fraud_cond_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::FraudConditionInfo& result_fraud_cond_info =
  ...
  void
  CampaignServerImpl_v340::convert_string_dictionaries_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::WebOperationInfo& info =
        update_info.web_operations[i];
      if(CorbaAlgs::unpack_time(info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::WebOperationInfo& result_info =
  ...
  void
  CampaignServerImpl_v340::convert_app_formats_(
    AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v350::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v350::AppFormatInfo& info =
        update_info.app_formats[i];
      if(CorbaAlgs::unpack_time(info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v340::AppFormatInfo& result_info =
