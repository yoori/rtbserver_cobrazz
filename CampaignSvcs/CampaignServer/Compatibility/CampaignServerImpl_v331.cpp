...
#include "CampaignServerImpl_v331.hpp"

namespace
{
  void
  copy_price_range_seq(
    AdServer::CampaignSvcs_v330::PriceRangeSeq& tgt,
    const AdServer::CampaignSvcs_v340::PriceRangeSeq& src)
    noexcept
  {
  ...
  void
  copy_option_values(
    AdServer::CampaignSvcs_v330::OptionValueSeq& tgt,
    const AdServer::CampaignSvcs_v340::OptionValueSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  copy_creative_size_info(
    AdServer::CampaignSvcs_v330::CreativeSizeSeq& tgt,
    const AdServer::CampaignSvcs_v340::CreativeSizeSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  void
  copy_tag_size_info(
    AdServer::CampaignSvcs_v330::TagSizeSeq& tgt,
    const AdServer::CampaignSvcs_v340::TagSizeSeq& src)
  {
    tgt.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
  ...
  void
  copy_channel_expr(
    AdServer::CampaignSvcs_v330::ExpressionInfo& result_expr,
    const AdServer::CampaignSvcs_v340::ExpressionInfo& expr)
  {
  ...
  void copy_channel(
    AdServer::CampaignSvcs_v330::ExpressionChannelInfo& result_channel_info,
    const AdServer::CampaignSvcs_v340::ExpressionChannelInfo& channel_info)
  {
  ...
  copy_simple_channels(
    const AdServer::CampaignSvcs_v340::SimpleChannelKeySeq& simple_channel_seq,
    AdServer::CampaignSvcs_v330::SimpleChannelKeySeq& result_simple_channel_seq,
    const Generics::Time& request_timestamp)
  {
    ...
    for(CORBA::ULong i = 0; i < simple_channel_seq.length(); ++i)
    {
      ...
      if(CorbaAlgs::unpack_time(simple_channel_info.timestamp) > request_timestamp)
      {
        ...
        if (strchr(simple_channel_info.str_behav_param_list_id, 'A'))
        {//clear str_behav_param_list_id for audience channels in 3.3.1
          result_simple_channel_info.str_behav_param_list_id = "";
        }
        else
        {
  ...
  void convert_bp_seq(
    AdServer::CampaignSvcs_v330::BehavParameterSeq& result,
    const AdServer::CampaignSvcs_v340::BehavParameterSeq& src)
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::BehavParameter& bp_el = src[bp_el_i];
      if (bp_el.trigger_type != 'A')
      {//exclude behavioral parameters for audience channels, it is unknown in 3.3.1
        AdServer::CampaignSvcs_v330::BehavParameter& result_bp_el = result[res_el_i++];
  ...
  void convert_ext_categories(
    AdServer::CampaignSvcs_v330::ExternalCategorySeq& result,
    const AdServer::CampaignSvcs_v340::ExternalCategorySeq& src)
  {
    result.length(src.length());
    for(CORBA::ULong i = 0; i < src.length(); ++i)
    {
      const AdServer::CampaignSvcs_v340::ExternalCategoryInfo& cat = src[i];
      AdServer::CampaignSvcs_v330::ExternalCategoryInfo& res = result[i];
...
namespace CampaignSvcs
{
  namespace Aspect
  {
    const char CAMPAIGN_SERVER_v330[] = "CampaignServer_v330";
  }

  CampaignServerImpl_v331::CampaignServerImpl_v331(
  ...
  CampaignServerImpl_v331::~CampaignServerImpl_v331() noexcept
  {}

  void
  CampaignServerImpl_v331::convert_config_(
    ...
    const Generics::Time& request_timestamp)
  {
    ...
    // to fill geo coord channels in next stub
  ...
  void
  CampaignServerImpl_v331::convert_get_config_settings_(
  ...
  AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo*
  CampaignServerImpl_v331::get_config(
    const AdServer::CampaignSvcs_v330::CampaignGetConfigSettings& settings)
  {
    static const char* FUN = "CampaignServerImpl_v331::get_config()";

    try
    {
      AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo_var campaign_config;
      AdServer::CampaignSvcs_v340::CampaignGetConfigSettings conv_settings;

      convert_get_config_settings_(settings, conv_settings);

      campaign_config = campaign_server_->get_config(conv_settings);

      AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo_var result_campaign_config =
        new AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo();
    ...
    catch(const AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException& ex)
    {
      Stream::Error ostr;
      ostr << FUN << ": AdServer::CampaignSvcs_v340::CampaignServer::"
        "ImplementationException caught: " <<
        ex.description;

      CORBACommons::throw_desc<
        CampaignSvcs_v330::CampaignServer::ImplementationException>(
          ostr.str());
    }
    catch(const AdServer::CampaignSvcs_v340::CampaignServer::NotReady& ex)
    {
      throw AdServer::CampaignSvcs_v330::CampaignServer::NotReady(ex.description);
    }
  ...
  CampaignServerImpl_v331::get_colocation_flags()
  {
    try
    {
      AdServer::CampaignSvcs_v340::ColocationFlagsSeq_var flags_src =
        campaign_server_->get_colocation_flags();

      AdServer::CampaignSvcs_v330::ColocationFlagsSeq_var flags_dest =
        new AdServer::CampaignSvcs_v330::ColocationFlagsSeq;

      flags_dest->length(flags_src->length());
      for(CORBA::ULong i = 0; i < flags_dest->length(); ++i)
      {
        const AdServer::CampaignSvcs_v340::ColocationFlags& flag_src =
          flags_src[i];
        AdServer::CampaignSvcs_v330::ColocationFlags& flag_dst = 
          flags_dest[i];
    ...
    {
      Stream::Error ostr;
      ostr << __func__ << ": AdServer::CampaignSvcs_v340::CampaignServer::"
        "ImplementationException caught: " <<
        ex.description;
  ...
  AdServer::CampaignSvcs_v330::EcpmSeq*
  CampaignServerImpl_v331::get_ecpms(
    const AdServer::CampaignSvcs_v330::TimestampInfo& request_timestamp)
  {
    static const char* FUN = "CampaignServerImpl_v331::get_ecpms()";
      
    try
    {
      AdServer::CampaignSvcs_v340::EcpmSeq_var ecpms  =
        campaign_server_->get_ecpms(request_timestamp);
      AdServer::CampaignSvcs_v330::EcpmSeq_var result_ecpms =
        new AdServer::CampaignSvcs_v330::EcpmSeq();
    ...
    catch(const AdServer::CampaignSvcs_v340::CampaignServer::ImplementationException& e)
    {
      Stream::Error ostr;
      ostr << FUN << ": CampaignSvcs_v340::CampaignServer::"
        "ImplementationException caught: " << e.description;
      CORBACommons::throw_desc<
        CampaignSvcs_v330::CampaignServer::ImplementationException>(
          ostr.str());
    }
    catch(const AdServer::CampaignSvcs_v340::CampaignServer::NotReady& )
    {
      throw AdServer::CampaignSvcs_v330::CampaignServer::NotReady();
    }
    return 0; // never reach
  }

  AdServer::CampaignSvcs_v330::BriefSimpleChannelAnswer*
  CampaignServerImpl_v331::brief_simple_channels(
    ...
    GetSimpleChannelsInfo& /*settings*/)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::brief_simple_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::SimpleChannelAnswer*
  CampaignServerImpl_v331::simple_channels(
    const AdServer::CampaignSvcs_v330::CampaignServer::GetSimpleChannelsInfo& /*settings*/)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::simple_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::ExpressionChannelsInfo*
  CampaignServerImpl_v331::get_expression_channels(
    const AdServer::CampaignSvcs_v330::CampaignServer::GetExpressionChannelsInfo&)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::get_expression_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::DetectorsConfig*
  CampaignServerImpl_v331::detectors(
    const AdServer::CampaignSvcs_v330::TimestampInfo& /*request_timestamp*/)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::search_engines(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::FreqCapConfigInfo*
  CampaignServerImpl_v331::freq_caps()
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::freq_caps(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::StatInfo*
  CampaignServerImpl_v331::get_stat()
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::NotSupport(
      "CampaignServerImpl_v331::get_stat(): Not supported.");
  }

  void
  CampaignServerImpl_v331::update_stat()
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::NotSupport(
      "CampaignServerImpl_v331::update_stat(): Not supported.");
  }

  CORBA::Boolean
  CampaignServerImpl_v331::need_config(
    const CORBACommons::TimestampInfo& master_stamp)
  {
    static const char* FUN = "CampaignServerImpl_v331::need_config()";

    try
    {
      return campaign_server_->need_config(master_stamp);
    }
    catch(const AdServer::CampaignSvcs_v330::
  ...
  CampaignServerImpl_v331::get_discover_channels(
    CORBA::ULong, CORBA::ULong)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::get_discover_channels(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::CampaignServer::PassbackInfo*
  CampaignServerImpl_v331::get_tag_passback(CORBA::ULong, const char*)
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::get_tag_passback(): Not supported.");
  }

  AdServer::CampaignSvcs_v330::FraudConditionConfig*
  CampaignServerImpl_v331::fraud_conditions()
  {
    throw AdServer::CampaignSvcs_v330::CampaignServer::ImplementationException(
      "CampaignServerImpl_v331::fraud_conditions(): Not supported.");
  }

  void
  CampaignServerImpl_v331::convert_campaigns_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    const
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CampaignInfo& campaign_info =
        update_info.campaigns[i];

      if(CorbaAlgs::unpack_time(campaign_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CampaignInfo& result_campaign_info =
        ...
        {
          const AdServer::CampaignSvcs_v340::CreativeInfo& creative_info =
            campaign_info.creatives[j];

          AdServer::CampaignSvcs_v330::CreativeInfo& result_creative_info =
  ...
  void
  CampaignServerImpl_v331::convert_accounts_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::AccountInfo& account_info =
        update_info.accounts[i];

      if(CorbaAlgs::unpack_time(account_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::AccountInfo& result_account_info =
  ...
  void
  CampaignServerImpl_v331::convert_creative_options_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CreativeOptionInfo& option_info =
        update_info.activate_creative_options[i];

      if(CorbaAlgs::unpack_time(option_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CreativeOptionInfo& result_option_info =
  ...
  void
  CampaignServerImpl_v331::convert_expression_channels_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v331::convert_geo_channels_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::GeoChannelInfo& channel_info =
        update_info.activate_geo_channels[ch_i];

      if(CorbaAlgs::unpack_time(channel_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::GeoChannelInfo& result_channel_info =
  ...
  void
  CampaignServerImpl_v331::convert_ecpm_seq_(
    ...
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CampaignEcpmInfo& ecpm_info = ecpms[i];

      if(CorbaAlgs::unpack_time(ecpm_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CampaignEcpmInfo& res_ecpm_info = result_ecpms[res_i++];
  ...
  void
  CampaignServerImpl_v331::convert_ecpms_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v331::convert_sites_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::SiteInfo& site_info = update_info.sites[i];

      if(CorbaAlgs::unpack_time(site_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::SiteInfo& result_site_info =
  ...
  void
  CampaignServerImpl_v331::convert_sizes_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::SizeInfo& size_info =
        update_info.sizes[i];
      if(CorbaAlgs::unpack_time(size_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::SizeInfo& result_size_info =
  ...
  void
  CampaignServerImpl_v331::convert_tags_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::TagInfo& tag_info =
         ...
         tag_info.sizes.length() > 0)
      {
        ...
        AdServer::CampaignSvcs_v330::TagInfo& result_tag_info =
        ...
        {
          const AdServer::CampaignSvcs_v340::TagPricingInfo& tag_pricing_info =
            tag_info.tag_pricings[j];
          AdServer::CampaignSvcs_v330::TagPricingInfo& result_tag_pricing_info =
  ...
  void
  CampaignServerImpl_v331::convert_frequency_caps_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::FreqCapInfo& freq_cap =
        update_info.frequency_caps[i];

      if(CorbaAlgs::unpack_time(freq_cap.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::FreqCapInfo& result_freq_cap =
  ...
  void
  CampaignServerImpl_v331::convert_simple_channels_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v331::convert_behav_params_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    {
      ...
      {
        const AdServer::CampaignSvcs_v340::BehavParamInfo& bp =
          update_info.behav_params[i];

        if(CorbaAlgs::unpack_time(bp.timestamp) > request_timestamp)
        {
          AdServer::CampaignSvcs_v330::BehavParamInfo& result_bp =
    ...
    {
      const AdServer::CampaignSvcs_v340::KeyBehavParamInfo& bp =
        update_info.key_behav_params[i];
      if(CorbaAlgs::unpack_time(bp.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::KeyBehavParamInfo& result_bp =
  ...
  void
  CampaignServerImpl_v331::convert_colocations_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::ColocationInfo& colo_info =
        update_info.colocations[i];
      if(CorbaAlgs::unpack_time(colo_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::ColocationInfo& result_colo_info =
  ...
  void
  CampaignServerImpl_v331::convert_creative_templates_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CreativeTemplateInfo& cr_templ_info =
        update_info.creative_templates[i];
      if(CorbaAlgs::unpack_time(cr_templ_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CreativeTemplateInfo& result_cr_templ_info =
        ...
        {
          const AdServer::CampaignSvcs_v340::CreativeTemplateFileInfo& ctf_info =
            cr_templ_info.files[ctf_i];
          AdServer::CampaignSvcs_v330::CreativeTemplateFileInfo& result_ctf_info =
          ...
          result_ctf_info.type = static_cast<
            AdServer::CampaignSvcs_v330::CreativeTemplateType>(
              ctf_info.type);
        }
  ...
  void
  CampaignServerImpl_v331::convert_currencies_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CurrencyInfo& currency_info =
        update_info.currencies[i];

      if(CorbaAlgs::unpack_time(currency_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CurrencyInfo& result_currency_info =
  ...
  void
  CampaignServerImpl_v331::convert_campaign_keywords_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CampaignKeywordInfo& keyword_info =
        update_info.campaign_keywords[i];

      if(CorbaAlgs::unpack_time(keyword_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CampaignKeywordInfo& res_keyword_info =
  ...
  void
  CampaignServerImpl_v331::convert_creative_categories_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CreativeCategoryInfo& category_info =
        update_info.creative_categories[i];
      if(CorbaAlgs::unpack_time(category_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CreativeCategoryInfo& res_category_info =
  ...
  void
  CampaignServerImpl_v331::convert_adv_actions_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  ...
  void
  CampaignServerImpl_v331::convert_category_channels_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::CategoryChannelInfo& channel_category_info =
        update_info.category_channels[cat_i];
      if(CorbaAlgs::unpack_time(channel_category_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::CategoryChannelInfo& result_channel_category_info =
  ...
  void
  CampaignServerImpl_v331::convert_search_engines_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::SearchEngineInfo& s_info =
        update_info.search_engines[i];
      if(CorbaAlgs::unpack_time(s_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::SearchEngineInfo& result_s_info =
  ...
  void
  CampaignServerImpl_v331::convert_web_browsers_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::WebBrowserInfo& b_info =
        update_info.web_browsers[i];
      if(CorbaAlgs::unpack_time(b_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::WebBrowserInfo& result_b_info =
        ...
        {
          const AdServer::CampaignSvcs_v340::WebBrowserDetectorInfo& d_info =
            b_info.detectors[j];
          AdServer::CampaignSvcs_v330::WebBrowserDetectorInfo& result_d_info =
  ...
  void
  CampaignServerImpl_v331::convert_platforms_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::PlatformInfo& p_info =
        update_info.platforms[i];
      if(CorbaAlgs::unpack_time(p_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::PlatformInfo& result_p_info =
        ...
        {
          const AdServer::CampaignSvcs_v340::PlatformDetectorInfo& pd_info =
            p_info.detectors[j];
          AdServer::CampaignSvcs_v330::PlatformDetectorInfo& result_pd_info =
  ...
  void
  CampaignServerImpl_v331::convert_fraud_conditions_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::FraudConditionInfo& fraud_cond_info =
        update_info.fraud_conditions[i];
      if(CorbaAlgs::unpack_time(fraud_cond_info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::FraudConditionInfo& result_fraud_cond_info =
  ...
  void
  CampaignServerImpl_v331::convert_string_dictionaries_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::WebOperationInfo& info =
        update_info.web_operations[i];
      if(CorbaAlgs::unpack_time(info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::WebOperationInfo& result_info =
  ...
  void
  CampaignServerImpl_v331::convert_app_formats_(
    AdServer::CampaignSvcs_v330::CampaignConfigUpdateInfo& result_update_info,
    const AdServer::CampaignSvcs_v340::CampaignConfigUpdateInfo& update_info,
    const Generics::Time& request_timestamp)
    noexcept
  {
    ...
    {
      const AdServer::CampaignSvcs_v340::AppFormatInfo& info =
        update_info.app_formats[i];
      if(CorbaAlgs::unpack_time(info.timestamp) > request_timestamp)
      {
        AdServer::CampaignSvcs_v330::AppFormatInfo& result_info =
