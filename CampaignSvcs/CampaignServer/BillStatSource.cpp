...
{
  void
  BillStatSource::Stat::AmountDistribution::print(std::ostream& out, const char* offset)
    const noexcept
  {
    out << offset <<
      "before " << prev_day.get_gm_time().format("%F") << ": " << prev_days_amount << std::endl;

    for (auto day_it = day_amounts.begin(); day_it != day_amounts.end(); ++day_it)
    {
      out << offset <<
        "at " << day_it->first.get_gm_time().format("%F") << ": " << day_it->second << std::endl;
    }
  }

  void
  BillStatSource::Stat::AmountCountDistribution::print(std::ostream& out, const char* offset)
    const noexcept
  {
    out << offset <<
      "before " << prev_day.get_gm_time().format("%F") <<
      ": amount = " << prev_days_amount <<
      ", imps = " << prev_days_imps.str() << ", clicks = " << prev_days_clicks.str() << std::endl;

    for (auto day_it = day_amount_counts.begin(); day_it != day_amount_counts.end(); ++day_it)
    {
      out << offset <<
    ...
    noexcept
  {
    ...
    for (auto acc_it = accounts.begin(); acc_it != accounts.end(); ++acc_it)
    {
    ...
    for (auto cmp_it = campaigns.begin(); cmp_it != campaigns.end(); ++cmp_it)
    {
    ...
    for (auto ccg_it = ccgs.begin(); ccg_it != ccgs.end(); ++ccg_it)
    {
