...
namespace AdServer::CampaignSvcs
{
  ...
  void unpack_non_linked_expression(
    ...
    const ExpressionInfo& expression_info)
  {
    expression.op = static_cast<NonLinkedExpressionChannel::Operation>(expression_info.operation);
    ...
    for (CORBA::ULong ei = 0; ei < expression_info.sub_channels.length(); ++ei)
    {
      ...
      unpack_non_linked_expression(sub_expr, expression_info.sub_channels[ei]);
    ...
    noexcept
  {
    ...
    const NonLinkedExpressionChannel::Expression* expr = expression_channel->expression();
    if (expr)
    {
  ...
  unpack_non_linked_channel(const ExpressionChannelInfo& channel_info)
    /*throw(eh::Exception)*/
  {
