...
namespace AdServer::CampaignSvcs
{
  ...
  public:
    ...
    CreativeDef(
      ...
      const CampaignSvcs_v360::OptionValueSeq& tokens_val = OptionValueSeq(),
      const CampaignSvcs_v360::CreativeCategoryIdSeq& categories_val =
  ...
  public:
    ...
    {
      return ecpm == right.ecpm && ctr == right.ctr;
    }
  ...
  {
    TimestampValue timestamp(const NonLinkedExpressionChannel* el) const
    {
      return el->params().timestamp;
    }

    void set_timestamp(NonLinkedExpressionChannel* el, const TimestampValue& ts) const
    {
  ...
  public:
    ...
    MarginRuleDef(const MarginRuleDef& init, bool with_conditions = true)
      noexcept;
  ...
  private:
    ...
    static bool account_set_eq_(const AccountIdSet& left, const AccountIdSet& right) noexcept;

    static bool pricerange_set_eq_(const PriceRangeSet& left, const PriceRangeSet& right) noexcept;
  };
...
namespace AdServer::CampaignSvcs
{
  ...
  bool CurrencyDef::operator==(const CurrencyDef& right) const noexcept
  {
    return rate == right.rate &&
      ...
      fraction_digits == right.fraction_digits && currency_code == right.currency_code;
  }
  ...
  bool CampaignKeyword::operator==(const CampaignKeyword& right) const noexcept
  {
    return ccg_keyword_id == right.ccg_keyword_id &&
      original_keyword == right.original_keyword && click_url == right.click_url;
  }
  ...
  TagPricingDef::operator==(const TagPricingDef& right) const noexcept
  {
    return country_code == right.country_code &&
      ...
      imp_revenue == right.imp_revenue && revenue_share == right.revenue_share;
  }
  ...
  TagDef::operator==(const TagDef& right) const noexcept
  {
    return tag_id == right.tag_id &&
      ...
      std::equal(template_tokens.begin(), template_tokens.end(), right.template_tokens.begin());
  }

  inline
  bool
  AppFormatDef::operator==(const AppFormatDef& right) const noexcept
  {
  ...
  inline
  bool CreativeOptionDef::operator==(const CreativeOptionDef& right) const noexcept
  {
    return token == right.token &&
      type == right.type &&
      token_relations.size() == right.token_relations.size() &&
      std::equal(token_relations.begin(), token_relations.end(), right.token_relations.begin());
  }
      ...
      sys_options(sys_options_val)
  {
    ...
    {
      tokens.insert(std::make_pair(tokens_val[token_i].option_id, tokens_val[token_i].value.in()));
    }
  ...
  bool ColocationDef::operator==(const ColocationDef& right) const noexcept
  {
    return colo_id == right.colo_id &&
      ...
      ad_serving == right.ad_serving && hid_profile == right.hid_profile && tokens == right.tokens;
  }
  ...
  FreqCapDef::operator==(const FreqCapDef& cp) const noexcept
  {
    return fc_id == cp.fc_id &&
      ...
      period == cp.period && window_limit == cp.window_limit && window_time == cp.window_time;
  }
  ...
  inline
  bool
  CreativeTemplateFileDef::operator==(const CreativeTemplateFileDef& right) const noexcept
  {
    return creative_format == right.creative_format &&
      ...
      track_impr == right.track_impr && template_file == right.template_file && type == right.type;
  }


  inline bool
  CreativeTemplateDef::operator==(const CreativeTemplateDef& right) const noexcept
  {
  ...
  inline
  MarginRuleDef::MarginRuleDef(const MarginRuleDef& init, bool with_conditions) noexcept
  ...
  inline
  bool MarginRuleDef::account_set_eq_(const AccountIdSet& left, const AccountIdSet& right) noexcept
  {
  ...
  inline
  bool BehavioralParameterDef::operator==(const BehavioralParameterDef& in) const noexcept
  {
    return min_visits == in.min_visits &&
      time_from == in.time_from &&
      time_to == in.time_to && weight == in.weight && trigger_type == in.trigger_type;
  }

  inline
  bool BehavioralParameterListDef::operator==(const BehavioralParameterListDef& in) const noexcept
  {
    return threshold == in.threshold &&
      behave_params.size() == in.behave_params.size() &&
      std::equal(behave_params.begin(), behave_params.end(), in.behave_params.begin());
  }
  ...
  inline
  bool CategoryChannelDef::operator==(const CategoryChannelDef& right) const noexcept
  {
  ...
  inline
  bool
  CreativeDef::SystemOptions::operator==(const CreativeDef::SystemOptions& right) const noexcept
  {
    return click_url_option_id == right.click_url_option_id &&
      click_url == right.click_url &&
      html_url_option_id == right.html_url_option_id && html_url == right.html_url;
  }
  ...
  inline
  bool FraudConditionDef::operator==(const FraudConditionDef& right) const noexcept
  {
    return id == right.id && type == right.type && limit == right.limit && period == right.period;
  }
  ...
  inline
  bool GeoChannelDef::GeoIPTarget::operator==(const GeoIPTarget& right) const
    noexcept
  {
    ...
    noexcept
  {
    return country == right.country &&
      ...
      std::equal(geoip_targets.begin(), geoip_targets.end(), right.geoip_targets.begin());
  }
  ...
  GeoCoordChannelDef::operator==(const GeoCoordChannelDef& right) const
    noexcept
  {
    return longitude == right.longitude && latitude == right.latitude && radius == right.radius;
  }
    ...
    noexcept
  {
    return host_postfix == cp.host_postfix &&
      ...
      post_encoding == cp.post_encoding && decoding_depth == cp.decoding_depth;
  }
  ...
  bool WebBrowser::Detector::operator==(const WebBrowser::Detector& right) const noexcept
  {
    return priority == right.priority &&
      marker == right.marker && regexp == right.regexp && regexp_required == right.regexp_required;
  }
  ...
  bool Platform::Detector::operator==(const Platform::Detector& right) const noexcept
  {
    return priority == right.priority &&
      ...
      marker == right.marker &&
      match_regexp == right.match_regexp && output_regexp == right.output_regexp;
  }
  ...
  inline
  bool
  CreativeCategoryDef::operator==(const CreativeCategoryDef& right) const noexcept
  {
  ...
  {
    return this->size() == right.size() && std::equal(begin(), end(), right.begin());
  }
  ...
  SizeDef::operator==(const SizeDef& right) const noexcept
  {
    return protocol_name == right.protocol_name &&
      ...
      width == right.width &&
      height == right.height && max_width == right.max_width && max_height == right.max_height;
  }
  ...
  WebOperationDef::operator==(const WebOperationDef& right) const noexcept
  {
    return app == right.app &&
      source == right.source && operation == right.operation && flags == right.flags;
  }
