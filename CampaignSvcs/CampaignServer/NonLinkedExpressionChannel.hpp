...
namespace AdServer::CampaignSvcs
{
  ...
  {
  public:
    friend void print(std::ostream& out, const NonLinkedExpressionChannel* channel)
      noexcept;
  ...
  public:
    ...
    bool equal(const NonLinkedExpressionChannel* right, bool status_check) const noexcept;
...
{
  inline
  void print_expression(std::ostream& out, const NonLinkedExpressionChannel::Expression& expr)
    noexcept
  {
  ...
  inline
  void print(std::ostream& out, const NonLinkedExpressionChannel* channel)
    noexcept
  {
  ...
  channel_equal(NonLinkedExpressionChannel* left,
    ...
    noexcept
  {
    return ((left->expr_.get() && right->expr_.get() && *(left->expr_) == *(right->expr_)) ||
      (!left->expr_.get() && !right->expr_.get())) &&
      left->channel_params_.equal(right->channel_params_, status_check);
  }

  inline
  bool NonLinkedExpressionChannel::Expression::operator==(const Expression& right) const
    noexcept
  {
    return op == right.op &&
  ...
  inline
  void NonLinkedExpressionChannel::Expression::swap(Expression& right)
    noexcept
  {
  ...
  inline
  void
  NonLinkedExpressionChannel::Expression::all_channels(ChannelIdSet& channels) const noexcept
  {
    if (op == NOP)
    {
      if (channel_id)
