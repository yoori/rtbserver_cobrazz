...
namespace AdServer::CampaignSvcs
{
  ...
  inline bool
  CampaignServerStatValues::get(const Key& key, Generics::Values::Floating& value) const
    /*throw(eh::Exception, InvalidType)*/
  {
    if (key == LAST_DB_UPDATE_DELAY)
    {
      bool result = Generics::Values::get<Generics::Values::Floating>(LAST_DB_UPDATE_TIME, value);
      if (result)
      {
    ...
    /*throw(eh::Exception)*/
  {
    ...
    if (member)
    {
      functor(LAST_DB_UPDATE_DELAY, Generics::Time::get_time_of_day().as_double() - *member);
    }
  }
}
