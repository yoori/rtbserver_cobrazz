...
namespace AdServer::CampaignSvcs
{
  ...
  {
    for (DomainConfig::Domain_sequence::const_iterator d_it = config.Domain().begin();
        d_it != config.Domain().end(); ++d_it)
    {
      if (!d_it->SubDomain().size())
      {
        domains_trie_.insert(FirstDomainMap::value_type(d_it->name(), SecondsDomainMap()));
        continue;
      }
          ...
          sd_it != d_it->SubDomain().end(); ++sd_it)
      {
        ...
        FirstDomainMap::mapped_type& second_domains = domains_trie_[d_it->name()];
  ...
  void
  DomainParser::specific_domain(const String::SubString& domain, std::string& result)
    const
    noexcept
  {
    ...
    for (;;)
    {
      ...
      {
        // get ".com" by com
        cit = domains_trie_.find(std::string(domain.data() + pos + 1, domain.size() - pos - 1));
      }
      ...
      ThirdsDomainSet::const_iterator cit_third =
        cit_two->second.find(std::string(domain.data() + pos + 1, last_pos - pos));
    ...
    if (pos == String::SubString::NPOS && last_pos == 2 && !strncmp("www", domain.data(), 3))
    {
