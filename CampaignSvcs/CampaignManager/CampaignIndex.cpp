...
namespace
{
  ...
  bool
  match(unsigned long campaign_flags, AdServer::CampaignSvcs::UserStatus st) noexcept
  {
    using namespace AdServer::CampaignSvcs;
    ...
    return
      (((campaign_flags & CampaignFlags::US_OPTIN) && (st == US_OPTIN)) ||
       ...
       ((campaign_flags & CampaignFlags::US_UNDEFINED) && (st == US_UNDEFINED || st == US_PROBE)));
  }
}
...
namespace AdServer::CampaignSvcs
{
  ...
  CampaignIndex::CampaignIndex(ConstCampaignConfigPtr campaign_config, Logging::Logger* logger)
    noexcept
    : logger_(ReferenceCounting::add_ref(logger)),
      campaign_config_(std::move(campaign_config))
  {}

  CampaignIndex::CampaignIndex(const CampaignIndex& init, Logging::Logger* logger)
    noexcept
    : logger_(ReferenceCounting::add_ref(logger)),
    ...
    /*throw(eh::Exception)*/
  {
    ...
    for (CampaignConfig::CampaignMap::const_iterator cmp_it = campaign_config_->campaigns.begin();
        cmp_it != campaign_config_->campaigns.end(); ++cmp_it, ++i)
    {
      ...
      if (cmp_it->second->account->is_active() && cmp_it->second->is_active())
      {
      ...
      if (logger_->log_level() >= Logging::Logger::TRACE)
      {
        ...
        if (i % INDEXING_PROGRESS_PRECISSION == 0)
        {
          ...
          for (std::set<unsigned long>::const_iterator pit = progress_campaigns.begin();
          ...
          logger_->log(ostr.str(), Logging::Logger::TRACE, Aspect::CAMPAIGN_SELECTION_INDEX);
    ...
    /*throw(Exception, eh::Exception)*/
  {
    if (trace_params)
    {
          ...
          trace_params->key.test_request))
      {
        trace_params->trace_stream <<
          "  <step name=\"status and test campaign check\" passed=\"yes\"/>" << std::endl;
    ...
    else if (campaign->is_active())
    {
      ...
      {
        index_for_colo_(user_status, ST_TEST_REAL, campaign, trace_params);

        if (!campaign->is_test())
        {
          index_for_colo_(user_status, ST_REAL, campaign, trace_params);
        }
      }
    }
  }
  ...
  bool
  CampaignIndex::colo_precheck_(UserStatus user_status, const Colocation* colocation)
    noexcept
  {
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    {
      trace_params->trace_stream <<
        "  <step name=\"include colocations check\" passed=\"yes\"/>" << std::endl;
    }

    index_for_tags_(user_status, status_type, campaign, trace_params);
  }
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    if (!trace_params)
    {
      TagCampaignApproveMap::const_iterator tag_cmp_it =
        tag_campaign_approve_.lower_bound(TagCampaignApprove(campaign->campaign_id, 0));
    ...
    {
      TagCampaignApproveMap::const_iterator tag_cmp_it =
        tag_campaign_approve_.find(TagCampaignApprove(campaign->campaign_id, trace_params->tag));
    ...
    noexcept
  {
    ...
    const CreativeTemplateMap& creative_templates = campaign_config_->creative_templates;
        ...
        size_it != creative->sizes.end(); ++size_it)
    {
      ...
      {
        logger_->sstream(Logging::Logger::WARNING, Aspect::TRAFFICKING_PROBLEM, "ADS-TF-0") <<
          "incorrect template file for template (format = '" <<
          creative->creative_format << "', size = '" <<
          size_it->second.size->protocol_name << "', app-format = '" <<
          app_format << "'): all creatives that uses it will be ignored: " << ex.what();
      }
  ...
  CampaignIndex::check_categories_(
    ...
    noexcept
  {
    for (Creative::CategorySet::const_iterator cat_it = creative_categories.begin();
        cat_it != creative_categories.end(); ++cat_it)
    {
      if (tag_exclusion)
      {
        if (tag->rejected_categories.find(*cat_it) != tag->rejected_categories.end())
        {
          return false;
        }

        if (tag->accepted_categories.find(*cat_it) != tag->accepted_categories.end())
        {
          continue;
        }
      }
    ...
    noexcept
  {
    ...
    {
      const Creative::CategorySet& creative_categories = creative->categories;

      for (Creative::CategorySet::const_iterator cat_it = creative_categories.begin();
          cat_it != creative_categories.end(); ++cat_it)
      {
        if (exclude_categories.find(*cat_it) != exclude_categories.end())
        {
          return false;
        }
      }
  ...
  bool
  CampaignIndex::creative_preavailable_by_sizes_(const Tag* tag, const Creative* creative)
    noexcept
  {
  ...
  CampaignIndex::creative_available_by_size(
    ...
    noexcept
  {
    Creative::SizeMap::const_iterator crs_it = creative->sizes.find(tag_size->size->size_id);
    ...
    if (tag->is_test() && status_type != ST_TEST_REAL)
    {
      if (trace_params)
      {
        trace_params->trace_stream <<
          ...
          "  request is test, but tag not" << std::endl << "</step>" << std::endl;
      }

      return;
    }

    index_for_countries_(user_status, status_type, campaign, tag, creatives, trace_params);
  }
    ...
    if (trace_params)
    {
      if (trace_params->key.country_code != campaign->country)
      {
        trace_params->trace_stream <<
          ...
          "    allowed countries = " << campaign->country << std::endl << "  </step>" << std::endl;
      }
      else
      {
        trace_params->trace_stream <<
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    if (!campaign->use_ctr())
    {
      ...
      RevenueDecimal current_cpm = tag_pricing ? tag_pricing->cpm : RevenueDecimal::ZERO;
      ...
      if (trace_params)
      {
        trace_params->trace_stream <<
          ...
          "    min ecpm = " << current_cpm << std::endl << "  </step>" << std::endl;
      }
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    if (campaign->ccg_type == CT_DISPLAY)
    {
      ...
      if (campaign->marketplace != 'O' && tag->marketplace != 'O' &&
         tag->site->account->walled_garden_accounts.find(campaign->account->account_id) !=
      ...
      if (!walled_garden && campaign->marketplace != 'W' && tag->marketplace != 'W')
      {
        if (campaign->mode == CM_NON_RANDOM)
        {
          margin_passed = margin_check_(
        ...
        cr_it != creatives.end(); ++cr_it)
    {
      for (Tag::SizeMap::const_iterator tag_size_it = tag->sizes.begin();
          tag_size_it != tag->sizes.end(); ++tag_size_it)
      {
        Creative::SizeMap::const_iterator crs_it = (*cr_it)->sizes.find(tag_size_it->first);
    ...
    if (trace_params)
    {
      if (!trace_creatives.empty())
      {
        ...
        for (ConstCreativePtrList::const_iterator cr_it = trace_creatives.begin();
        ...
        format_it != creative_formats.end(); ++format_it)
    {
      KeyHashAdapter key_hash(
        static_cast<unsigned char>(user_status) | (static_cast<unsigned char>(status_type) << 4),
        tag->tag_id,
        country_code,
        format_it->c_str());
      ...
      if (text_candidate)
      {
        ...
        else // channel targeted text ad
        {
          ...
          {
            index_node.text_campaigns = campaign_cell_holder_->get(index_node.text_campaigns, cell);
          }
        }
      }
        ...
        if (campaign->mode == CM_RANDOM)
        {
          if (tag->max_random_cpm >= tag_pricing->cpm)
          {
            CampaignSelectionCell_var cell(
              new CampaignSelectionCell(campaign, adjusted_campaign_ecpm, tag_pricing));
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              index_node.display_campaigns = cell_holder_->get(index_node.display_campaigns, cell);
            }
          }
        }
      }
    ...
    /*throw(Exception, eh::Exception)*/
  {
       ...
       campaign->sites.end())
    {
      ...
      {
        trace_params->trace_stream <<
          "  <step name=\"site targeting\" passed=\"no\"/>" << std::endl;
      }
    ...
    {
      trace_params->trace_stream << "  <step name=\"site targeting\" passed=\"yes\"/>" << std::endl;
    }
    ...
    if (td_it != campaign->exclude_tags.end() && td_it->second == 0)
    {
      ...
      {
        trace_params->trace_stream <<
          "  <step name=\"tag delivery full exclusion\" passed=\"no\"/>" << std::endl;
      }
    ...
    if (campaign->exclude_pub_accounts.find(site->account->account_id) !=
       campaign->exclude_pub_accounts.end())
    {
      if (trace_params)
      {
        trace_params->trace_stream <<
          "  <step name=\"publisher accounts exclusion\" passed=\"no\"/>" << std::endl;
      }
    ...
    {
      trace_params->trace_stream <<
        "  <step name=\"publisher accounts exclusion\" passed=\"yes\"/>" << std::endl;
    }
         ...
         tag->site->account->walled_garden_accounts.end()))
    {
      ...
      {
        trace_params->trace_stream <<
          "  <step name=\"walled garden approval\" passed=\"yes\"/>" << std::endl;
      }
      ...
      {
        if ((*cr_it)->status == 'A' &&
           creative_available_by_categories_(campaign, *cr_it, tag, !campaign->keyword_based()) &&
           creative_preavailable_by_sizes_(tag, *cr_it))
        {
      ...
      if (trace_params)
      {
        ...
        else
        {
          trace_params->trace_stream <<
            "  <step name=\"creative categories and tag size\""
               " passed=\"no\">" << std::endl <<
            "    No creatives available by categories and tag size" << std::endl << "  </step>";
        }
      }
      ...
      if (!creatives.empty())
      {
          ...
          ++cr_it)
        {
          ...
          {
            for (Creative::SizeMap::const_iterator crs_it = creative->sizes.begin();
      ...
      if (trace_params)
      {
        if (!creatives.empty())
        {
          trace_params->trace_stream <<
            ...
            " passed=\"yes\">" << std::endl << "     Next creatives available:";
        ...
        else
        {
          trace_params->trace_stream <<
            "  <step name=\"expandable creatives\""
            " passed=\"no\">" << std::endl <<
            "    No creatives available" << std::endl << "  </step>";
        }
      }
      ...
      if (!creatives.empty())
      {
        tag_campaign_approve_.insert(
          std::make_pair(TagCampaignApprove(campaign->campaign_id, tag), creatives));
      }
    } // walled garden check
    else if (trace_params)
    {
  ...
  void
  CampaignIndex::index_campaign(const Campaign* campaign, TraceParams* trace_params)
    /*throw(Exception, eh::Exception)*/
  {
    if (!trace_params)
    {
    ...
    if (trace_params)
    {
      ...
      if ((only_for_opt_in && trace_params->key.user_status == US_OPTIN) || !only_for_opt_in)
      {
        trace_params->trace_stream <<
    ...
    if (only_for_opt_in)
    {
      ...
      //
      if ((campaign->flags & CampaignFlags::US_NONE) || (campaign->flags & CampaignFlags::US_OPTIN))
      {
        index_for_status_(
          campaign,
    ...
    else
    {
      ...
      {
        index_for_status_(campaign, US_OPTOUT, trace_match_type == US_OPTOUT ? trace_params : 0);
      }

      if (campaign->flags & CampaignFlags::US_OPTIN)
      {
        index_for_status_(campaign, US_OPTIN, trace_match_type == US_OPTIN ? trace_params : 0);
      }
    }
  }
  ...
  bool
  CampaignIndex::match_domain(const std::string& domain, const String::SubString& referer_hostname)
    noexcept
  {
    if (domain.size() > 0)
    {
  ...
  bool
  CampaignIndex::check_tag_domain_exclusion(std::string_view url_val, const Tag* tag)
    noexcept
  {
    try
    {
    ...
    TraceParams* trace_params)
  {
    bool result = tag->site->freq_cap_id &&
      (!profiling_available || full_freq_caps.find(tag->site->freq_cap_id) != full_freq_caps.end());
  ...
  {
    {
      ...
      bool res = (campaign->fc_id || campaign->group_fc_id) && (
        ...
        full_freq_caps.find(campaign->group_fc_id) != full_freq_caps.end() || !profiling_available);
...

    {
      ...
      {
        trace_params->trace_stream <<
          "  <step name=\"time of day\" passed=\"" << (res ? "no" : "yes") << "\"/>" << std::endl;
      }
...

    {
      ...
      bool res = (user_id.hash() % MAX_TARGET_USERS_GROUPS <
        ...
        user_id.hash() % MAX_TARGET_USERS_GROUPS >= campaign->end_user_group_id);
...

    {
      ...
      bool res = !campaign->is_available() ||
        !campaign->account->is_available() || !campaign->advertiser->is_available();
      ...
      if (trace_params)
      {
        ...
        {
          trace_params->trace_stream <<
            "  <step name=\"available\" passed=\"" << (res ? "no" : "yes") << "\"/>" << std::endl;
        }
      }
    ...
    const
  {
    ...
    */
    return !campaign->fast_channel.in() || campaign->fast_channel->triggered(&matched_channels, 0);
  }
    ...
    const
  {
    ...
    const auto indexed_creative_available = [&, this](const Creative* creative)
    {
      return (creative->fc_id == 0 || key.user_status == US_OPTIN) &&
        ...
        creative_available_by_sizes_(
          ...
          down_expand_space,
          left_expand_space) &&
        creative_available_by_categories_(campaign, creative, tag, check_click_categories) &&
        creative_available_by_templates_(campaign, creative, tag, key.format.c_str()) &&
        ...
        (creative->video_duration >= video_min_duration &&
         (creative->video_skip_offset.present() ? // skippable
           ...
           (!video_max_duration.present() || creative->video_duration <= *video_max_duration)
          )) &&
        ...
        (creative->fc_id == 0 ||
         (profiling_available && full_freq_caps.find(creative->fc_id) == full_freq_caps.end())) &&
        creative_available_by_exclude_categories_(creative, exclude_categories) &&
        creative_available_by_required_categories_(creative, required_categories);
    };

    const auto creative_available = [&, this](const Creative* creative)
    {
      return creative->status == 'A' && indexed_creative_available(creative);
    };
    ...
    if (!trace_params)
    {
      if (check_tag_sizes->size() == 1)
      {
        ...
        {
          const Campaign::CreativeBySizeArray& size_creatives = size_creatives_it->second;

          for (Campaign::CreativeBySizeArray::const_iterator cr_it = size_creatives.begin();
      ...
      else
      {
          ...
          ++tag_size_it)
        {
          ...
          {
            const Creative* creative = cr_it->creative;
            if (seen_creatives.insert(creative).second && indexed_creative_available(creative))
            {
              creatives.push_back(creative);
            }
          }
        ...
        ++cr_it)
    {
      ...
      {
        trace_params->trace_stream << "    <creative ccid=\"" << creative->ccid <<
          "\" passed=\"no\">" << std::endl << "      Not available ";
        ...
        if (!creative_available_by_categories_(campaign, creative, tag, check_click_categories))
        {
          trace_params->trace_stream << "by categories; ";
        }

        if (!creative_available_by_templates_(campaign, creative, tag, key.format.c_str()))
        {
          trace_params->trace_stream << "by templates: format = '" << key.format << "'; ";
        }
        ...
        if (!creative_available_by_exclude_categories_(creative, exclude_categories))
        {
          trace_params->trace_stream << "by external exclude categories; ";
        }

        if (!creative_available_by_required_categories_(creative, required_categories))
        {
    ...
    if (campaign->seq_set_rotate_imps != 0 && !creatives.empty())
    {
      ...
      {
        SeqOrderMap::const_iterator seq_it = seq_orders.find(campaign->campaign_id);

        unsigned long search_order_set_id = *(campaign->opt_order_sets.begin());
    ...
    const LessPredType& less_pred)
  {
    ...
    for (IndexNodeList::const_iterator ll_it = nodes.begin(); ll_it != nodes.end(); ++ll_it)
    {
      if ((*ll_it)->*list_field)
      {
    ...
    Algs::custom_merge_n(ranges.begin(), ranges.end(), std::back_inserter(result), less_pred);
  }

  void
  CampaignIndex::get_index_nodes_(IndexNodeList& result_nodes, const Key& request_params) const
    noexcept
  {
        ...
        ++user_status_check_it)
    {
      unsigned char match_status_type =
        static_cast<unsigned char>(*user_status_check_it) |
          (static_cast<unsigned char>(request_params.test_request ? ST_TEST_REAL : ST_REAL) << 4);
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    else
    {
      trace_params.trace_stream <<
        ...
        "    matched channels = ";
      Algs::print(trace_params.trace_stream, matched_channels.begin(), matched_channels.end());
      trace_params.trace_stream << std::endl << "    fast expr = ";
      ...
      trace_params.trace_stream << std::endl << "    expr = ";
      if (campaign->channel)
      {
    ...
    {
      trace_params.trace_stream <<
        "  <step name=\"negative keywords checking\" passed=\"unknown\"/>" << std::endl;
    }
  ...
  std::string
  CampaignIndex::decode_match_status_type_(unsigned char match_status_type) noexcept
  {
    char res[] = {
    ...
    noexcept
  {
    ...
    {
      for (CampaignSelectionCellList::const_iterator cmp_it = campaign_selection_cell_list->begin();
        ...
        it != ordered_campaigns_.end(); ++it)
    {
      ostr << "(" << decode_match_status_type_(it->first.match_status_type) <<
        ...
        (it->first.country_code[0] && it->first.country_code[1] ?
          it->first.country_code[1] : ' ') << ", " << it->first.app_format << "): " << std::endl;

      ostr << OFFSET << "wg-display-campaigns ";
      print_selection_campaign_cell_list_(ostr, it->second.wg_display_campaigns.in());
      ostr << std::endl;
      ostr << OFFSET << "display-campaigns ";
      print_selection_campaign_cell_list_(ostr, it->second.display_campaigns.in());
      ostr << std::endl;
      ...
      print_selection_campaign_cell_list_(ostr, it->second.wg_display_random_campaigns);
      ostr << std::endl;
      ostr << OFFSET << "display-random-campaigns ";
      print_selection_campaign_cell_list_(ostr, it->second.display_random_campaigns);
      ostr << std::endl;
