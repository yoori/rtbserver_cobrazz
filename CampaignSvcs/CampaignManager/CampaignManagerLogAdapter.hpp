...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    public:
      ...
      fill(
        const CampaignConfig* campaign_config,
        const Proto::AnonymousRequestInfo& request_params,
        CampaignManagerLogger::AnonymousRequestInfo& logger_info);
      ...
      fill_request_info(
        ...
        const CampaignConfig* campaign_config,
        const Colocation* colocation,
        const Proto::CommonAdRequestInfo& common_info,
        const Proto::ContextAdRequestInfo& context_info,
      ...
      fill_match_request_info(
        CampaignManagerLogger::MatchRequestInfo& result_match_request,
        const CampaignConfig* campaign_config,
        const Proto::MatchRequestInfo& match_request_info,
        const ChannelIdList& geo_channels);
    ...
    private:
      ...
      convert_channel_ids_(
        ...
        CampaignManagerLogger::TriggerChannelMap& triggers,
        CampaignManagerLogger::TriggerChannelMap* discover_keyword_triggers,
        const google::protobuf::RepeatedPtrField<Proto::ChannelTriggerMatchInfo>& behav_params,
        const CampaignConfig* campaign_config) noexcept;
...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    CampaignManagerLogAdapter::convert_channel_ids_(
      ...
      CampaignManagerLogger::TriggerChannelMap& triggers,
      CampaignManagerLogger::TriggerChannelMap* discover_keyword_triggers,
      const google::protobuf::RepeatedPtrField<Proto::ChannelTriggerMatchInfo>& channel_trigger_matches,
      const CampaignConfig* campaign_config)
      noexcept
    {
      if (campaign_config)
      {
        ...
        for(const auto& channel_trigger_matche : channel_trigger_matches)
        {
          CampaignConfig::ChannelMap::const_iterator ch_it =
            campaign_config->expression_channels.find(
              channel_trigger_matche.channel_id());
          if (ch_it != campaign_config->expression_channels.end() &&
             ch_it->second->has_params() &&
             (ch_it->second->params().type == 'D' ||
              ch_it->second->params().type == 'K'))
          {
            if(discover_keyword_triggers)
            {
              (*discover_keyword_triggers)[channel_trigger_matche.channel_trigger_id()] =
                channel_trigger_matche.channel_id();
            }
          }
          else
          {
            triggers[channel_trigger_matche.channel_trigger_id()] =
              channel_trigger_matche.channel_id();
          }
          channels.insert(channel_trigger_matche.channel_id());
        }
      }
      else
      {
      ...
      const Proto::CommonAdRequestInfo& common_info)
    {
      ...
      try
      {
        request_info.household_id = GrpcAlgs::unpack_user_id(
          request_params.household_id());
        request_info.merged_user_id = GrpcAlgs::unpack_user_id(
          request_params.merged_user_id());
      }
      catch(const eh::Exception& ex)
      {
      ...
      request_info.history_channels.insert(
        std::end(request_info.history_channels),
        std::begin(request_params.channels()),
        std::end(request_params.channels()));

      request_info.hid_history_channels.insert(
        std::end(request_info.hid_history_channels),
      ...
      const Proto::CommonAdRequestInfo& common_info)
    {
      ...
      try
      {
        request_info.user_id = GrpcAlgs::unpack_user_id(
          common_info.user_id());
      }
      catch(const eh::Exception& ex)
      {
      ...
      try
      {
        request_info.request_id = GrpcAlgs::unpack_request_id(
          common_info.request_id());
      }
      catch(const eh::Exception& ex)
      {
      ...
      request_info.urls.insert(
        std::end(request_info.urls),
    ...
    CampaignManagerLogAdapter::fill(
      ...
      CampaignManagerLogger::AnonymousRequestInfo& logger_info)
    {
      logger_info.time = GrpcAlgs::unpack_time(request_params.time());
      logger_info.user_status = static_cast<UserStatus>(request_params.user_status());
      logger_info.log_as_test = request_params.test_request();
      logger_info.search_engine_id = request_params.search_engine_id();
      logger_info.client_app = request_params.client();
      ...
      const auto& platform_ids = request_params.platform_ids();
      ...
      if(campaign_config)
      {
        ...
        auto colo_it = campaign_config->colocations.find(
          request_params.colo_id());
        if(colo_it != campaign_config->colocations.end())
        {
          logger_info.isp_time_offset = colo_it->second->account->time_offset;
          logger_info.isp_time = logger_info.time +
    ...
    CampaignManagerLogAdapter::fill_match_request_info(
      ...
      const ChannelIdList& geo_channels)
    {
      result_match_request.user_id = GrpcAlgs::unpack_user_id(match_request_info.user_id());
      result_match_request.household_id = GrpcAlgs::unpack_user_id(
        match_request_info.household_id());
      result_match_request.time = GrpcAlgs::unpack_time(match_request_info.request_time());
      result_match_request.match_info.colo_id = match_request_info.match_info().colo_id();

      const auto& channels = match_request_info.match_info().channels();
      ...
      const auto& hid_channels = match_request_info.match_info().hid_channels();
      result_match_request.match_info.hid_channels.insert(
        std::begin(hid_channels),
        std::end(hid_channels));
      ...
      const CampaignManagerImpl::AdSlotContext& ad_slot_context)
    {
      if(request_params)
      {
        ...
        request_info.is_ad_request = (request_params->ad_slots().size() > 0);
        request_info.disable_fraud_detection =
          request_params->disable_fraud_detection();
        request_info.track_passback = request_params->required_passback();
      }
      ...
      Generics::Time request_time = GrpcAlgs::unpack_time(common_info.time());
      request_info.time = request_time;
      request_info.isp_time = request_time;
      ...
      if(campaign_config)
      {
        ...
        if(ad_request_debug_info)
        {
          auto* platform_channels_proto =
            ad_request_debug_info->mutable_platform_channels();
          GrpcAlgs::fill_repeated_field(
            std::begin(request_info.platform_channels),
            std::end(request_info.platform_channels),
            *platform_channels_proto);

          ad_request_debug_info->set_last_platform_channel_id(
            request_info.last_platform_channel_id);
        }
      }
      else if(ad_request_debug_info)
      {
        ad_request_debug_info->set_last_platform_channel_id(0);
      }

      request_info.geo_channels.insert(
        std::end(request_info.geo_channels),
      ...
      const CampaignManagerImpl::AdSlotContext& ad_slot_context)
    {
      ...
      try
      {
        ...
        if(cs_data)
        {
          const Generics::Time curr_time = GrpcAlgs::unpack_time(common_info.time());
          campaign_currency = cs_data->campaign->account->currency;
