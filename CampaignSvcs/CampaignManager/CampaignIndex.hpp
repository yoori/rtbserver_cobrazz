...
namespace AdServer::CampaignSvcs
{
  ...
  bool
  campaign_cell_less_pred(const CampaignCell* left, const CampaignCell* right) noexcept;
  ...
  public:
    CampaignIndex(ConstCampaignConfigPtr campaign_config, Logging::Logger* logger)
      noexcept;

    CampaignIndex(const CampaignIndex& init, Logging::Logger* logger)
      noexcept;
    ...
    static bool
    match_domain(const std::string& domain, const String::SubString& referer)
      noexcept;

    static bool
    check_tag_domain_exclusion(std::string_view url, const Tag* tag)
      noexcept;

    static bool
    check_tag_visibility(long tag_visibility, const Tag* tag, TraceParams* trace_params = 0);
    ...
    void
    index_campaign(const Campaign* campaign, TraceParams* trace_params = 0)
    ...
    bool
    check_campaign_channel(const Campaign* campaign, const ChannelIdHashSet& matched_channels)
      const;
  ...
  private:
    ...
    struct TagCampaignApprove
    {
      ...
      TagCampaignApprove(unsigned long campaign_id_val, const Tag* tag_val)
        noexcept;
  ...
  private:
    ...
    void
    preindex_for_tag_(const Tag* tag, const Campaign* campaign, TraceParams* trace_params)
      /*throw(Exception, eh::Exception)*/;

    void
    index_for_status_(const Campaign* campaign, UserStatus user_status, TraceParams* trace_params)
      /*throw(Exception, eh::Exception)*/;

    static bool
    colo_precheck_(UserStatus user_status, const Colocation* colocation)
      noexcept;
    ...
    static bool
    creative_preavailable_by_sizes_(const Tag* tag, const Creative* creative)
      noexcept;
    ...
    bool
    check_campaign_time_(const Campaign* campaign, const Generics::Time& current_time)
      const;

    void
    get_index_nodes_(IndexNodeList& result_nodes, const Key& request_params)
      const
      noexcept;
    ...
    static
    std::string
    decode_match_status_type_(unsigned char match_status_type) noexcept;
...
namespace AdServer::CampaignSvcs
{
  ...
  {
    uint32_t res = 0;
    res = Generics::CRC::quick(res, &(campaign->campaign_id), sizeof(campaign->campaign_id));
  ...
  CampaignSelectionCellList::insert(const CampaignSelectionCell* ins)
  {
    ...
    std::list<ConstCampaignSelectionCell_var>::insert(ins_it, ReferenceCounting::add_ref(ins));
  }
  ...
  inline
  CampaignCell::CampaignCell(const Campaign* campaign_val)
    : campaign(ReferenceCounting::add_ref(campaign_val))
  {}
  ...
  inline
  bool
  CampaignCell::operator==(const CampaignCell& right) const
  {
    return campaign->campaign_id == right.campaign->campaign_id;
  }
  ...
  CampaignCellList::insert(const CampaignCell* ins)
  {
    ...
    std::list<ConstCampaignCell_var>::insert(ins_it, ReferenceCounting::add_ref(ins));
  }
  ...
  inline
  bool
  CampaignIndex::KeyHashAdapter::operator==(const KeyHashAdapter& right) const noexcept
  {
    return match_status_type == right.match_status_type &&
      tag_id == right.tag_id &&
      country_code[0] == right.country_code[0] &&
      country_code[1] == right.country_code[1] && app_format == right.app_format;
  }
  ...
  inline
  bool
  CampaignIndex::TagCampaignApprove::operator<(const TagCampaignApprove& right) const
  {
    return campaign_id < right.campaign_id || (campaign_id == right.campaign_id && tag < right.tag);
  }
}
