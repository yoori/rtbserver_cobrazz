...
namespace AdServer::CampaignSvcs
{
  ...
  void
  PlainTextToken::encrypt(const Commons::AesEncryptKey& key, unsigned char *out) const
    noexcept
  {
  ...
  // SecTokenGenerator
  //
  SecTokenGenerator::SecTokenGenerator(const Config& config, Logging::Logger* logger)
  ...
  std::string
  SecTokenGenerator::get_text_token(in_addr_t client_address) const
    /*throw(Exception, eh::Exception)*/
  {
    const Generics::Time now = dependency_container_->get_time_of_day();

    PlainTextToken plain_token(client_address, now, dependency_container_->safe_rand());
    ...
    reinterpret_cast<uint16_t*>(crypto_data + PlainTextToken::size)[0] = htons(key_id);
  ...
  {
    std::string tmp;
    String::StringManip::base64mod_encode(tmp, ctoken, PlainTextToken::size + KEYID_BYTES);
    return tmp;
  }
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    if (base64_buf[AES_KEY_BYTES_BASE64 - 1] != '=' || base64_buf[AES_KEY_BYTES_BASE64 - 2] != '=')
    {
      Stream::Error ostr;
      ostr << FUN << ": invalid key format '" << base64_buf << "', '==' at the end expected";
      throw Exception(ostr.str());
    }
    ...
    catch(const eh::Exception& e)
    {
      ...
      os << FUN << ": '" << base64_buf << "' base64 decode failed: " << e.what();
    ...
    {
      std::ostringstream os;
      os << FUN << ": '" << base64_buf << "' invalid key length: " << key.length();
      throw Exception(os.str());
    }

    return Commons::AesEncryptKey(reinterpret_cast<const unsigned char*>(key.c_str()), key.size());
  }
}
