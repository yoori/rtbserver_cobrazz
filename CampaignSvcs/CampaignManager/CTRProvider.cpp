        ...
        alg_it != config_descriptor->algorithms.end(); ++alg_it)
    {
      if(alg_it->weight > 0)
      {
            ...
            model_it != alg_it->models.end(); ++model_it)
        {
          ...
          {
            model->ctr_evaluator = std::make_shared<FTRLCTREvaluator>(model_file);
          }
          else if(model->method == MM_XGBOOST)
          {
            model->ctr_evaluator = std::make_shared<XGBoostCTREvaluator>(model_file);
          }
          else if(model->method == MM_VANGA)
          {
            model->ctr_evaluator = std::make_shared<VangaCTREvaluator>(model_file);
          }
          else if(model->method == MM_TRIVIAL)
          {
            model->ctr_evaluator = std::make_shared<TrivialCTREvaluator>(logger_, model_file);
          }
          else if(model->method == MM_CATBOOST)
          {
            model->ctr_evaluator = std::make_shared<CatBoostCTREvaluator>(
              model_file, model->features_size);
          }
          else
          {
            assert(0);
