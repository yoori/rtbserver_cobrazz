...
namespace AdServer::CampaignSvcs
{
  ...
  struct Template: public ReferenceCounting::AtomicImpl
  {
    ...
    std::string
    instantiate(const TemplateParams* request_params, const TemplateParamsList& creative_params)
      /*throw(InvalidParams,
        InvalidTemplate,
  ...
  protected:
    struct TemplateWithState
    {
      ...
      TemplateWithState(const typename FactoryType::State& state_val, Template* templ_val)
        : state(state_val),
...
namespace AdServer::CampaignSvcs
{
  ...
  inline
  bool
  CreativeTemplateKey::operator==(const CreativeTemplateKey& right) const
    /* noexcept */
  {
    return
      creative_format == right.creative_format &&
      creative_size == right.creative_size && app_format == right.app_format;
  }
  ...
  TemplateMap<KeyType, ValueType, ValueHandlerType, FactoryType>::assign(
    ...
    /*throw(Exception)*/
  {
    for (typename KeySet::const_iterator it = key_set.begin(); it != key_set.end(); ++it)
    {
      typename KeyMap::iterator key_it = source_map.key_map_.find(*it);
      if (key_it != source_map.key_map_.end())
      {
        typename ValueMap::iterator val_it = source_map.value_map_.find(key_it->second);
  ...
  template<
    ...
    typename FactoryType>
  bool
  TemplateMap<KeyType, ValueType, ValueHandlerType, FactoryType>::exist(const KeyType& key) const
    noexcept
  {
    ...
    /*throw(Template::FileNotExists, Template::InvalidTemplate)*/
  {
...

    {
      ...
      {
        if (!factory_.need_update(value_map_it->first, value_map_it->second.state) ||
    ...
    try
    {
      updated_template = factory_.update(current_template, value_map_it->first, current_state);
    }
    catch(...)
    {
      typename SyncPolicy::WriteGuard lock(value_map_it->second.lock);
  ...
  template<
    ...
    typename FactoryType>
  Template*
  TemplateMap<KeyType, ValueType, ValueHandlerType, FactoryType>::get(const KeyType& key) const
    /*throw(Template::FileNotExists, Template::InvalidTemplate, Exception)*/
  {
    typename KeyMap::const_iterator it = key_map_.find(key);
    if (it == key_map_.end())
