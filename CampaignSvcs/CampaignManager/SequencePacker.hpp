...
namespace AdServer::CampaignSvcs
{
  ...
  protected:
    ...
    public:
      ...
      /* adapters for search */
      ElementSeqHashAdapter(const ElementSeqType* el_seq, const ElementType* el);
  ...
  public:
    ...
    ConstElementSeq_var create_seq(const ElementSeqType* el_seq, const ElementType* el) noexcept;
...
namespace AdServer::CampaignSvcs
{
    ...
    const ElementType* right_cell)
  {
    ...
    if (left_it != left->end())
    {
      return !right_cell_found && *(*left_it) == *right_cell && ++left_it == left->end();
    }
    else if (right_it != right->end())
    {
      return !left_cell_found && *(*right_it) == *left_cell && ++right_it == right->end();
    }
  ...
  bool
  SequencePacker<ElementType, ElementSeqType, ElementSeqHashType>::
  ElementSeqHashAdapter::operator==(const ElementSeqHashAdapter& right) const
  {
    ...
    return size_ == right.size_ &&
       compare_lists_(cell_list_, right.cell_list_, add_cell_, right.add_cell_);
  }
  ...
  typename SequencePacker<ElementType, ElementSeqType, ElementSeqHashType>::
    ConstElementSeq_var
  SequencePacker<ElementType, ElementSeqType, ElementSeqHashType>::
  create_seq(const ElementSeqType* el_seq, const ElementType* el) noexcept
  {
    typename SequencePacker<ElementType, ElementSeqType, ElementSeqHashType>::
    ...
    const ElementType* el)
  {
    ...
    if (el_seq)
    {
      typename HashAdapterTable::const_iterator el_seq_it = adapter_table_.find(el_seq);
    ...
    if (el_seq_adapter_it == adapters_.end())
    {
      ...
      ElementSeqHashAdapter new_el_seq_adapter(search_el_seq_adapter, ret_list.in());

      adapter_table_.insert(std::make_pair(ret_list.in(), new_el_seq_adapter));
    ...
    const ElementSeqBase* el_seq)
  {
    if (el_seq)
    {
      typename HashAdapterTable::iterator el_seq_it = adapter_table_.find(el_seq);
      ...
      */
      ...
      typename HashAdapterSet::iterator el_seq_adapter_it = adapters_.find(el_seq_it->second);
