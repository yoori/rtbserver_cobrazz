...
namespace AdServer::CampaignSvcs
{
  ...
  using CampaignKeywordCreativeList = Generics::MonoList<CampaignKeywordCreative>;
  ...
  using CPCKeywordCreativeMap = Generics::MonoMap<RevenueDecimal, CampaignKeywordCreativeList>;
  ...
  using RandomTextSelectionBySizeList = Generics::MonoList<RandomTextSelectionBySize>;
  ...
  using CTRWeightedCampaignHolderList = Generics::MonoList<CTRWeightedCampaignHolder>;
  ...
  namespace
  {
    ...
    inline unsigned long
    ctr_creative_holder_balance_function(const SizedCreativeHolder& creative_holder) noexcept
    {
  ...
  CampaignSelector::ExpectedEcpm::value() const noexcept
  {
    ...
    const ExtRevenueDecimal ex_exp_ecpm = ExtRevenueDecimal::div(ex_ecpm_quad_sum_, ex_ecpm_sum_);
  ...
  CampaignSelector::ExpectedEcpm::operator+= (const RevenueDecimal& arg) noexcept
  {
    ...
    ex_ecpm_quad_sum_ += ExtRevenueDecimal::mul(ex_ecpm, ex_ecpm, Generics::DMR_FLOOR);
  ...
  RevenueDecimal
  CampaignSelector::default_campaign_ecpm_(const Tag* tag, const Campaign* campaign)
    noexcept
  {
    if (campaign->use_ctr())
    {
      return RevenueDecimal::mul(
        std::min(
          RevenueDecimal::mul(
            RevenueDecimal::mul(campaign->ctr, ECPM_FACTOR, Generics::DMR_FLOOR),
            tag->adjustment,
            Generics::DMR_FLOOR),
          ECPM_FACTOR),
    ...
    noexcept
  {
    if (campaign->use_ctr())
    {
      ctr = campaign_ctr_(result_creative, ctr_calculation_context, available_creatives);
    ...
    if (campaign_keyword)
    {
      return campaign_keyword->campaign->
        account->currency->to_system_currency(
          RevenueDecimal::mul(
            RevenueDecimal::mul(ECPM_FACTOR, campaign_keyword->ctr, Generics::DMR_FLOOR),
            campaign_keyword->max_cpc,
            Generics::DMR_FLOOR));
    }

    return default_campaign_ecpm_(tag, campaign);
  }
  ...
  const Tag::Size*
  CampaignSelector::select_tag_size_(const Tag* tag, const Creative* creative)
    noexcept
  {
    ...
    {
      Creative::SizeMap::const_iterator cs_it = creative->sizes.find(ts_it->first);
    ...
    noexcept
  {
        ...
        cit != campaign_list.end(); ++cit)
    {
      ...
      if (campaign_selection_index_->check_campaign(
           ...
           0) &&
         campaign_selection_index_->check_campaign_channel((*cit)->campaign, matched_channels))
      {
    ...
    noexcept
  {
    ...
    for (CampaignIndex::CampaignSelectionCellPtrList::const_iterator cmp_it = campaign_list.begin();
        cmp_it != campaign_list.end(); ++cmp_it)
    {
      if (campaign_selection_index_->check_campaign(
           ...
           request_params.user_id,
           0) &&
         campaign_selection_index_->check_campaign_channel((*cmp_it)->campaign, matched_channels))
      {
      ...
      ++tag_size_it)
    {
      ...
      {
        conv_rate_calculation_context = conv_rate_calculation->create_context(tag_size_it->second);
      }

      // fetch unknown_ctr_campaign_candidates
      // filter creatives by ctr with known size
      for (CTRWeightedCampaignHolderList::iterator wit = unknown_ctr_campaign_candidates.begin();
      ...
      // not eval
      for (CTRWeightedCampaignHolderList::iterator wit = known_ctr_campaign_candidates.begin();
    ...
    noexcept
  {
        ...
        cmp_it != campaign_list.end(); ++cmp_it)
    {
         ...
         campaign_selection_index_->check_campaign_channel((*cmp_it)->campaign, matched_channels))
      {
        ...
        if (!available_creatives.empty()) // any creative can be selected
        {
          const Creative* result_creative = select_display_creative_(available_creatives);
    ...
    noexcept
  {
    ...
    for (; cmp_it != campaign_list.end(); ++cmp_it)
    {
         ...
         campaign_selection_index_->check_campaign_channel((*cmp_it)->campaign, matched_channels))
      {
        ...
        if (!check_min_ecpm_((*cmp_it)->tag_pricing, request_params.min_ecpm, current_ecpm))
        {
          break;
        }
    ...
    noexcept
  {
    ...
    for (CampaignKeywordMap::const_iterator kw_it = filtered_campaign_keywords.begin();
      ...
      */
      ...
      if (!random_select_campaigns.empty())
      {
        if (auction_type == AT_RANDOM)
        {
          ...
          unsigned long pos = request_params.random2 % random_select_campaigns.size();

          WeightedCampaignList::iterator cmp_it = random_select_campaigns.begin();
          ...
          // defined ctr_calculation_context
        {
          ...
          if (auction_type == AT_PROPORTIONAL_PROBABILITY)
          {
            ...
            for (WeightedCampaignList::const_iterator it = random_select_campaigns.begin();
          ...
          else // ctr_calculation_context defined
          {
            ...
            for (WeightedCampaignList::iterator it = random_select_campaigns.begin();
                it != random_select_campaigns.end(); ++it)
            {
            ...
            unsigned long pos = request_params.random2 % max_ecpm_campaigns.size();
  ...
  CampaignSelector::convert_text_candidates_to_cpc_map_(
    ...
    noexcept
  {
    for (WeightedCampaignKeywordList::const_iterator kit = text_campaign_candidates.begin();
  ...
  void
  CampaignSelector::revert_cost_(WeightedCampaignKeywordList& weighted_campaign_keywords)
    noexcept
  {
    for (WeightedCampaignKeywordList::iterator it = weighted_campaign_keywords.begin();
        it != weighted_campaign_keywords.end(); ++it)
    {
      if (it->campaign_keyword)
      {
        ...
        {
          it->actual_cpc.ceil(it->campaign_keyword->campaign->account->currency->fraction);
        }
      }
    }
  }
  ...
  CampaignSelector::select_text_campaigns_randomly_(
    ...
    noexcept
  {
    unsigned long grouped_text_campaign_candidates_size = grouped_text_campaign_candidates.size();

    for (unsigned long select_i = 0;
        select_i < max_text_creatives && !grouped_text_campaign_candidates.empty();
        ++select_i)
    {
      unsigned long up_pos = Generics::unsafe_rand(grouped_text_campaign_candidates_size);
      ...
      WeightedCampaignKeywordPtrArray::iterator wcmp_it = cmp_cell_it->begin();
    ...
    noexcept
  {
    ...
    for (unsigned long select_i = 0; select_i < max_text_creatives; ++select_i)
    {
      ...
      for (WeightedCampaignKeywordList::iterator it = text_campaign_candidates.begin();
          it != text_campaign_candidates.end(); ++it)
      {
        if (it->campaign && it->actual_ecpm >= min_ecpm)
        {
      ...
      if (cur_ecpm_sum != RevenueDecimal::ZERO)
      {
        ...
        for (; group_it != grouped_text_campaign_candidates.end(); ++group_it)
        {
      ...
      for (WeightedCampaignKeywordPtrArray::iterator cmp_it = cmp_list.begin();
  ...
  {
    Tag::SizeMap tag_sizes;
    tag_sizes.insert(std::make_pair(tag_size->size->size_id, ReferenceCounting::add_ref(tag_size)));
        ...
        ++cit)
    {
      ...
      for (WeightedCampaignKeywordList::const_iterator sub_cit = kws.begin();
          sub_cit != kws.end() && selected_keywords < max_keywords;
          ++sub_cit)
      {
        bool filtered = false;

        AccountDef::TextAdserving ta_type = sub_cit->campaign->account->get_text_adserving();

        // check multi showing filters
        if (ta_type == AccountDef::TA_ALL) // one per CCG
        {
          filtered = ccg_filter_ids.find(sub_cit->campaign->campaign_id) != ccg_filter_ids.end();
        }
        else if (ta_type == AccountDef::TA_ADVERTISER_ONE) // one per advertiser
        {
          filtered = advertiser_filter_ids.find(sub_cit->campaign->advertiser->account_id) !=
            advertiser_filter_ids.end();
        }
        else if (ta_type == AccountDef::TA_ONE) // one per account
        {
          filtered = account_filter_ids.find(sub_cit->campaign->account->account_id) !=
        ...
        if (!filtered)
        {
          ...
          if (creative_candidate)
          {
            get_or_create_mono_mapped_(filtered_cpc_keyword_map, cit->first).push_back(
              CampaignKeywordCreative(
                *sub_cit,
                available_creatives, // clear available_creatives
                creative_candidate));
            ...
            {
              advertiser_filter_ids.insert(sub_cit->campaign->advertiser->account_id);
            }
            else if (ta_type == AccountDef::TA_ONE) // one per account
            {
              account_filter_ids.insert(sub_cit->campaign->account->account_id);
            }
          }
        }
      }
    ...
    if (!filtered_cpc_keyword_map.empty())
    {
      ...
      CPCKeywordCreativeMap::reverse_iterator cit = filtered_cpc_keyword_map.rbegin();

      for (; cit != filtered_cpc_keyword_map.rend() && selected_keywords < max_keywords - 1; ++cit)
      {
        ...
        for (; sub_cit != kws.end() && selected_keywords < max_keywords - 1; ++sub_cit)
        {
          if (bidder_number > 0)
          {
            ++selected_keywords;
      ...
      const CampaignKeywordCreativeList& kws = filtered_cpc_keyword_map.rbegin()->second;

      const WeightedCampaignKeyword& weighted_campaign = kws.front().weighted_campaign_keyword;
      ...
      if (weighted_campaign.campaign_keyword && bidder_number > 1)
      {
        ...
        {
          RevenueDecimal ctr_mul = RevenueDecimal::mul(ctr, ECPM_FACTOR, Generics::DMR_FLOOR);

          // actual_cpc can be == 0 if max_ecpm_bid == 0 (
          //   very low ctr * max_cpc),
          // this will be solved after commision revert
          actual_cpc = currency->from_system_currency(RevenueDecimal::div(actual_ecpm, ctr_mul));
        }
      }
      ...
      bool text_ads_selected = (non_first_bidders_actual_ecpm_sum + actual_ecpm) >= min_sum_ecpm;
  ...
  RevenueDecimal
  CampaignSelector::sum_campaign_ecpm_(const WeightedCampaignList& campaign_candidates)
    noexcept
  {
    RevenueDecimal result = RevenueDecimal::ZERO;

    for (WeightedCampaignList::const_iterator it = campaign_candidates.begin();
  ...
  RevenueDecimal
  CampaignSelector::campaign_ecpm_expected_value_(const WeightedCampaignList& campaign_candidates)
    noexcept
  {
    ExpectedEcpm expected_ecpm;

    for (WeightedCampaignList::const_iterator cmp_it = campaign_candidates.begin();
  ...
  CampaignSelector::group_id_map_(
    ...
    noexcept
  {
    for (IdWeightedCampaignKeywordMap::const_iterator group_it = campaigns.begin();
        group_it != campaigns.end(); ++group_it)
    {
      ...
      text_campaign_map.insert(std::make_pair(exp_ecpm_val, group_it->second));
    }
  }
  ...
  CampaignSelector::group_id_map_(
    ...
    noexcept
  {
    for (IdWeightedCampaignKeywordMap::iterator group_it = campaigns.begin();
        group_it != campaigns.end(); ++group_it)
    {
    ...
    noexcept
  {
        ...
        ++group_it)
    {
      ...
      if (cur_elements.empty() || group_max > *cur_elements.begin())
      {
    ...
    for (std::set<RevenueDecimal>::const_iterator it = cur_elements.begin();
  ...
  {
    AccountDef::TextAdserving ta_type = wc->campaign->account->get_text_adserving();
  ...
  CampaignSelector::fill_id_weighted_campaigns_keyword_maps(
    ...
    WeightedCampaignKeywordList& text_campaign_candidates)
  {
    for (WeightedCampaignKeywordList::iterator cmp_it = text_campaign_candidates.begin();
        cmp_it != text_campaign_candidates.end(); ++cmp_it)
    {
      AccountDef::TextAdserving ta_type = cmp_it->campaign->account->get_text_adserving();
  ...
  CampaignSelector::group_text_campaigns_(
    ...
    noexcept
  {
    IdWeightedCampaignKeywordMaps campsigns_maps(text_campaign_map.get_allocator().arena());
    ...
    noexcept
  {
    ...
    for (WeightedCampaignKeywordPtrArray::iterator cmp_it = text_campaign_candidates.begin();
        cmp_it != text_campaign_candidates.end(); ++cmp_it)
    {
      AccountDef::TextAdserving ta_type = (*cmp_it)->campaign->account->get_text_adserving();
    ...
    noexcept
  {
    ...
    WeightedCampaignList::iterator it = campaign_candidates.begin();
  ...
  CampaignSelector::WeightedCampaignList::iterator
  CampaignSelector::select_campaign_randomly_(WeightedCampaignList& campaign_candidates)
    noexcept
  {
    if (campaign_candidates.empty())
    {
                    ...
                    if (cmp_it->campaign->use_ctr())
                    {
                      if (cmp_it->campaign_keyword)
                      {
                        new_ecpm = cmp_it->campaign_keyword->campaign->
                          account->currency->to_system_currency(
                            RevenueDecimal::mul(
                              RevenueDecimal::mul(ECPM_FACTOR, max_ctr, Generics::DMR_FLOOR),
    ...
    noexcept
  {
    ...
    if (!wg_display_campaign_candidates.empty())
    {
      ...
      unsigned long campaign_offset = Generics::unsafe_rand(wg_display_campaign_candidates.size());

      WeightedCampaignList::iterator cmp_it = wg_display_campaign_candidates.begin();
       ...
       )
    {
      ...
      if (!request_params.only_display_ad)
      {
        ...
        select_result.min_text_ecpm = std::max(text_tag_pricing->cpm, request_params.min_ecpm);
        ...
        for (auto ts_it = text_selections.begin(); ts_it != text_selections.end(); ++ts_it)
        {
              ...
              cmpg_it != ts_it->grouped_campaign_candidates.end(); ++cmpg_it)
          {
            ...
            for (auto cmp_it = cmpg_it->begin(); cmp_it != cmpg_it->end(); ++cmp_it)
            {
      ...
      for (RandomTextSelectionBySizeList::const_iterator ts_it = text_selections.begin();
           ...
           sum_max_text_creatives == 0))
      {
        ...
        WeightedCampaignList::iterator cmp_it = display_campaign_candidates.begin();
      ...
      else if (text_count > 0)
      {
        ...
        RandomTextSelectionBySizeList::iterator ts_it = text_selections.begin();
        ...
        result_text_campaigns.reset(new WeightedCampaignKeywordList(arena_));
    ...
    noexcept
  {
    ...
    if (!wg_display_campaign_candidates.empty())
    {
      RevenueDecimal wg_display_ecpm_sum = sum_campaign_ecpm_(wg_display_campaign_candidates);
      ...
      result_display_campaign = std::move(
        *select_campaign_prop_probability_(wg_display_campaign_candidates, ecpm_offset));
    }
       ...
       )
    {
      ...
      if (!text_selections.empty())
      {
        ...
        for (TextSelectionBySizeList::iterator text_selection_it = text_selections.begin();
      ...
      RevenueDecimal display_ecpm_expected_value =
        campaign_ecpm_expected_value_(display_campaign_candidates);
          ...
          text_ecpm_expected_value == RevenueDecimal::ZERO)
      {
        ...
        if (!result_text_selection || !result_text_selection->campaign_candidates.empty())
        {
      ...
      {
        const RevenueDecimal display_ecpm_sum = sum_campaign_ecpm_(display_campaign_candidates);
        ...
        {
          result_cmp_it = select_campaign_randomly_(display_campaign_candidates);
        }

        result_display_campaign = std::move(*result_cmp_it);
      }
      else if (result_text_selection && !result_text_selection->campaign_candidates.empty())
      {
        result_text_campaigns.reset(new WeightedCampaignKeywordList(arena_));
    ...
    noexcept
  {
       ...
       )
    {
      ...
      if (!request_params.only_display_ad)
      {
        select_result.min_text_ecpm = std::max(tag_pricing->cpm, request_params.min_ecpm);
            ...
            tag_size_it != tag_sizes.end(); ++tag_size_it)
        {
          if (tag_size_it->second->max_text_creatives != 0)
          {
            ...
            */
            ...
            convert_text_candidates_to_cpc_map_(cpc_keyword_map, filtered_text_campaign_candidates);
        ...
        if (!text_selections.empty())
        {
          ...
          {
            const unsigned long pos = Generics::unsafe_rand() % text_selections_size;
            std::advance(ts_it, pos);
          }

          assert(ts_it->tag_sizes.size() == 1);
          select_result.tag_size = ts_it->tag_sizes.begin()->second;
          result_weighted_campaign_keywords.reset(new WeightedCampaignKeywordList(arena_));
          result_weighted_campaign_keywords->swap(ts_it->campaign_candidates);
    ...
    AdSelectionResult& select_result)
  {
    ...
    {
      conv_rate_calculation = conv_rate_provider_->create_calculation(request_params_ptr);
    ...
    if (result_weighted_campaign_keywords.get())
    {
      ...
      for (WeightedCampaignKeywordList::iterator kw_it = result_weighted_campaign_keywords->begin();
    ...
    select_result.min_no_adv_ecpm = std::max(tag_pricing->cpm, request_params.min_ecpm);
