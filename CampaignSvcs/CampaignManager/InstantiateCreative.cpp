...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    bool CampaignManagerImpl::instantiate_creative_preview(
      ...
      const String::SubString& ext_tag_id)
    {
      ...
      request_args[CreativeTokens::PUB_PIXELS_OPTIN] = "";
      request_args[CreativeTokens::PUB_PIXELS_OPTOUT] = "";
      const char* const COUNTRY = request_params.location().size() != 0 ?
      ...
      {
        AdServer::Commons::ExternalUserIdArray user_ids;
        Commons::UserId req_user_id = GrpcAlgs::unpack_user_id(request_params.user_id());
      ...
      {
        const char* referer = request_params.full_referer().empty() ?
          request_params.referer().c_str() : request_params.full_referer().c_str();
      ...
      request_args[CreativeTokens::UID] = request_params.signed_user_id();
      const Commons::UserId user_id = GrpcAlgs::unpack_user_id(request_params.track_user_id());
      if(!user_id.is_null())
      {
        request_args[CreativeTokens::UNSIGNEDUID] = user_id.to_string();
      }
      ...
      request_args[CreativeTokens::GREQUESTID] =
        rid_signer_.sign(
          GrpcAlgs::unpack_request_id(request_params.request_id())).str();
      ...
      const AccountIdList* consider_pub_pixel_accounts) noexcept
    {
      ...
      if(!request_ids.empty())
      {
        ...
        ostr << AdProtocol::REQUEST_ID << "=" << request_ids << "&" <<
          AdProtocol::GLOBAL_REQUEST_ID << "=" <<
          request_result_params.request_id.to_string() << "&" <<
          AdProtocol::BID_TIME << "=" << GrpcAlgs::unpack_time(request_params.time()).tv_sec;
        ...
        AdServer::Commons::UserId track_user_id = GrpcAlgs::unpack_user_id(
          request_params.track_user_id());
    ...
    CampaignManagerImpl::init_click_params0_(
      ...
      const InstantiateParams& inst_params,
      const Proto::CommonAdRequestInfo& request_params,
      const AdSlotContext& ad_slot_context) noexcept
    {
      const char* LOCAL_EQL = (
        request_params.request_type() == AR_YANDEX ? "=" : EQL);
      const char* LOCAL_AMP = (
      ...
      ostr_click_url <<
        ...
        AdProtocol::CAMPAIGN_MANAGER_INDEX << LOCAL_EQL <<
          campaign_manager_config_.service_index() << LOCAL_AMP <<
        AdProtocol::BID_TIME << LOCAL_EQL << GrpcAlgs::unpack_time(request_params.time()).tv_sec;
      ...
      const AdServer::Commons::UserId track_user_id(
        GrpcAlgs::unpack_user_id(request_params.track_user_id()));
      const AdServer::Commons::UserId user_id(
        GrpcAlgs::unpack_user_id(request_params.user_id()));
      ...
      for(int i = 0; i < request_params.tokens().size(); ++i)
      {
        std::string mime_token_name;
        String::StringManip::mime_url_encode(
          String::SubString(request_params.tokens()[i].name()),
      ...
      bool fill_creative_params)
   {
     ...
     AdServer::Commons::UserId user_id = GrpcAlgs::unpack_user_id(
       request_params.user_id());
     ...
     for(const auto& token : request_params.tokens())
     {
       ...
       url_ostr << "&t." << token.name() << '=' << mime_enc_tok_value;
     }
      ...
      const google::protobuf::RepeatedField<std::uint32_t>* exclude_pubpixel_accounts)
    {
      ...
      try
      {
        ...
        std::string inst_rule(
          request_params.request_type() == AR_GOOGLE &&
          creative->https_safe_flag ?
          AdInstantiateRule::SECURE.str() : request_params.creative_instantiate_type());
      ...
      const AdSlotContext& ad_slot_context) noexcept
    {
      ...
      int tok_i = 0;
      const auto& tokens = request_params.tokens();
      for(; tok_i < tokens.size(); ++tok_i)
      {
        String::SubString tok_name(tokens[tok_i].name());
        String::SubString param_name = tok_name;

        std::string enc_tok_value;
        encode_parameter(
          enc_tok_value,
          String::SubString(tokens[tok_i].value()),
          true);
      ...
      bool /*need_absolute_urls*/)
    {
        ...
        i < sizeof(TRACK_TOKENS) / sizeof(TRACK_TOKENS[0]); ++i)
      {
          ...
          )
        {
          ...
          for(const auto& token : request_params.tokens())
          {
            args[token.name()] = OptionValue(0, token.value());
          }
      ...
      const Creative* creative)
    {
      ...
      for(const auto& token : ad_slot.native_data_tokens())
      {
        OptionTokenValueMap::const_iterator token_it = creative->tokens.find(
          token.name());
        if (token_it == creative->tokens.end() || token_it->second.value.empty())
      ...
      for(const auto& token : ad_slot.native_image_tokens())
      {
        unsigned long width = token.width();
        unsigned long height = token.height();

        auto token_it = creative->tokens.find(token.name());
        if (token_it == creative->tokens.end() || token_it->second.value.empty())
        {
          if (token.required())
          {
        ...
        else
        {
          ...
          args[token.name()] = token_it->second;
          ...
          try
          {
            ...
            request_result_params.native_image_tokens.insert(
              std::make_pair(token.name(), image));
          }
          catch(const eh::Exception& ex)
          {
            if (token.required())
