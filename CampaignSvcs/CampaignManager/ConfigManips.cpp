...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    CampaignManagerImpl::get_config(GetConfigRequestPtr&& request)
    {
      ...
      try
      {
        ...
        for(const auto& [key, campaign]: config->campaigns)
        {
          ...
          fill_campaign_contracts_(
            *campaign_info_response.mutable_contracts(),
            campaign->contracts);
        }
    ...
    CampaignManagerImpl::fill_campaign_contracts_(
      ...
      const Campaign::CampaignContractArray& contracts) noexcept
    {
      contract_seq.Reserve(
        contract_seq.size() + static_cast<int>(contracts.size()));
      for (const auto& contract : contracts)
      {
        auto& contract_response = *contract_seq.Add();
        contract_response.set_ord_contract_id(contract->ord_contract_id);
        contract_response.set_ord_ado_id(contract->ord_ado_id);
        contract_response.set_id(contract->id);
        contract_response.set_date(contract->date);
        contract_response.set_type(contract->type);
        contract_response.set_client_id(contract->client_id);
        contract_response.set_client_name(contract->client_name);
        contract_response.set_contractor_id(contract->contractor_id);
        contract_response.set_contractor_name(contract->contractor_name);
      }
    }
  }
}
