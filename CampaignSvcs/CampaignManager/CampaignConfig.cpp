...
namespace AdServer::CampaignSvcs::AdInstances
{
    ...
    CCGRateType ccg_rate_type) const noexcept
  {
    ...
    if ((tp_it = tag_pricings.find(Tag::TagPricingKey(country_code, ccg_type, ccg_rate_type))) !=
      tag_pricings.end())
    {
      return &(tp_it->second);
    }

    if ((tp_it = tag_pricings.find(Tag::TagPricingKey(country_code, ccg_type, CR_ALL))) !=
      tag_pricings.end())
    {
      return &(tp_it->second);
    }

    if ((tp_it = tag_pricings.find(Tag::TagPricingKey(country_code, CT_ALL, ccg_rate_type))) !=
      tag_pricings.end())
    {
      return &(tp_it->second);
    }

    if ((tp_it = tag_pricings.find(Tag::TagPricingKey(country_code, CT_ALL, CR_ALL))) !=
      tag_pricings.end())
    {
      return &(tp_it->second);
    }
  ...
  const Tag::TagPricing*
  Tag::select_country_tag_pricing(const char* country_code) const noexcept
  {
    CountryTagPricingMap::const_iterator tp_it;

    if ((tp_it = country_tag_pricings.find(country_code)) != country_tag_pricings.end())
    {
      return &(tp_it->second);
    }

    if ((tp_it = country_tag_pricings.find("")) != country_tag_pricings.end())
    {
      return &(tp_it->second);
    }
  ...
  const Tag::TagPricing*
  Tag::select_no_impression_tag_pricing(const char* country_code) const noexcept
  {
    ...
    CountryTagPricingPtrMap::const_iterator tp_it = no_imp_tag_pricings.find(country_code);
