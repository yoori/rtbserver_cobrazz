    ...
    */
      ...
      const ChannelIdHashSet& matched_channels)
    {
      ...
      if(!passback)
      {
        ...
        if(!ad_selection_result.selected_campaigns.empty())
        {
          ad_slot_result.set_erid(
            ad_selection_result.selected_campaigns.begin()->creative->erid);
        }

        for(auto cmp_it = ad_selection_result.selected_campaigns.begin();
          cmp_it != ad_selection_result.selected_campaigns.end();
          ++cmp_it)
        {
          if(cmp_it->campaign)
          {
            const auto& contracts = cmp_it->campaign->contracts;
            auto* ad_slot_result_contracts = ad_slot_result.mutable_contracts();
            ad_slot_result_contracts->Reserve(contracts.size() + ad_slot_result_contracts->size());
            for (const auto& contract : contracts)
            {
              auto* res_contract = ad_slot_result_contracts->Add();
              res_contract->set_ord_contract_id(contract->ord_contract_id);
              res_contract->set_ord_ado_id(contract->ord_ado_id);
              res_contract->set_id(contract->id);
              res_contract->set_date(contract->date);
              res_contract->set_type(contract->type);
              res_contract->set_client_id(contract->client_id);
              res_contract->set_client_name(contract->client_name);
              res_contract->set_contractor_id(contract->contractor_id);
              res_contract->set_contractor_name(contract->contractor_name);
            }
          }
        }
