...
VERSION=1.0.0.0
#UNIXCOMMONSDIR=$2
UNIXCOMMONSDIR=/app/unixcommons_cobrazz
SERVERDIR=/app/rtbserver_cobrazz
...
# get spec from tag
cp SPECS/ci/server.spec "$DSP_SPEC_FILE"
...
rsync -av --exclude 'build' --exclude '.git' --exclude '.github' --exclude '.idea' --exclude '.svn' --exclude 'ci' --exclude 'cmake-build-debug' "$SERVERDIR" "$RES_TMP/foros-server-$VERSION"
rsync -av --exclude 'build' --exclude '.git' --exclude '.github' --exclude '.idea' --exclude '.svn' --exclude 'ci' --exclude 'cmake-build-debug' "$UNIXCOMMONSDIR" "$RES_TMP/foros-server-$VERSION"
