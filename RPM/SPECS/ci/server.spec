...
%define __product               rtbserver_cobrazz
%define __unixcommons           unixcommons_cobrazz

%define __ob_ver_req            4.3.2
%define __inst_root             /opt/foros/%{__product}
%define __osbe_build_dir        build
...
%prep
#extracts from /root/rpmbuild/SOURCE to /root/rpmbuild/BUILD
%setup -q -n foros-server-%{version}
pip3 install --user protobuf jinja2

cpp_flags=''
if [ '%{?buildType}' == 'nb' ]; then cpp_flags='-DDEBUG '; fi

mkdir -p %{__unixcommons}/%{__osbe_build_dir}
echo -e "build DIRECTORY for %{__unixcommons}"
echo %{__unixcommons}/%{__osbe_build_dir}
...
echo -e "build DIRECTORY for %{__product}"
echo %{__product}/{__osbe_build_dir}
...
cmake -DUNIXCOMMONS=$(pwd)/../../%{__unixcommons} -DCMAKE_BUILD_TYPE=RelWithDebInfo ../
#${product_root}/configure --enable-no-questions --enable-guess-location=no --prefix=%{__inst_root}
...
rm -f %{__product}.list
rm -f tests.list
test_dir="%{__unixcommons}/tests %{__product}/tests"
if [ "%{__osbe_build_dir}" != "." ]; then
  test_dir="$test_dir %{__unixcommons}/%{__osbe_build_dir}/tests %{__product}/%{__osbe_build_dir}/tests %{__product}/%{__osbe_build_dir}/Utils"
fi
echo "save tests.list in ${pwd}"
