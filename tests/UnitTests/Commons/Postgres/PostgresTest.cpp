...
namespace AdServer::UnitTests
{
    ...
    : file_logger_()
  {
    ...
    file_logger_ = new Logging::DistributorLogger(loggers.begin(), loggers.end());
  }
  ...
  int PostgresTest::run() noexcept
  {
    ...
    if (test_case_ == TC_RES || test_case_ == TC_ALL)
    {
      ...
      ret_value += resultset_test(false, Commons::Postgres::TEXT_FORMAT);
      log_("ResultSet test case async in binary format");
      ret_value += resultset_test(false, Commons::Postgres::BINARY_FORMAT);
      log_("ResultSet test case sync in text format");
      ret_value += resultset_test(true, Commons::Postgres::TEXT_FORMAT);
      log_("ResultSet test case sync in binary format");
      ret_value += resultset_test(true, Commons::Postgres::BINARY_FORMAT);
    }
  ...
  void PostgresTest::generate_record_(Record& rec) noexcept
  {
    ...
    rec.timestamp_value = Generics::Time(Generics::safe_rand(), Generics::safe_rand() % 1000000);
      //644527776, 700300);
      //291538254, 626021);
  ...
  PostgresTest::CharArray PostgresTest::generate_uid_data_(size_t count, unsigned& size)
    noexcept
  {
    CharArray array;
    size = 0;
  ...
  std::string PostgresTest::write_uid_file_(char* buf, unsigned size) /*throw(Exception)*/
  {
    ...
    std::ofstream my_file(file_name.c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
  ...
  PostgresTest::CharArray PostgresTest::read_file_(const std::string& file_name, unsigned size)
    /*throw(Exception)*/
  {
    CharArray array(size);
    std::ifstream my_file(file_name.c_str(), std::ios::in | std::ios::binary);
  ...
  void PostgresTest::compary_content_(const char* array1, const char* array2, unsigned array_size)
    /*throw(Exception)*/
  {
  ...
  int PostgresTest::lob_test() noexcept
  {
    ...
    try
    {
      ...
      AdServer::Commons::Postgres::ResultSet_var res = con->execute_query(sql_create_lob_table);
  ...
  int PostgresTest::object_test() noexcept
  {
    ...
    try
    {
      ...
      AdServer::Commons::Postgres::ResultSet_var res = con->execute_query(sql_create_obj_table);
  ...
  int PostgresTest::numeric_test() noexcept
  {
    ...
    try
    {
      ...
      AdServer::Commons::Postgres::ResultSet_var res = con->execute_query(sql_7);
      ...
      while (res->next())
      {
        ...
        fractional = 0;
        ret_value |= check_value_(__func__, value, TestDecimal2(false, whole, fractional));
        fractional = 1;
        for (size_t index = 9; index > 1; index--, fractional *= 10)
        {
  ...
  void PostgresTest::query_perf_one_row(AdServer::Commons::Postgres::Connection* con) noexcept
  {
    ...
    file_logger_->sstream(LEVEL_LOW)
      << "select query performance in mode 'row-by-row': " << timer.elapsed_time();
  }

  void PostgresTest::query_perf_all_row(AdServer::Commons::Postgres::Connection* con) noexcept
  {
    ...
    timer.stop();
    file_logger_->sstream(LEVEL_LOW)
      << "select query performance in mode 'full fetch': " << timer.elapsed_time();
  }

  void PostgresTest::query_perf_cursor(AdServer::Commons::Postgres::Connection* con) noexcept
  {
    for (auto i = 1000; i < 10000001; i *= 4)
    {
      ...
      timer.stop();
      file_logger_->sstream(LEVEL_LOW)
        << "Execute performance query in 'cursor(" << i << ")' mode: " << timer.elapsed_time();
    }
  }
  ...
  int PostgresTest::active_query_test() noexcept
  {
    ...
    try
    {
      ...
      AdServer::Commons::Postgres::ResultSet_var res = con1->execute_statement(stm, false);
      bool next_rec;
  ...
  int PostgresTest::connection_actions_(AdServer::Commons::Postgres::Connection* con, int error)
    noexcept
  {
    int ret_value = 0;
    try
    {
      ...
      AdServer::Commons::Postgres::ResultSet_var res = con->execute_query(sql_1);
    ...
    {
      file_logger_->sstream(error ? LEVEL_ERROR : LEVEL_MIDDLE)
        << "Error on connection actions '" << database_ << "' :" << e.what() << std::endl;
      ret_value = error;
    }
    ...
    noexcept
  {
    ...
    {
      file_logger_->sstream(error ? LEVEL_ERROR : LEVEL_MIDDLE)
        << "Error on dropping " << table << " table: " << e.what() << std::endl;
      ret_value = error;
    }
    return ret_value;
  }
  ...
  int PostgresTest::environment_test() noexcept
  {
    ...
    {
      file_logger_->sstream(LEVEL_ERROR)
        << "Error on testing environment '" << database_ << "' :" << e.what();
    }
    return 1;
  }
  ...
  int PostgresTest::pool_test() noexcept
  {
    ...
    try
    {
      ...
      Commons::Postgres::ConnectionPool_var pool = env->create_connection_pool();
    ...
    {
      file_logger_->sstream(LEVEL_ERROR)
        << "Error on testing pool '" << database_ << "' :" << e.what();
      res = 1;
    }
    return res;
  }
  ...
  void PostgresTest::deactivate_env(Commons::Postgres::Environment* env) noexcept
  {
    log_("Start task");
  ...
  int PostgresTest::resultset_test(bool sync, Commons::Postgres::DATA_FORMATS format)
    noexcept
  {
    int ret_value = 0;
    AdServer::Commons::Postgres::Connection_var con;
    try
    {
      ...
      while (res->next())
      {
        ...
        if (rec_result.timestamp_value.tv_sec / 86400 != date_from_timestamp.tv_sec / 86400)
        {
          Stream::Error err;
          err << "Date in timestamp values isn't equal date: '"
            << rec_result.timestamp_value << "' != '" << date_from_timestamp << '\'';
          log_(err.str().str().c_str(), 0);
          ret_value |= 1;
        }
        ...
        if (rec.defnum_value != rec_result.defnum_value)
        {
          ...
          err << "Default decimal values isn't equal: '"
            << rec.defnum_value << "' != '" << rec_result.defnum_value << '\'';
          log_(err.str().str().c_str(), 0);
          ret_value |= 1;
        }
      }
  ...
  int PostgresTest::no_db_test() noexcept
  {
    ...
    try
    {
      ...
      log_("Getting connection");
      Commons::Postgres::Connection_var con = pool->get_connection();
      Generics::Task_var task(new ConnectionActionTask(this, env, task_runner));
      AdServer::Commons::Postgres::Statement_var stm =
        new AdServer::Commons::Postgres::Statement(sql_6);
      log_("Prepared statement");
      try
      {
        ...
        AdServer::Commons::Postgres::ResultSet_var res = con->execute_statement(stm, false);
        log_("Finish query");
        while (res->next())
        {
          count_records++;
      ...
      {
        Stream::Error err;
        err << "caught exception : " << e.what() << ", fetched " << count_records << " records";
