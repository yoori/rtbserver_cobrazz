...
namespace AdServer::UnitTests
{
  ...
  public:
    ...
    int resultset_test(bool sync, Commons::Postgres::DATA_FORMATS format)
      noexcept;
    ...
    void query_perf_cursor(AdServer::Commons::Postgres::Connection* con)
      noexcept;

    void query_perf_one_row(AdServer::Commons::Postgres::Connection* con)
      noexcept;

    void query_perf_all_row(AdServer::Commons::Postgres::Connection* con)
      noexcept;
    ...
    void deactivate_env(Commons::Postgres::Environment* pool) noexcept;
  ...
  private:
    ...
    void compary_content_(const char* array1, const char* array2, unsigned array_size)
      /*throw(Exception)*/;
    ...
    int connection_actions_(AdServer::Commons::Postgres::Connection* con, int error = 1)
      noexcept;
    ...
    template<typename T>
    unsigned long check_value_(const char* context, const T& value, const T& expected) noexcept;
  ...
  template<typename T>
  unsigned long PostgresTest::check_value_(const char* context, const T& value, const T& expected)
    noexcept
  {
    if (verbose_ >= 3)
    {
    ...
    {
      Stream::Error err;
      err << context << " value != expected : '" << value << "' != '" << expected << '\'';
