...
<xsl:template match="creative">
  ...
  <xsl:for-each select="token">
    ...
    <xsl:value-of select="external:escape-js(string(external:escape-js(string(@value))))"/>
    <xsl:value-of select="external:escape-js-unicode(string(external:escape-js(string(@value))))"/>
  </xsl:for-each>
</xsl:template>
