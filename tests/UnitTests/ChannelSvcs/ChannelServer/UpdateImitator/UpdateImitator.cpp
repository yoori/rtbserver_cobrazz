...
namespace AdServer::UnitTests
{
          ...
          AdServer::ChannelSvcs::NotConfigured)*/
  {
    ...
    for (size_t i = 0; i < result->keywords.length(); i++)
    {
      ...
      value.ctr = CorbaAlgs::pack_decimal<CampaignSvcs::CTRDecimal>(CampaignSvcs::CTRDecimal(0.12));
          ...
          AdServer::ChannelSvcs::NotConfigured)*/
  {
    ...
    data->source_id = 1;
    data->max_time = CorbaAlgs::pack_time(Generics::Time::get_time_of_day() - stamp);
  }
          ...
          AdServer::ChannelSvcs::NotConfigured)*/
  {
    ...
    for (size_t i = 0; i < count_channels; i++)
    {
          ...
          it != triggers_.end() && j < channel.words.length(); ++it, j++)
      {
        ...
        memcpy(channel.words[j].trigger.get_buffer(), it->second.data(), it->second.size());
      }
  ...
  UpdateImitator::Algorithm UpdateImitator::parse_alg_name(const std::string& name) noexcept
  {
    if (name == "random")
    {
      return ALG_RANDOM;
  ...
  const std::string UpdateImitator::control(const char* param_name, const char* param_value)
  {
    ...
    if (strcmp(param_name, "alg") == 0)
    {
      ...
      {
        res << "Invalid value '" << param_value << "' for '" <<  param_name << "'";
        setted = false;
      }
    }
    else if (strcmp(param_name, "channels") == 0)
    {
      read_number(param_value, 1UL, std::numeric_limits<size_t>::max(), count_channels_);
    }
    else if (strcmp(param_name, "triggers") == 0)
    {
      read_number(param_value, 1UL, std::numeric_limits<size_t>::max(), count_triggers_);
      generate_triggers_();
    }
    else if (strcmp(param_name, "keywords") == 0)
    {
      read_number(param_value, 1UL, std::numeric_limits<size_t>::max(), count_keywords_);
    }
    else if (strcmp(param_name, "length") == 0)
    {
      read_number(param_value, 1UL, 1024UL, trigger_length_);
      generate_triggers_();
    }
    else if (strcmp(param_name, "from") == 0)
    {
      read_number(param_value, 1UL, count_channels_, start_point_);
      count_channels_i_ = start_point_;
    }
    else if (strcmp(param_name, "step") == 0)
    {
      read_number(param_value, 1UL, count_channels_, grow_step_);
      count_channels_i_ = start_point_;
    }
    ...
    {
      res << '\'' <<  param_name << "' unknown parameter";
      setted = false;
    }

    if (setted)
    {
      res << '\'' <<  param_name << "' was setted to '" << param_value << '\'';
    }
    return res.str();
  }
  ...
  {
    return CORBA::string_dup(server_->control(param_name, param_value).c_str());
  }
...
int main(int argc, char* argv[])
{
  ...
  args.add(Generics::AppUtils::equal_name("port") || Generics::AppUtils::short_name("p"), port);
  args.add(
  ...
  args.add(
    ...
    trigger_length);
  args.add(Generics::AppUtils::equal_name("alg") || Generics::AppUtils::short_name("a"), alg);
  args.add(Generics::AppUtils::equal_name("step") || Generics::AppUtils::short_name("s"), step);
  args.add(Generics::AppUtils::equal_name("from") || Generics::AppUtils::short_name("f"), start);
  args.add(
  ...
  try
  {
    ...
    AdServer::UnitTests::ControlImpl_var proccessor =
      new AdServer::UnitTests::ControlImpl(corba_server_adapter->shutdowner(), server.in());

    corba_server_adapter->add_binding(CHANNEL_UPDATE_OBJECT_KEY, server.in());
