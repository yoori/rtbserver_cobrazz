...
namespace AdServer::UnitTests
{
  ...
  void MatchPerformanceLargeTest::parse_keywords_(ChannelSvcs::MatchWords& out)
    noexcept
  {
    ...
    /*throw(eh::Exception)*/
  {
    try
    {
      ...
      unsigned long count_soft_match = (unsigned long)
        (count_soft_in_sentence * count_soft_sentence * (match_percent_ / 100.0f));
      ...
      if (body_)
      {
        fill_matched_words_(matched_words, count_hard_match, count_soft_match);
      }
      ...
      for (size_t j = 0; j < count_hards_; j++)
      {
        ...
        {
          ChannelServerTestCommons::generate_asc_word(trigger.trigger, hard_length_);
      ...
      for (size_t j = 0; j < count_soft_; j++)
      {
        ...
        trigger.type = 'P';
        trigger.negative = false;
        if (body_ && j < count_soft_match && matched_words.size() < count_hard_match + j)
        {
          trigger.trigger = matched_words[j];
        }
        else
        {
          for (size_t k = 0; k < word_length_; k++)
          {
            ...
            ChannelServerTestCommons::generate_asc_word(word, soft_length_);
          ...
          for (size_t j = 0; j < count_soft_in_sentence; j++)
          {
            ...
            else
            {
              for (size_t k = 0; k < word_length_; k++)
              {
                ...
                ChannelServerTestCommons::generate_asc_word(match_data_, soft_length_);
              }
            }
          }
          for (size_t j = count_soft_in_sentence * word_length_; j < sentence_words_; j++)
          {
            ChannelServerTestCommons::generate_asc_word(match_data_, soft_length_);
          }
          match_data_.append(",", 1);
      ...
      if (verbose_level_)
      {
        std::cout << "Generating " << count_urls_ << " urls, "
          ...
          << " match soft words, " << count_hard_match << " match hard words." << std::endl;
      }

      if (verbose_level_ > 1)
      {
        std::cout << "Match words: " << std::endl;
        for (r_it = matched_words.rbegin(); r_it != matched_words.rend(); ++r_it)
        {
          std::cout << *r_it << std::endl;
        }
        std::cout << "Match data: " << std::endl << match_data_.c_str() << std::endl;
      }
    }
    catch(const ChannelSvcs::ChannelContainer::Exception& e)
    {
      std::cerr << "caught ChannelContainer::Exception: " << e.what() << std::endl;
      throw eh::Exception();
    }
    catch(const ChannelSvcs::TriggerParser::Exception& e)
    {
      std::cerr << "caught  TriggerParser::Exception: " << e.what() << std::endl;
      throw eh::Exception();
    }
  }
  ...
  int MatchPerformanceLargeTest::execute_() /*throw(eh::Exception)*/
  {
    ...
    {
      std::cerr << "caught eh::Exception: " << e.what() << std::endl;
      return 1;
    }
  }
    ...
    try
    {
      ...
      else
      {
        std::cerr << "Length of soft word " << word_length_
          << " more than words in sentence " << sentence_words_ << "." << std::endl;
      }
    }
    catch(const eh::Exception& e)
    {
      std::cerr << FUN << ": caught eh::Exception: " << e.what() << std::endl;
    }
    return 1;
  }
