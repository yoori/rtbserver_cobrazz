...
namespace AdServer::UnitTests
{
    ...
    /*throw(eh::Exception)*/
  {
    try
    {
      ...
      if (verbose_level_)
      {
        std::cout  << std::endl << "Generating data: " << data_size_ << '.' << std::endl;
      }
    }
  ...
  int MatchPerformanceTest::execute_() /*throw(eh::Exception)*/
  {
    try
    {
      ...
      Generics::Statistics::TimedStatSink_var stat(new Generics::Statistics::TimedStatSink);
      Generics::Statistics::TimedStatSink_var cpu_stat(new Generics::Statistics::TimedStatSink);
