...
namespace AdServer::Testing
{
  ...
  public:
    ...
    enum TestCases
    {
      ...
      TC_UPDATE = 0x10,
      TC_UIDS = 0x20,
      TC_ALL_CASES = TC_CONSTRUCTOR + TC_ADD_TRIGGER + TC_MERGE + TC_MATCH + TC_UPDATE };
    struct IdToTrigger
    {
      unsigned int id;
      const char* trigger;
  ...
  private:
    ...
    void
    log_action_(const std::string& action, unsigned int level = 1, const char* function = 0) const
      /*throw(eh::Exception)*/;
    ...
    bool compary_words_(const CompType& orig, const CompType2& res)
      noexcept;
    ...
    void gen_atom_(ChannelSvcs::MergeAtom& atom, size_t num, const char* prefix = 0)
      /*throw(eh::Exception)*/;
    void check_atom_(
    ...
    void check_uids_prepare_removing_(IdSet& channels, IdSet& channels2, size_t percent)
      noexcept;
