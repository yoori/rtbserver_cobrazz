...
namespace AdServer::Testing
{
  using namespace AdServer::ChannelSvcs;

  template<typename OUTTYPE>
  void read_number(const char* in, OUTTYPE min_value, OUTTYPE max_value, OUTTYPE &value) noexcept
  {
    OUTTYPE converted;
  ...
  int ChannelContainerTest::check_add_trigger_() noexcept
  {
    ...
    try
    {
      ...
      log_action_("Container was created");
      create_data_for_container_(10, 10 + count_triggers_, cont.get(), 0, &buffer);
      {
  ...
  int ChannelContainerTest::check_merge_() noexcept
  {
    ...
    try
    {
      ...
      create_data_for_container_(10, 10 + count_triggers_, cont.get(), "old_", &empty);
      ChannelSvcs::ChannelIdToMatchInfo_var info = new ChannelSvcs::ChannelIdToMatchInfo;
      base->merge(*cont, *info, true);
      ...
      create_data_for_container_(10, 10 + half_triggers, cont.get(), "new_");
      base->merge(*cont, *info, true);
      ...
      check_data_of_container_(10 + half_triggers, 10 + count_triggers_, buffer, FN, "old_");
      create_data_for_container_(10 + half_triggers, 10 + count_triggers_, cont.get(), "new_");
      base->merge(*cont, *info, true);
  ...
  int ChannelContainerTest::check_match_() noexcept
  {
    ...
    try
    {
      ...
      log_action_("Container was created");
      init_triggers_(triggers, sizeof(triggers)/sizeof(triggers[0]), *base);
  ...
  {//inspired by ADSC-4874
    ...
    try
    {
                ...
                it_tr != it->second->triggers.end(); it_tr++)
            {
              ...
              std::cerr << '\'' <<
                ChannelSvcs::Serialization::get_trigger(
                  it_tr->matcher->get_trigger(), trigger_text) << '\'' << std::endl;
            }
          }
          return 1;
        }
      ...
      init_triggers_(triggers2, sizeof(triggers2)/sizeof(triggers2[0]), *base, 100);
      log_action_("Get trigger lists from container");
      data.clear();
      data[11].reset();
      data[12].reset();
    ...
    /*throw(ErrorDescriptor)*/
  {
    ...
    {
      if (!channels.empty() && channels.find(it->first) == channels.end())
      ...
      if (it->first != res_it->first)
      {
        ...
        {
          err << __func__ << ": unexpected id " << res_it->first << " in result";
        }
        throw ErrorDescriptor(err);
      }
    ...
    noexcept
  {
    ...
    timer.start();
    cpu_timer.start();
    ChannelSvcs::ChannelChunk::apply_uid_map_update_(res, added, removed_uid_channels);
    ...
    /*throw(ErrorDescriptor)*/
  {
    ...
    log_action_(out.str());
    ChannelSvcs::ChannelContainerBase::ChannelMap::const_iterator it = buffer.find(start + 1);
    ...
    /*throw(eh::Exception)*/
  {
    ...
    for (size_t i = start; i < last; i++)
    {
      ...
      //m_info.channel.mark_type(Channel::CT_ACTIVE);
      {
        ...
        ostr << std::endl <<"Adding atom with id " << atom.id << " in buffer";
  ...
  void ChannelContainerTest::gen_atom_(MergeAtom& atom, size_t num, const char* prefix)
    /*throw(eh::Exception)*/
  {
    std::string pre;
    if (prefix)
    ...
    switch(num % 3)
    {
      ...
      case 2:
        ...
        gen_soft_words_((pre + "soft").c_str(), atom.soft_words, num, 3, false);
        log_action_("Generating negative hard words.", 3);
        gen_hard_words_((pre + "neghard").c_str(), atom.soft_words, num, true);
        log_action_("Generating negative soft words.", 3);
        gen_soft_words_((pre + "negsoft").c_str(), atom.soft_words, num, 3, true);
        break;
      case 1://url
    ...
    /*throw(eh::Exception)*/
  {
    for (size_t i = 0; i < count;  i++)
    {
      ...
      std::ostringstream ostr;
      ostr << (negative ? "minus" : " ") << prefix << "and" << id;
      TriggerParser::TriggerParser::parse_word(
  ...
  bool ChannelContainerTest::compary_words_(const CompType& orig, const CompType2& res)
    noexcept
  {
    for (typename CompType::const_iterator it = orig.begin(); it != orig.end(); ++it)
    {
  ...
  void ChannelContainerTest::parse_tests_(const char* tests) noexcept
  {
    ...
    while (tok.get_token(sub))
    {
      for (size_t i = 0; i < sizeof(test_names_)/sizeof(test_names_[0]); i++)
      {
        if (strncasecmp(test_names_[i].name, sub.begin(), sub.end() - sub.begin()) == 0)
        {
  ...
  void ChannelContainerTest::parse_argc_(int argc, char* argv[]) noexcept
  {
    ...
    do
    {
      opt = getopt_long(argc, argv, "vt:c:r:u:h:", long_options, &index);
      switch(opt)
      {
        case 'v':
          verbose_++;
