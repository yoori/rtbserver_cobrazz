...
REFLECT_UNIT(ChannelTriggerPerformanceTest) ("Statistics", AUTO_TEST_SLOW);
  ...
  Diffs& diffs)
{
  for (size_t i = 0; i < Count; ++i)
  {
    ...
    key.sdate(stats[i].sdate).
        ...
        channel_id(fetch_int(prefix + "/" + stats[i].channel_id + "/" + "CHANNEL_ID"));
    if (stats[i].trigger_type)
    { key.trigger_type(stats[i].trigger_type); }
    if (stats[i].channel_trigger_id)
    {
      key.channel_trigger_id(
        fetch_int(prefix + "/" + stats[i].channel_id + "/TRIGGERS/" + stats[i].channel_trigger_id));
    }
    Stat stat(key);
...
{
  const std::string description("Test 2.1. Reason of impression (channel types logging)");
...
//    sdate   colo_id       channel_id        trigger_type  channel_trigger_id  imps  clicks
  ...
  client.process_request(NSLookupRequest().tid(default_tag_).debug_time(today_));
  ...
  client.process_request(client.debug_info.selected_creatives.first().click_url);
...
//    sdate     colo_id       channel_id  trigger_type  channel_trigger_id  imps        clicks
  ...
  std::string click_url = client.debug_info.selected_creatives.first().click_url;
    ...
    FAIL_CONTEXT(AutoTest::predicate_checker(!click_url.empty()), "server must return click_url");
  ...
  client.process_request(NSLookupRequest().tid(default_tag_).debug_time(today_ + 5 * MINUTE + 1));
  ...
  client.process_request(NSLookupRequest().tid(tag).debug_time(tomorrow));
...
//    sdate     colo_id       channel_id  trigger_type  channel_trigger_id  imps        clicks
  ...
  FAIL_CONTEXT(
    AutoTest::equal_checker(fetch_string("Test#2.3/Campaign/CCID"), client.debug_info.ccid).check(),
    "checking expected display creative");
}

void ChannelTriggerPerformanceTest::channels_linked_with_text_campaign_()
{
  const std::string description("Test 2.4. Channels linked with text campaigns");
  add_descr_phrase(description);
...
//    sdate     colo_id         channel_id  trigger_type  channel_trigger_id  imps  clicks
  ...
  std::string click_url1 = client.debug_info.selected_creatives.first().click_url;
  std::string click_url2 = client.debug_info.selected_creatives.last().click_url;
    ...
    FAIL_CONTEXT(
      AutoTest::predicate_checker(!click_url1.empty() && !click_url2.empty()),
...
//    sdate     colo_id         channel_id  trigger_type  channel_trigger_id  imps  clicks
  ...
  FAIL_CONTEXT(
    AutoTest::equal_checker(fetch_string("Test#2.5/Campaign/CCID"), client.debug_info.ccid).check(),
    "checking expected display creative");
}

void ChannelTriggerPerformanceTest::trigger_performance_for_temp_users_()
{
  const std::string description("Test 2.6. Trigger performance for temporary users");
  add_descr_phrase(description);
...
//    sdate     colo_id         channel_id  trigger_type  channel_trigger_id  imps  clicks
  ...
  add_descr_phrase("Test 2.6.1. Regular merging with temporary user");
  {
    ...
    client.merge(temp_client, NSLookupRequest().tid(default_tag_).debug_time(today_));
...
//    sdate     colo_id         channel_id  trigger_type  channel_trigger_id  imps        clicks
  ...
  add_stats("Test#2.7", IMTERMEDIATE_STATS, intermediate_stats, intermediate_diffs);
  ...
  add_descr_phrase("Test 2.7.1. Asynchronous logging for persistent users");
  {
    ...
    client.process_request(NSLookupRequest().tid(default_tag_). debug_time(today_ + 1));
    ...
    FAIL_CONTEXT(AutoTest::wait_checker(
        AutoTest::stats_diff_checker(conn_, intermediate_diffs, intermediate_stats)).check(),
...
//    sdate     colo_id         channel_id  trigger_type  channel_trigger_id  imps        clicks
  ...
  add_descr_phrase("Test 2.7.2. Asynchronous logging " "on merging with temporary users");
  {
    TemporaryAdClient temp_client(TemporaryAdClient::create_user(this));
    ...
    client.merge(temp_client, NSLookupRequest().tid(default_tag_). debug_time(today_ + 1));
