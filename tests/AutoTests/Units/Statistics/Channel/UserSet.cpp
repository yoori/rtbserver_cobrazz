...
namespace AutoTest::Statistics
{

  // class UserSet
  void
  UserSet::initialize(BaseUnit* test, unsigned long size)
  {
  ...
  UserSet::process_request(
    ...
    /*throw(eh::Exception)*/
  {
    unsigned long stop_index_ = stop_index? std::min(stop_index, users_.size()): users_.size();
  ...
  void
  UserSet::process_request_i(unsigned long index, const BaseRequest& request)
    /*throw(eh::Exception)*/
  {
    check_index(index);
