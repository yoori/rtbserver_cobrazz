...
REFLECT_UNIT(PublisherInventoryRequestTest) ("CreativeSelection", AUTO_TEST_FAST);
...
PublisherInventoryRequestTest::run_test()
{
  ...
  {
    request.tid = tag_inv;
    client.process_request(request, "tag in inventory estimation mode and tag.inv=0");
  }
  // tag in advertising mode and no tag.inv => show ad
  {
    request.tid = tag_no_inv;
    client.process_request(request, "tag in advertising mode and no tag.inv");
    FAIL_CONTEXT(
  ...
  {
    request.tag_inv = 1;
    client.process_request(request, "tag in advertising mode and tag.inv=1");
    FAIL_CONTEXT(
  ...
  {
    request.tid = tag_inv;
    client.process_request(request, "tag in inventory mode and tag.inv=1");
    FAIL_CONTEXT(
      AutoTest::equal_checker(204, client.req_status()).check(),
      "Server must return 'no content' status");
  }
