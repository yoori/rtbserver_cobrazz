...
REFLECT_UNIT(NonRecursiveExistingToken) ("CreativeInstantiation", AUTO_TEST_FAST);
...
NonRecursiveExistingToken::run_test()
{
  ...
  add_descr_phrase("Ensure that Creative#2 (cc_id=" + fetch_string("CC2") + ") status 'C'");

  FAIL_CONTEXT(
    CreativeChecker(this, fetch_int("CC2"), CreativeChecker::Expected(). status("C")).check(),
    "Check creative#2 status");

  add_descr_phrase("Ensure that Creative#1 (cc_id=" + fetch_string("CC1") + ") status 'A'");
  FAIL_CONTEXT(
    CreativeChecker(this, fetch_int("CC1"), CreativeChecker::Expected(). status("A")).check(),
    "Check creative#1 status");

  add_descr_phrase("Check that Creative with high " "weight doesn't match");
