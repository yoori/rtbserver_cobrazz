...
REFLECT_UNIT(OutOfScopeToken) ("CreativeInstantiation", AUTO_TEST_SLOW);
...
OutOfScopeToken::tear_down()
{
  ...
  option.select(fetch_int("CC"), fetch_int("OPTION"));
...
OutOfScopeToken::run()
{
  ...
  add_descr_phrase("Test 2.2. Dynamic test - changing Text " "Creative with version updating");

  CreativeOptionValue option(pq_conn_);
  FAIL_CONTEXT(
    AutoTest::predicate_checker(option.select(fetch_int("CREATIVE"), fetch_int("OPTION"))),
  ...
  FAIL_CONTEXT(
    AutoTest::wait_checker(
      CreativeChecker(this, fetch_int("CC"), CreativeChecker::Expected(). status("A"))).check(),
...
void OutOfScopeToken::set_creative_option()
{
  ...
  FAIL_CONTEXT(
    AutoTest::wait_checker(
      CreativeChecker(this, fetch_int("CC"), CreativeChecker::Expected(). status("C"))).check(),
    "Check creative status");
}
