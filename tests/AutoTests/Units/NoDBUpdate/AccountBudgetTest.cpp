...
REFLECT_UNIT(AccountBudgetTest) ("NoDBUpdate", AUTO_TEST_QUIET);
...
AccountBudgetTest::run_test()
{
  ...
  unsigned long max_repeat_count =
    (fetch_int("ZEROBALANCE_ACCOUNT_CREDIT") * 1000 / fetch_int("ZEROBALANCE_CPM")) + 1;
  ...
  for (unsigned long i = 0; i < max_repeat_count; ++i)
  {
    ...
    {
        FAIL_CONTEXT(
          AutoTest::equal_checker(fetch_string("ZEROBALANCE_CCID"), client.debug_info.ccid).check(),
          "must return zero balance ccid");
    }
  }

  FAIL_CONTEXT(AutoTest::wait_checker(
    CampaignChecker(this, ccgid, CampaignChecker::Expected().eval_status("I"))).check(),
