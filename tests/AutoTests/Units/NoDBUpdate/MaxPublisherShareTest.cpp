...
REFLECT_UNIT(MaxPublisherShareTest) ("NoDBUpdate", AUTO_TEST_SLOW);
...
MaxPublisherShareTest::run()
{
  ...
  AutoTest::Time new_day((AutoTest::Time() + Generics::Time::ONE_DAY).get_gm_time().get_date());
...
MaxPublisherShareTest::share_expiring_()
{
  ...
  FAIL_CONTEXT(
    SelectedCreativeChecker(client, request, fetch_int("EXPIRE/CC")).check(),
  ...
  add_wait_checker(
    ...
    CampaignChecker(
      ...
      CampaignChecker::Expected().
        status("A").
        eval_status("A").
        exclude_pub_accounts(fetch_string("EXPIRE/PUBLISHER/1"))));

 add_checker(description + " Check ccid#2", SelectedCreativeChecker(client, request, 0));
}
...
MaxPublisherShareTest::increase_budget_()
{
  ...
  ORM::ORMRestorer<ORM::PQ::CampaignCreativeGroup>* ccg =
    create<ORM::PQ::CampaignCreativeGroup>(fetch_int("EXPIRE/CCG"));
  ccg->budget = 18;
  FAIL_CONTEXT(AutoTest::predicate_checker(ccg->update()), description + " Update budget");
...
void
MaxPublisherShareTest::three_sites_part_1_(const AutoTest::Time& new_day)
{
  std::string description("MPS For Three Site and Delivery " "Pacing: Daily Budget Cap, part#1.");
  ...
  add_wait_checker(
    ...
    AutoTest::and_checker(
      AutoTest::throw_checker(AutoTest::TimeLessChecker(new_day)),
      CampaignChecker(
        this,
        fetch_int("3SITES/CCG"),
        CampaignChecker::Expected().
          status("A").
          eval_status("A").
          exclude_pub_accounts(fetch_string("3SITES/PUBLISHER/1")))));

}

void
MaxPublisherShareTest::three_sites_part_2_(const AutoTest::Time& new_day)
{
  std::string description("MPS For Three Site and Delivery " "Pacing: Daily Budget Cap, part#2.");
    ...
    AutoTest::and_checker(
      ...
      CampaignChecker(
        ...
        CampaignChecker::Expected().
          status("A").
          eval_status("A").
          exclude_pub_accounts(map_objects("3SITES/PUBLISHER/1,3SITES/PUBLISHER/2")))));
...
MaxPublisherShareTest::text_static_()
{
  ...
  fetch_objects(
    std::inserter(expected_ccids, expected_ccids.begin()),
  ...
  add_wait_checker(
    ...
    CampaignChecker(
      ...
      CampaignChecker::Expected().
        status("A").
        eval_status("A").
         exclude_pub_accounts(fetch_string("TEXT/PUBLISHER/1"))));
...
MaxPublisherShareTest::text_dynamic_()
{
  ...
  ORM::ORMRestorer<ORM::PQ::CampaignCreativeGroup>* ccg =
    create<ORM::PQ::CampaignCreativeGroup>(fetch_int("TEXT/CCG/1"));
  ccg->budget = 0.1;
  FAIL_CONTEXT(AutoTest::predicate_checker(ccg->update()), description + " Update budget");

  ORM::ORMRestorer<ORM::PQ::Campaign>* campaign =
    create<ORM::PQ::Campaign>(fetch_int("TEXT/CAMPAIGN/1"));
...
void
MaxPublisherShareTest::text_daily_part_1_(const AutoTest::Time& new_day)
{
  std::string description("Text Ad Group with non " "installed budget (=0), part#1.");
  add_descr_phrase("Start part#1 of the case");
    ...
    AutoTest::and_checker(
      ...
      CampaignChecker(
        ...
        CampaignChecker::Expected().
          status("A").
          eval_status("A").
          exclude_pub_accounts(fetch_string("TEXTDAILY/PUBLISHER/1")))));

}

void
MaxPublisherShareTest::text_daily_part_2_(const AutoTest::Time& new_day)
{
  std::string description("Text Ad Group with non " "installed budget (=0), part#2.");
  add_descr_phrase("Start part#2 of the case");
    ...
    AutoTest::and_checker(
     ...
     CampaignChecker(
       ...
       CampaignChecker::Expected().
         status("A").
         eval_status("A").
         exclude_pub_accounts(map_objects("TEXTDAILY/PUBLISHER/1,TEXTDAILY/PUBLISHER/2")))));
...
void
MaxPublisherShareTest::text_daily_part_3_(const AutoTest::Time& new_day)
{
  std::string description("Text Ad Group with non " "installed budget (=0), part#3.");
  add_descr_phrase("Start part#3 of the case");

  ORM::ORMRestorer<ORM::PQ::Campaign>* campaign =
    create<ORM::PQ::Campaign>(fetch_int("TEXTDAILY/CAMPAIGN/1"));
