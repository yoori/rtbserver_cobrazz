...
namespace AutoTest::UserProfilesExchange
{
      ...
      /*throw(eh::Exception)*/
  {
    for (unsigned int my_time = 0; my_time < wait_time_; my_time += SLEEP_TIME)
    {
      ...
      FAIL_CONTEXT(
        AutoTest::equal_checker(colo_, client_.debug_info.colo_id).check(),
        "must receive expected colo");

      if (AutoTest::entry_in_seq(expected_, client_.debug_info.history_channels))
      {
        return true;
      }
