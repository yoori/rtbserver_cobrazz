...
#define NUMERICOID		1700
    ...
    public:
      ...
      void dump(unsigned char* mem, size_t num)
      {
        int i;
    ...
    // Utils
    double
    getFloat(Oid oid, int len, char* ptr, bool check_other = true);

    bool
    getBool(Oid oid, int len, char* ptr);

    long long
    getInt(Oid oid, int len, char* ptr, bool check_other = true)
    {
    ...
    double getFloat(Oid oid, int len, char* ptr, bool check_other)
    {
      String::SubString str(ptr, len);
      Stream::Parser strm(str);
      switch(oid)
    ...
    bool getBool(Oid oid, int len, char* ptr)
    {
      if (oid != bool_oid)
      {
  ...
  static void
  noticeProcessor(void *, const char * msg)
  {
  ...
  Conn::open_()
  {
    connection_ = PQsetdbLogin(host_.c_str(), 0, 0, 0, db_.c_str(), user_.c_str(), pswd_.c_str());
  ...
  QueryTuple
  Conn::query(const std::string& statm)
  {
      ...
      found != std::string::npos; found = statement.find(':', found + 1))
    {
      ...
      while (std::isalnum(statement[found + name_sz + 1])) name_sz++;

      if (name_sz)
      {
        statement.replace(found, name_sz + 1, "$" + strof(++param_count));
      }
    }
    AutoTest::Logger::thlog().debug_trace("create query: " + statement);
    return QueryTuple(connection_, statement);
  }

  AutoTest::DBC::IQuery*
  Conn::get_query(const std::string& statement)
  {
  ...
  Query::Query(const QueryTuple& tuple) :
  ...
  void
  Query::throw_exception(const std::string& message)
  {
    Stream::Error error;
    error << "exception in:"
          << std::endl << "[" << statement_ << "]" << std::endl << "description: " << message;
  ...
  Query::ask()
  {
    ...
    if (pq_status != PGRES_TUPLES_OK && pq_status != PGRES_COMMAND_OK)
    {
  ...
  Result::Result(const ResultTuple& tuple) :
  ...
  void
  Result::get(Generics::ExtendedTime& value)
  {
    Oid oid = PQftype(result_set_, value_index_);
    char* ptr = PQgetvalue(result_set_, row_index_, value_index_++);
    struct tm tm;
  ...
  void
  Result::get(Generics::Time& value)
  {
    Oid oid = PQftype(result_set_, value_index_);
    char* ptr = PQgetvalue(result_set_, row_index_, value_index_++);
    struct tm tm;
  ...
  void
  Result::get(AutoTest::DBC::TextField&)
  {
  ...
  void
  BasicQueryStream::set_string_(Oid type_oid, unsigned long type_size, const std::string& str_value)
  {
  ...
  void
  BasicQueryStream::set_as_string_(Oid type_oid, unsigned long type_size, const ValueType& val)
  {
    set_string_(type_oid, type_size, strof(val));
  }

  void
  BasicQueryStream::set_null_(Oid type_oid)
  {
    oids_.push_back(type_oid);
  ...
  void
  BasicQueryStream::null(char)
  {
    set_null_(char_oid);
  }

  void
  BasicQueryStream::null(int)
  {
    set_null_(int4_oid);
  }

  void
  BasicQueryStream::null(bool)
  {
    set_null_(bool_oid);
  }

  void
  BasicQueryStream::null(unsigned int)
  {
    set_null_(int4_oid);
  }

  void
  BasicQueryStream::null(float)
  {
    set_null_(float4_oid);
  }

  void
  BasicQueryStream::null(double)
  {
    set_null_(numeric_oid);
  }

  void
  BasicQueryStream::null(const std::string&)
  {
    set_null_(text_oid);
  }
  ...
  void
  BasicQueryStream::null(const Generics::ExtendedTime&)
  {
    set_null_(timestamp_oid);
  }

  void
  BasicQueryStream::null(const Generics::Time&)
  {
    set_null_(date_oid);
  }

  void
  BasicQueryStream::null(const AutoTest::DBC::TextField&)
  {
    throw Exception("Text operation not supported");
  }

  void
  BasicQueryStream::set(char value)
  {
    set_as_string_(char_oid, 1, value);
  }

  void
  BasicQueryStream::set(int value)
  {
    set_as_string_(int4_oid, 4, value);
  }

  void
  BasicQueryStream::set(bool value)
  {
    set_as_string_(bool_oid, 1, value);
  }

  void
  BasicQueryStream::set(unsigned int value)
  {
    set_as_string_(int4_oid, 4, value);
  }

  void
  BasicQueryStream::set(float value)
  {
    set_as_string_(float4_oid, 4, value);
  }

  void
  BasicQueryStream::set(double value)
  {
    set_as_string_(numeric_oid, 8, value);
  }

  void
  BasicQueryStream::set(const std::string&  value)
  {
    set_string_(text_oid, value.size() + 1, value);
  }

  void
  BasicQueryStream::set(const Generics::ExtendedTime& value)
  {
  ...
  void
  BasicQueryStream::set(const Generics::Time& value)
  {
  ...
  void
  BasicQueryStream::set(const AutoTest::DBC::TextField&)
  {
    throw Exception("Text operation not supported");
  }
  ...
  {
    return
      PQgetisnull(result_set_, row_index_, value_index_) == 1;
  }
}
