...
namespace AutoTest::ORM
{
  ...
  ORMString::ORMString(const std::string& value) :
    Base(value)
  { }

  ORMString::ORMString(const ORMString& from) :
    Base(from)
  { }
  ...
  ORMText::ORMText(const std::string& value) :
    Base()
  {
    _value.set(value);
  }
  ...
  ORMDate::ORMDate() :
    Base(Generics::Time::ZERO.get_gm_time())
  {
    _changed = false;
    _null = true;
  }

  ORMDate::ORMDate(const value_type& value) :
    Base(value)
  {
  ...
  ORMTimestamp::ORMTimestamp(const value_type& value) :
    Base(value)
  {
    _null = _value == Generics::Time::ZERO;
  }
  ...
  IORMValue&
  ORMObjectMember::value(void* object) const
  {
  ...
  Restorer::Restorer(EntityType entity_type) :
  ...
  bool
  SerializeQueryManager_::execute(AutoTest::DBC::IConn& connection, AutoTest::DBC::IQuery& query)
  {
