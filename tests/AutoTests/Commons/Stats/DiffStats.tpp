...
namespace AutoTest::ORM
{
  ...
  DiffStats<T, Count>::DiffStats (const char* description) :
    Base(description)
  {}

  template<typename T, int Count>
  void  DiffStats<T, Count>::print_diff(std::ostream& out, const stats_diff_type& diff)
  {
  ...
  template<typename T, int Count>
  void
  DiffStats<T, Count>::print_diff(std::ostream& out, const Diffs& values)
  {
    ...
    const DiffStats<T, Count>& real) const
  {
    ...
    {
      diff_out << "  " << Base::field_names[i] << " = " << Base::values[i] << " + ";
      Base::print_diff(diff_out, diff[i]);
      diff_out << " -> " << real[i] << ';';
      if (diff[i] != any_stats_diff && !equal(diff[i], real[i], Base::values[i]))
      {
        not_expected = true;
        diff_out << " *** ";
      }
