...
namespace AutoTest::ORM
{
    ...
    public DiffStats<ColoIdBasedStats, 54>
  {
    ...
    public:
      ...
      ColoIdBasedStats (const int& colo_id);
      ColoIdBasedStats ();
      Key& key ();
      Key& key (const Key& value);
      Key& key (const int& colo_id);
...
namespace AutoTest::ORM
{
  ...
  inline
  ColoIdBasedStats::ColoIdBasedStats (const int& colo_id)
    :Base("ColoIdBasedStats")
  {
    key_ = Key (colo_id);
  }
  ...
  inline
  ColoIdBasedStats::Key::Key (const int& colo_id)
    :colo_id_(colo_id)
  {
    colo_id_used_ = true;
    colo_id_null_ = false;
  ...
  inline
  ColoIdBasedStats::Key&
  ColoIdBasedStats::key (const int& colo_id)
  {
    key_ = Key (colo_id);
    return key_;
  }
