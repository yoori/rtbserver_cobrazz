...
namespace AutoTest::ORM
{
  ...
  update_channel_display_status(StatsDB::IConn& conn, int channel_id)
  {
...

    {
      ...
      Channel_var channel = new ORMChannel(conn, channel_id);
      ...
      result &= SerializeQueryManager::instance().execute(conn, query);
    }
  ...
  bool
  update_display_status(BaseUnit* test, const char* object_name, int id)
  {
    ...
    return SerializeQueryManager::instance().execute(conn, query);
  }
  ...
  bool
  Selectable::select(StatsDB::IConn& connection, bool initial)
  {
    initial_ = initial;
  ...
  bool
  OneRowObject::ask(StatsDB::Query& query)
  {
