...
namespace AutoTest::ORM
{
  ...
  template<typename T>
  bool clear_stats(StatsDB::IConn& connection, const char* column_name, const T& column_value);
   ...
   */
  bool
  update_channel_display_status(StatsDB::IConn& conn, int channel_id);
   ...
   */
  bool
  update_display_status(BaseUnit* test, const char* object_name, int account_id);
  ...
  int check_difference(const Object1& real, const Object2& initial, const stats_diff_type& diff)
  {
    return check_difference(beginof(real), endof(real), beginof(initial), endof(initial), diff);
  }
   ...
   */
  template<typename Object1, typename Object2>
  int check_difference(const Object1& real, const Object2& initial, int diff)
  {
    return check_difference(beginof(real), endof(real), beginof(initial), endof(initial), diff);
  }
  ...
  int check_difference(const Object1& real, const Object2& initial, const Diffs& diffs)
  {
   ...
   */
  template<typename Object1, typename Object2, typename Diffs>
  int check_each_difference(const Object1& real, const Object2& initial, const Diffs& diffs)
  {
  ...
  public:
     ...
     */
    bool
    select(StatsDB::IConn& connection, bool initial = true);
  ...
  public:
    ...
    static void print_diff(std::ostream& out, const stats_diff_type& value)
    {
    ...
    template<class Diff>
    static void print_diff(std::ostream& out, const Diff (&values)[Count])
    {
    ...
    void print_diff  (std::ostream& out, const Diff& diff, const BasicStats<T, Count>& real) const
    {
