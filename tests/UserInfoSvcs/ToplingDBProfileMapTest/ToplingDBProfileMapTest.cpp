...
namespace
{
  ...
  double
  avg_batch_size(
    const AdServer::ProfilingCommons::ProfileMap<std::string>::Stats& stats)
  {
    const auto logical_operations =
      stats.logical_read_operations + stats.logical_write_operations;
  ...
  {
    std::cerr
      << "Usage: ToplingDBProfileMapTest [OPTIONS]\n"
      << "Options:\n"
      << "  --path <path>       ToplingDB path (default: /tmp/ToplingDBProfileMapTest.<pid>)\n"
...
main(int argc, char** argv)
{
  try
  {
    ...
    using ProfileMap = AdServer::ProfilingCommons::ToplingDBProfileMap<std::string>;

    StringOption opt_path(
      std::string("/tmp/ToplingDBProfileMapTest.") + std::to_string(::getpid()));
    ...
    std::thread reporter_thread([&]() {
      ...
      while(done_count.load(std::memory_order_relaxed) < *opt_count)
      {
        ...
        std::cout << std::put_time(&tm, "%T") <<
          ...
          ", max_latency=" <<
            latency_max_us.load(std::memory_order_relaxed) << "us" <<
          std::endl;
      }
    });
