...
namespace AdServer::Predictor
{
    ...
    const AdServer::CampaignSvcs::CTRGenerator::CalculateParams& calc_params)
  {
    ...
    {
      const unsigned long index = hash_it->first >> index_shifter_;
      ordered_hashes.emplace(index + 1, hash_it->second);
    }
  ...
  // OutputLibSvmSet impl
  OutputLibSvmSet::~OutputLibSvmSet() noexcept
  {
    file_dict_stream_.close();
    file_svm_stream_.close();
  ...
  void OutputLibSvmSet::write_record(
    ...
    const AdServer::CampaignSvcs::CTRGenerator::CalculateParams& calc_params)
  {
    output_libsvm_stream_->write_record(label, calc_params);
  }

  // OutputLibSvmListWrapper impl
  OutputLibSvmListWrapper::OutputLibSvmListWrapper(
    const OutputLibSvmList& svm_list)
    : svm_list_(svm_list)
  {}

  void OutputLibSvmListWrapper::write_record(
    const std::string& label,
    const AdServer::CampaignSvcs::CTRGenerator::CalculateParams& calc_params)
  {
    for (auto& svm_stream : svm_list_)
    {
      svm_stream->write_record(label, calc_params);
    }
  }
}
