      ...
      OutputLibSvmStreamList& lib_svms)
    {
      ...
      if (file_lock.try_lock())
      {
        ...
        unsigned long label_index =
          parse_header_(stream, feature_columns);
        ...
        while(!stream.eof())
        {
          ...
          for (OutputLibSvmStreamList::iterator it = lib_svms.begin();
               it != lib_svms.end(); ++it)
          {
            (*it)->write_record(label, calc_params);
          }
        }
        return true;
      }
