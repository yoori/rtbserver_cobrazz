...
void testSuit(const std::string& directory)
{
  ...
  if (!Utils::ExistDirectory(directory))
  {
    Stream::Error ostr;
  ...
  Test test1(
          ...
          number_files);
  test1.addProcessor(std::move(aggregator_processor));
  test1.addProcessor(std::move(reaggregator_processor));
  test1.run();
  ...
  Test test2(
          ...
          number_record_per_date,
          number_files);
  test2.addProcessor(std::move(aggregator_multy_processor));
  test2.addProcessor(std::move(reaggregator_multy_processor));
  test2.run();
}
...
bool Test::runHelper()
{
  ...
  if (!Utils::ExistDirectory(directory_))
    throw Exception("Not existing directory=" + directory_);

  auto files = Utils::GetDirectoryFiles(directory_);
  ...
  files = Utils::GetDirectoryFiles(directory_, prefix);
  if (files.empty())
  ...
  {
    copyFile(path, stream.str() + std::to_string(i));
  }
  logger_->info(
  ...
  const std::string prefix_inner(LogInnerTraits::B::log_base_name());
  files = Utils::GetDirectoryFiles(directory_, prefix_inner);
