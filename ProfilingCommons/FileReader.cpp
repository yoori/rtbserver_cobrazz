...
namespace AdServer::ProfilingCommons
{
      ...
      mem_buf_pos_(0)
  {
    ...
    {
      eh::throw_errno_exception<Exception>("Can't do fstat64");
    }
      ...
      mem_buf_pos_(0)
  {
    ...
    if (fd_ < 0)
    {
      eh::throw_errno_exception<Exception>("Can't open file '", file_name, "'");
    }
    ...
    /*throw(Exception)*/
  {
    ...
    {
      unsigned long filled_size = std::min(mem_buf_->membuf().size() - mem_buf_pos_, cur_read_size);
  ...
  FileReader::eof()
    const noexcept
  {
    return fd_pos_ == file_size_ && mem_buf_pos_ == mem_buf_->membuf().size();
  }
    ...
    /*throw(Exception)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << "FileReader::read_(): error on file reading: " << ex.what();
