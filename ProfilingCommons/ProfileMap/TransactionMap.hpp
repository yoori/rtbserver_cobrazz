...
namespace AdServer::ProfilingCommons
{
  ...
  public:
    TransactionMap(unsigned long max_waiters = 0, unsigned long portions = 1024) noexcept;
...
namespace AdServer::ProfilingCommons
{
  ...
  TransactionMap<KeyType, TransactionImplType>::
  get_transaction(const KeyType& key, bool check_max_waiters, const TransactionArgType& arg)
    /*throw(MaxWaitersReached, Exception)*/
  {
    ...
    try
    {
      ...
      {
        SyncPolicy::WriteGuard lock(portion->open_transaction_map_lock);
        typename OpenedTransactionMap::iterator it = portion->open_transaction_map.find(key);
        if (it == portion->open_transaction_map.end())  // not exists
        {
          holder = new TransactionHolder(*this, key);
          Transaction_var transac(create_transaction_impl_(holder, key, arg));
        ...
        if (check_max_waiters && max_waiters_ != 0 && lock_count >= max_waiters_)
        {
  ...
  AdServer::Commons::StartableAwaitable<
    typename TransactionMap<KeyType, TransactionImplType>::Transaction_var>
  TransactionMap<KeyType, TransactionImplType>::
  co_get_transaction(const KeyType& key, bool check_max_waiters, const TransactionArgType& arg)
  {
  ...
  template <typename KeyType, typename TransactionImplType>
  void
  TransactionMap<KeyType, TransactionImplType>::close_i_(Portion* portion, const KeyType& key)
    noexcept
  {
    typename OpenedTransactionMap::iterator it = portion->open_transaction_map.find(key);
    if (it != portion->open_transaction_map.end())
