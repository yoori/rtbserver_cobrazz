...
namespace
{
    ...
    ChannelToChTrId& channel_to_triggers)
  {
    ...
    {
      channel_to_triggers[channel_seq[i].id].push_back(channel_seq[i].trigger_channel_id);
    }
  }
...
namespace AdServer::ChannelSearchSvcs
{
      ...
      channel_matcher_(new ChannelMatcher())
  {
    ...
    campaign_servers_ = resolve_campaign_servers_(corba_client_adapter_, config);

    resolve_channel_session_(config, callback);

    task_runner_->enqueue_task(Generics::Task_var(new UpdateExpressionChannelsTask(this, 0)));
  }
  ...
  ChannelSearchServiceImpl::resolve_channel_session_(
    ...
    /*throw(Exception)*/
  {
    static const char* FUN = "ChannelSearchServiceImpl::resolve_channel_session_()";
    try
    {
      AdServer::Grpc::BatchingOptions batching_options;
      if (config.BatchingOptions().present())
      {
        batching_options = Config::read_xsd_grpc_options(*config.BatchingOptions());
      }
    ...
    noexcept
  {
    ...
    try
    {
      for (;;)
      {
        ...
        try
        {
          ...
          for (unsigned long portion = 0; portion < PORTIONS_NUMBER; ++portion)
          {
            ...
            for (CORBA::ULong ch_i = 0; ch_i < ccg_links.length(); ++ch_i)
            {
              ...
              CorbaAlgs::convert_sequence(ccg_links[ch_i].ccgs, channel_traits.ccg_ids);
            }
          }
        ...
        catch(const AdServer::CampaignSvcs::CampaignServer::ImplementationException& ex)
        {
          ...
          logger_->log(ostr.str(), Logging::Logger::ERROR, ASPECT, "ADS-IMPL-147");
        }
        catch(const CORBA::SystemException& e)
        {
          ...
          campaign_server.release_bad(ostr.str());
          logger_->log(ostr.str(), Logging::Logger::ERROR, ASPECT, "ADS-ICON-12");
        }
      }
    }
    ...
    try
    {
      Generics::Goal_var task(new UpdateExpressionChannelsTask(this, task_runner_));
      ...
      AdServer::ChannelSearchSvcs::ChannelSearch::ImplementationException)*/
  {
    ...
    catch (const eh::Exception& ex)
    {
      ...
      CORBACommons::throw_desc<AdServer::
        ChannelSearchSvcs::ChannelSearch::ImplementationException>(ostr.str());
    }
  }
    ...
    /*throw(AdServer::ChannelSearchSvcs::ChannelSearch::ImplementationException)*/
  {
    ...
    try
    {
      ChannelSearchResultSeq_var result = new AdServer::ChannelSearchSvcs::ChannelSearchResultSeq;
      ...
      {
        Stream::Error ostr;
        ostr << FUN << ": Caught ChannelSearch::ImplementationException" << ex.description;

        CORBACommons::throw_desc<AdServer::
          ChannelSearchSvcs::ChannelSearch::ImplementationException>(ostr.str());
      }

      fill_channels(match_result->matched_channels.page_channels, simple_channels);
      fill_channels(match_result->matched_channels.search_channels, simple_channels);
      fill_channels(match_result->matched_channels.url_channels, simple_channels);
      fill_channels(match_result->matched_channels.url_keyword_channels, simple_channels);
      ...
      if (logger_->log_level() >= Logging::Logger::TRACE)
      {
        ...
        trace_ss << std::endl << "Matching for phrase = '" << phrase << "'" << std::endl;
        ...
        trace_ss << "Result channels(" << result_ref.length() << "): " << std::endl;
    ...
    /*throw(AdServer::ChannelSearchSvcs::ChannelSearch::ImplementationException)*/
  {
    ...
    try
    {
      ...
      {
        Stream::Error ostr;
        ostr << FUN << ": Caught ChannelSearch::ImplementationException" << ex;
      ...
      fill_channel_triggers(match_result->matched_channels.page_channels, ch_to_tr_id);
      fill_channel_triggers(match_result->matched_channels.search_channels, ch_to_tr_id);
      fill_channel_triggers(match_result->matched_channels.url_channels, ch_to_tr_id);
      fill_channel_triggers(match_result->matched_channels.url_keyword_channels, ch_to_tr_id);
           ...
           ch_it != result_expression_channels.end(); ++ch_it)
      {
        ...
        for (ChannelIdSet::const_iterator ms_ch_it = ch_it->second.matched_simple_channels.begin();
            ms_ch_it != ch_it->second.matched_simple_channels.end(); ++ms_ch_it)
        {
          ChannelToChTrId::const_iterator ch_to_tr_it = ch_to_tr_id.find(*ms_ch_it);
...

      {
        ...
        for (ChannelMatchMap::const_iterator ch_match_it = channel_matches.begin();
            ch_match_it != channel_matches.end() && ch_match_i < channels_count;
            ++ch_match_it)
        {
          result->channels[ch_match_i].channel_id = ch_match_it->first;

          fill_trigger_seq(result->channels[ch_match_i].triggers, ch_match_it->second.triggers);
      ...
      if (logger_->log_level() >= Logging::Logger::TRACE)
      {
        ...
        trace_ss << std::endl <<
          ...
          "Trigger matched channels(" << simple_channels.size() << "):" << std::endl << "  ";
    ...
    /*throw(AdServer::ChannelSearchSvcs::ChannelSearch::ImplementationException)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << FUN << ": Can't convert wchar string to utf8. Caught eh::Exception: " << ex.what();
    ...
    /*throw(AdServer::ChannelSearchSvcs::ChannelSearch::ImplementationException)*/
  {
    ...
    try
    {
      ...
      MatchInfo_var match_result = match(utf8_url.c_str(), utf8_phrase.c_str(), channels_count);
