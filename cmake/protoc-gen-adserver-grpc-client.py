...
def generate_hpp(file_desc, namespace):
  lines = [
    ...
    "#include <Commons/Grpc/GrpcUnaryAwaiter.hpp>",
  ...
  for service in file_desc.service:
    ...
    for method in methods:
      ...
      lines.extend([
        ...
        "    };",
        "",
        "    using {} = AdServer::Grpc::GrpcUnaryAwaiter<".format(aw_name),
        "      {},".format(async_client),
        "      {},".format(request_type),
        ...
        "",
        "    {}".format(aw_name),
        "    co_{}(const {}& request);".format(method.name, request_type),
        "",
      ])
    if methods:
      lines.pop()
  ...
  for service in file_desc.service:
    ...
    for method in unary_methods(service):
      ...
      lines.extend([
        ...
        "  {}::co_{}(const {}& request)".format(
          coro_client, method.name, request_type),
        "  {",
        "    return {}(".format(aw_name),
        "      client_,",
        "      executor_pool_,",
        "      request,",
        "      &{}::{},".format(async_client, method.name),
        '      "{}");'.format(default_response_error),
        "  }",
        "",
      ])
      lines.extend([
