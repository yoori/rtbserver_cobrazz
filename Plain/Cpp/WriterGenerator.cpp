...
namespace Cpp
{
  ...
  {
    FieldBuffersImplOps(std::ostream& out, const char* offset, const char* class_name)
      ...
      const char*) const
    {
      out_ << offset_ << "const unsigned char " <<
        class_name_ << "::DEFAULT_FIXED_BUF_" << fixed_buf_i << "[" << fixed_buf_size << "] = { ";
  ...
  {
    FieldBuffersSwapImplOps(std::ostream& out, const char* offset)
      : out_(out), offset_(offset)
    {}
      ...
      const char*) const
    {
      out_ << offset_ << "  ::memcpy(swap_buf_, "
        ...
        offset_ << "  ::memcpy(" <<
          FIXED_BUFFER_PREFIX << fixed_buf_i << "_, " <<
          "swap_buf_, " << fixed_buf_size << ");" << std::endl;
    }
  ...
  {
    FieldBuffersCopyImplOps(std::ostream& out, const char* offset)
      : out_(out), offset_(offset)
    {}
  ...
  struct FieldBuffersMaxOps: public OnlyFixedSumOps
  {
    ...
    void process_fixed_sum(unsigned long, unsigned long fixed_buf_size, const char*) const
    {
  ...
  struct FieldDeclOps
  {
    FieldDeclOps(
      ...
      : out_(out),
        offset_(offset),
        field_writers_(ReferenceCounting::add_ref(field_writers)),
    ...
    void process_non_fixed_or_complex(const Declaration::StructDescriptor::PosedField* field) const
    {
      if (field_writers_.in() &&
      ...
      {
        out_ << offset_ << field_type_op_(field) << " " << field->name() << "_;" << std::endl;
      }
    }
  ...
  struct FieldInitDefaultImplOps
  {
      ...
      const char*) const
    {
      out_ << offset_ << "::memcpy(fixed_buf_" << fixed_buf_i <<
        "_, DEFAULT_FIXED_BUF_" << fixed_buf_i << ", " << fixed_buf_size << ");" << std::endl;
    }

    template<typename FieldType>
    void process_non_fixed_or_complex(const FieldType& field) const
    {
      out_ << offset_ << field->name() << "_.init_default();" << std::endl;
    }
  ...
  {
    FieldInitImplOps(std::ostream& out, const char* offset)
      : out_(out), offset_(offset)
    {}
      ...
      const
    {
      out_ << offset_ << "::memcpy(fixed_buf_" << fixed_buf_i << "_, "
        ...
        FIELD_OFFSET_SUFFIX << ", " << fixed_buf_size << ");" << std::endl;
    }
  ...
  struct FieldSaveImplOps
  {
      ...
      const char* first_fixed_field_name) const
    {
      out_ << offset_ << "::memcpy("
        ...
        FIXED_BUFFER_PREFIX << fixed_buf_i << "_, " << fixed_buf_size << ");" << std::endl;
    }
    ...
    void process_non_fixed_or_complex(const FieldType& field) const
    {
      out_ << offset_ << field->name() << "_.save_("
        "buf_ptr + " << field->pos() << ", " <<
        (!field->descriptor()->is_fixed() ? "dyn_buf_ptr" : "0") << ");" << std::endl;

      if (!field->descriptor()->is_fixed())
      {
        out_ << offset_ << "dyn_buf_ptr += " << field->name() << "_.dyn_size_();" << std::endl;
      }
    }
  ...
  struct FieldMoveCtorInitOps: public OnlyFixedSumOps
  {
    ...
    void process_fixed_sum(unsigned long, unsigned long, const char*) const
    {}
  ...
  /* WriterGenerator */
  WriterGenerator::WriterGenerator(std::ostream& out_hpp, std::ostream& out_cpp, const char* offset)
    noexcept
    : out_(out_hpp),
      out_cpp_(out_cpp),
      offset_(offset)
  ...
  void
  WriterGenerator::generate_default_buffers_decl(Declaration::StructDescriptor* struct_descriptor)
    noexcept
  {
    ...
    out_ << offset_ << "struct " << struct_descriptor->name() <<
        DEFAULT_BUFFERS_SUFFIX << std::endl << offset_ << "{" << std::endl;
  ...
  void
  WriterGenerator::generate_default_buffers_impl(Declaration::StructDescriptor* struct_descriptor)
    noexcept
  {
    Declaration::StructDescriptor::PosedFieldList_var fields = struct_descriptor->fields();
    ...
    Utils::fetch_fields_with_fixed_sum(
      *fields, FieldBuffersImplOps(out_cpp_, offset_.c_str(), class_name.c_str()));
  }

  /* protected writer generation */
  void WriterGenerator::generate_protected_decl(Declaration::StructDescriptor* struct_descriptor)
    noexcept
  {
    Declaration::StructDescriptor::PosedFieldList_var fields = struct_descriptor->fields();
    ...
    Utils::fetch_fields_with_fixed_sum(
      *fields, FieldDeclOps<Utils::HolderTypeOfField>(out_, (offset_ + "  ").c_str()));

    out_ << offset_ << "};" << std::endl << std::endl;
  }

  void WriterGenerator::generate_protected_impl(Declaration::StructDescriptor* struct_descriptor)
    noexcept
  {
    ...
    generate_swap_impl_((std::string(struct_descriptor->name()) + PROTECTED_WRITER_SUFFIX).c_str(),
      struct_descriptor);
  }

  void WriterGenerator::generate_decl(Declaration::StructWriter* struct_writer)
    noexcept
  {
    out_ << offset_ << "/* " << struct_writer->name() <<
      " writer declaration */" << std::endl;

    Declaration::StructDescriptor_var struct_descriptor = struct_writer->descriptor()->as_struct();
    assert(struct_descriptor.in());

    Declaration::StructWriter::FieldWriterList_var write_fields = struct_writer->fields();
    ...
    Utils::fetch_fields_with_fixed_sum(
      *fields,
      FieldDeclOps<Utils::HolderTypeOfField>(out_, (offset_ + "  ").c_str(), write_fields.in()));

    out_ << offset_ << "};" << std::endl << std::endl;
  }

  void WriterGenerator::generate_impl(Declaration::StructWriter* struct_writer)
    noexcept
  {
    Declaration::StructWriter::FieldWriterList_var fields = struct_writer->fields();
    ...
    generate_common_funs_impl_(struct_writer->name(), struct_descriptor);

    generate_swap_impl_(struct_writer->name(), struct_descriptor);
    ...
    noexcept
  {
    out_ << offset_ << name << "(bool init_defaults = false);" << std::endl <<
      ...
      offset_ << "unsigned long dyn_size_() const;" << std::endl <<
        std::endl << offset_ << "void save_(void* buf, void* dyn_buf) const;" << std::endl;
  }
    ...
    noexcept
  {
    ...
    for (Declaration::StructDescriptor::PosedFieldList::const_iterator fit = fields->begin();
        fit != fields->end(); ++fit)
    {
      if (!(*fit)->descriptor()->is_fixed() || (*fit)->descriptor()->as_struct())
      {
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "  }" << std::endl << offset_ << "}" << std::endl << std::endl;
    ...
    Utils::fetch_fields_with_fixed_sum(*fields, FieldMoveCtorInitOps(out_, offset_.c_str()));
    ...
    out_ << offset_ << "}" << std::endl << "#pragma GCC diagnostic pop" << std::endl << std::endl;
    ...
    out_ << offset_ << "  return *this;" << std::endl <<
      offset_ << "}" << std::endl << "#pragma GCC diagnostic pop" << std::endl << std::endl;
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "{" << std::endl <<
      offset_ << "  init(buf, buf_size);" << std::endl << offset_ << "}" << std::endl << std::endl;
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << class_name << "::init_default()" << std::endl << offset_ << "{" << std::endl;
    ...
    Utils::fetch_fields_with_fixed_sum(*fields, FieldInitImplOps(out_, (offset_ + "  ").c_str()));

    out_ << offset_ << "}" << std::endl << std::endl << "#pragma GCC diagnostic pop" << std::endl;
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "  {" << std::endl <<
      offset_ << "    Stream::Error ostr;" << std::endl <<
      offset_ << "    ostr << \"" << class_name <<
        "::init(): buffer size = \" << size << \" is less then fixed size = \" << FIXED_SIZE;" <<
         std::endl <<
      offset_ << "    throw PlainTypes::CorruptedStruct(ostr);" << std::endl <<
      offset_ << "  }" << std::endl << std::endl <<
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "{" << std::endl <<
      offset_ << "  unsafe_init(buf.get(), buf.size());" << std::endl <<
      offset_ << "  return *this;" << std::endl << offset_ << "}" << std::endl << std::endl;

    // unsigned long dyn_size_() const
    out_ << offset_ << "inline" << std::endl <<
      offset_ << "unsigned long" << std::endl <<
      offset_ << class_name << "::dyn_size_() const" << std::endl <<
      offset_ << "{" << std::endl << offset_ << "  return ";

    {
          ...
          fit != fields->end(); ++fit)
      {
        if (!(*fit)->descriptor()->is_fixed())
        {
          ...
          {
            out_ << " +" << std::endl << offset_ << "    " << (*fit)->name() << "_.dyn_size_()";
          }
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "  void* buf, void* " <<
        ...
        ") const" << std::endl <<
      offset_ << "{" << std::endl <<
      offset_ << "  unsigned char* buf_ptr = static_cast<unsigned char*>(buf);" << std::endl;

    if (dyn_buf_required)
    {
      out_ << offset_ <<
        "  unsigned char* dyn_buf_ptr = static_cast<unsigned char*>(dyn_buf);" << std::endl;
    }

    Utils::fetch_fields_with_fixed_sum(*fields, FieldSaveImplOps(out_, (offset_ + "  ").c_str()));
    ...
    out_ << offset_ << "inline" << std::endl <<
      ...
      offset_ << "  save_(buf, static_cast<unsigned char*>(buf) + FIXED_SIZE);" <<
        std::endl << offset_ << "}" << std::endl << std::endl;
  }
    ...
    noexcept
  {
    ...
    Utils::fetch_fields_with_fixed_sum(*fields, field_buffers_max_ops);
    ...
    if (field_buffers_max_ops.value() > 0)
    {
      out_ << offset_ << "  // fixed buffers swap" <<
        ...
        offset_ << "  char swap_buf_[" <<
          field_buffers_max_ops.value() << "];" << std::endl << std::endl;

      Utils::fetch_fields_with_fixed_sum(*fields, FieldBuffersSwapImplOps(out_, offset_.c_str()));
    }
    ...
    Declaration::StructWriter* struct_writer) noexcept
  {
    ...
    for (Declaration::StructWriter::FieldWriterList::const_iterator fit = fields->begin();
        fit != fields->end(); ++fit)
    {
      ...
      if (simple_field_writer.in())
      {
        ...
        Declaration::SimpleWriter::CppWriteTraits_var
          cpp_write_traits = cpp_write_traits_generator->generate((*fit)->mapping_specifiers());
  ...
  void WriterGenerator::generate_accessors_decl_(Declaration::StructWriter* struct_writer) noexcept
  {
    Declaration::StructWriter::FieldWriterList_var fields = struct_writer->fields();
        ...
        fit != fields->end(); ++fit)
    {
      ...
      // accessor
      out_ << std::endl << offset_ << ret_type_name << " " << (*fit)->name() << "();" << std::endl;
  ...
  void WriterGenerator::generate_accessors_impl_(Declaration::StructWriter* struct_writer) noexcept
  {
    Declaration::StructDescriptor::PosedFieldList_var fields =
      struct_writer->descriptor()->as_struct()->fields();

    Declaration::StructWriter::FieldWriterList_var field_writers = struct_writer->fields();
        ...
        fit != fields->end(); ++fit)
    {
      ...
      for (; field_writer_it != field_writers->end(); ++field_writer_it)
      {
      ...
      if (field_writer_it != field_writers->end())
      {
        ...
        if (!ret_cast_call.empty()) // fixed simple type
        {
          out_ << ret_cast_call << "(static_cast<void*>(" <<
            FIXED_BUFFER_PREFIX << fixed_buf_i << "_" << " + " << fixed_buf_pos << "))";
        }
        else
        {
          out_ << (*fit)->name() << "_";
      ...
      if (!(*fit)->descriptor()->is_fixed() || (*fit)->descriptor()->as_struct().in())
      {
        if (fixed_buf_pos != 0)
        {
          fixed_buf_pos = 0;
