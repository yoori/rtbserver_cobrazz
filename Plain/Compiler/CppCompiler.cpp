...
int main(int argc, char** argv)
{
  ...
  args.add(
    ...
    opt_trace_types);
  args.add(Generics::AppUtils::equal_name("output-hpp"), opt_out_hpp);
  args.add(Generics::AppUtils::equal_name("output-cpp"), opt_out_cpp);
  ...
  if (parser->parse(std::cerr, elements, file, &root_namespace))
  {
    ...
    {
      const Declaration::Namespace::BaseTypeMap& types = root_namespace->types();
      for (Declaration::Namespace::BaseTypeMap::const_iterator type_it = types.begin();
    ...
    else
    {
...

      {
        ...
        for (std::string::iterator it = upper_out_hpp.begin(); it != upper_out_hpp.end(); ++it)
        {
