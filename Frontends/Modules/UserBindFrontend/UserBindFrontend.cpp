        ...
        if(!result_user_id.is_null())
        {
          if(user_bind_client_)
          {
            try
            {
              ...
              auto prev_user_bind_info = get_user_id_(get_request);
                ...
                ++ext_user_i)
              {
                ...
                if(!cur_external_id.empty())
                {
                  try
                  {
                    ...
                    adserver::user_info_svcs::user_bind::GetUserIdRequest
                      get_request;
                    get_request.set_id(external_id_str);
                    get_request.set_timestamp(
                      GrpcAlgs::pack_time(request_info.time));
                    get_request.set_silent(true);
                    get_request.set_generate_user_id(
                ...
                ++ext_user_i)
              {
                    ...
                    resolve_failed_external_ids.end())
                {
                  try
                  {
                    ...
                    adserver::user_info_svcs::user_bind::AddUserIdRequest
                      add_user_request;
                    add_user_request.set_id(cur_external_id);
    ...
    noexcept
  {
      ...
      !result_user_id.is_null())
    {
      ...
      try
      {
        ...
        channel_request.set_pwords(keywords.str());
        channel_request.set_first_url(referer.str());

        trigger_match_result = AdServer::ChannelSvcs::GrpcAlgs::channel_match(
          *channel_client_,
          channel_request);
        ...
        try
        {
          ...
          if(trigger_match_result_present)
          {
              ...
              ch_it != page_channels.end(); ++ch_it, ++res_ch_i)
            {
              ...
              channel_match->set_channel_id(ch_it->channel_id);
              channel_match->set_channel_trigger_id(ch_it->channel_trigger_id);
            }
          ...
          grpc_match_params->set_publishers_optin_timeout(
            GrpcAlgs::pack_time(Generics::Time::ZERO));
          grpc_match_params->set_cohort(cohort.str());
          ...
          if(!merge_user_id.is_null())
          {
            ...
            if(AdServer::UserInfoSvcs::GrpcAlgs::get_user_profile(
                 *user_info_client_,
                 ...
                 merge_user_profile.out()))
            {
              ...
              AdServer::UserInfoSvcs::GrpcAlgs::merge(
                *user_info_client_,
                user_info,
                match_params,
              ...
              AdServer::UserInfoSvcs::GrpcAlgs::remove_user_profile(
                *user_info_client_,
                CorbaAlgs::pack_user_id(merge_user_id));
            }
          }
