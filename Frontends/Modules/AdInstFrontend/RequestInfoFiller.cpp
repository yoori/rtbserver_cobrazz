...
namespace AdServer::Instantiate
{
  ...
  public:
    virtual void process(RequestInfo& request_info, const String::SubString& value) const
    {
      typedef String::StringManip::Splitter<
        ...
        if (!ccid_str.empty() && !String::StringManip::str_to_int(ccid_str, creative_info.ccid))
        {
          throw RequestInfoFiller::InvalidParamException("");
        }

        if (!ccg_keyword_id_str.empty() &&
           !String::StringManip::str_to_int(ccg_keyword_id_str, creative_info.ccg_keyword_id))
        {
          throw RequestInfoFiller::InvalidParamException("");
        }
  ...
  public:
    virtual void process(RequestInfo& request_info, const String::SubString& value) const
    {
      if (value.find('R') != String::SubString::NPOS || value.find('r') != String::SubString::NPOS)
      {
        request_info.consider_request = true;
      }

      if (value.find('N') != String::SubString::NPOS || value.find('n') != String::SubString::NPOS)
      {
        request_info.enabled_notice = true;
      }

      if (value.find('U') != String::SubString::NPOS || value.find('u') != String::SubString::NPOS)
      {
        request_info.set_uid = true;
      }

      if (value.find('C') != String::SubString::NPOS || value.find('c') != String::SubString::NPOS)
      {
      ...
      ip_map_(std::move(ip_map))
  {
    ...
    cookie_processors_.insert(std::make_pair(
      ...
      RequestInfoParamProcessor_var(
        new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::cohort, 50))));

    add_processor_(true, true, Request::Context::GLOBAL_REQUEST_ID,
      new FrontendCommons::RequestIdParamProcessor<RequestInfo>(&RequestInfo::global_request_id));
    add_processor_(false, true, Request::Context::CREATIVE_LIST, new CreativeListParamProcessor());
    add_processor_(false, true, Request::Context::CREATIVE_ID, new CreativeIdParamProcessor());
    ...
    add_processor_(true, true, Request::Context::SECURITY_TOKEN,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::security_token));
    add_processor_(true, true, Request::Context::ENCRYPTED_USER_IP,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::encrypted_user_ip));
    ...
    add_processor_(false, true, Request::Context::CLICKPREFIX_URL,
      new FrontendCommons::UrlParamProcessor<RequestInfo>(&RequestInfo::click_prefix_url));
    ...
    add_processor_(true, true, Request::Context::OPEN_SETTLE_PRICE,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::open_price));
    add_processor_(true, true, Request::Context::OPENX_SETTLE_PRICE,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::openx_price));
    add_processor_(true, true, Request::Context::LIVERAIL_SETTLE_PRICE,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::liverail_price));
    add_processor_(true, true, Request::Context::GOOGLE_SETTLE_PRICE,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::google_price));
    add_processor_(true, true, Request::Context::SECURE.str,
      new FrontendCommons::BoolParamProcessor<RequestInfo>(&RequestInfo::secure));
    add_processor_(true, true, Request::Context::FLAGS, new FlagsProcessor());
    add_processor_(true, true, Request::Context::BID_PRICE, new BidPriceProcessor());
    ...
    add_processor_(true, true, Request::Context::PUB_POSITION_BOTTOM,
      new FrontendCommons::NumberParamProcessor<RequestInfo, unsigned long>(&RequestInfo::hpos));
    ...
    add_processor_(true, true, Request::Context::EXT_TRACK_PARAMS,
      new FrontendCommons::StringParamProcessor<RequestInfo>(&RequestInfo::ext_track_params));
  ...
  void
  RequestInfoFiller::fill(RequestInfo& request_info, const FCGI::HttpRequest& request) const
    /*throw(InvalidParamException, ForbiddenException, Exception)*/
  {
    ...
    try
    {
      ...
      for (HTTP::ParamList::const_iterator it = params.begin(); it != params.end(); ++it)
      {
            ...
            Request::Context::TOKEN_PREFIX.data()) == 0)
        {
          request_info.tokens.insert(
            std::make_pair(it->name.substr(Request::Context::TOKEN_PREFIX.size()), it->value));
        }
      }
      ...
      if (ip_crypter_ && !request_info.encrypted_user_ip.empty())
      {
        try
        {
          ip_crypter_->decrypt(request_info.peer_ip, request_info.encrypted_user_ip);
