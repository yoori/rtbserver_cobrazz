...
namespace AdServer::Bidding
{
  ...
  GrpcClientMetricsProvider::GrpcClientMetricsProvider(std::vector<ClientSource> clients)
  ...
  GrpcClientMetricsProvider::get_values()
  {
    ...
    for (const auto& source : clients_)
    {
      if (auto client = source.client.lock())
      {
        ...
        add_client_endpoint_stats_(result, source.prefix, client->endpoint_stats());
      }
    }
    return result;
  }
    ...
    const AdServer::Grpc::Client::EndpointStats& endpoint_stats)
  {
    ...
    {
      const auto endpoint_prefix = prefix + "_endpoints_" + sanitize_endpoint_key_(endpoint);
