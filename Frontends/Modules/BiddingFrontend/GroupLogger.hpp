...
namespace AdServer::Bidding
{
    ...
    const char* /*code*/)
  {
    ...
    ostr << buffer << buf << " (sec), " << errors_count << " requests";

    logger->log(ostr.str(), key->severity(), key->aspect(), key->code());
  }
  ...
  CellsKey::operator ==(const CellsKey& right) const noexcept
  {
    return hostname_ == right.hostname_ &&
      ...
      severity_ == right.severity_ && code_ == right.code_ && aspect_ == right.aspect_;
  }
