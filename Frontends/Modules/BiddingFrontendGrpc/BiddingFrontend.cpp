...
#include <Frontends/Modules/BiddingFrontend/Utils.hpp>
...
namespace AdServer::Bidding
{
  namespace
  {
    ...
    struct ContextualChannelConverter
    {
      ...
      operator()(const AdServer::UserInfoSvcs::UserInfoMatcher::ChannelWeight& ch_weight) const
      {
        return ch_weight;
      }
      ...
      operator()(const AdServer::ChannelSvcs::
        ChannelServerBase::ContentChannelAtom& contextual_channel) const
      {
    ...
    noexcept
  {
      ...
      !common_info.external_user_id.empty())
    {
      ...
      try
      {
        const auto& grpc_user_bind_operation_distributor = grpc_container_->grpc_user_bind_operation_distributor;
    ...
    if (!request_info.filter_request)
    {
      try
      {
        ...
        std::string urls_words;
        {
          ...
          const auto size = request_params.common_info.urls.size();
          ...
          urls = std::move(urls_str);
          if (!urls_words_str.empty())
          {
            urls_words = std::move(urls_words_str);
          }
        }
        ...
        {
          const auto& trigger_page_channels = trigger_matched_channels->matched_channels().page_channels();
          ...
          const auto& trigger_url_channels = trigger_matched_channels->matched_channels().url_channels();
          ...
          const auto& trigger_url_keyword_channels = trigger_matched_channels->matched_channels().url_keyword_channels();
          ...
          const auto& trigger_search_channels = trigger_matched_channels->matched_channels().search_channels();
          ...
          const auto& trigger_uid_channels = trigger_matched_channels->matched_channels().uid_channels();
    ...
    const AdServer::CampaignSvcs::Proto::RequestCreativeResult& campaign_match_result) noexcept
  {
    ...
    else
    {
      ...
      try
      {
        ...
        const auto grpc_user_info_operation_distributor = grpc_container_->grpc_user_info_operation_distributor;
      ...
      if (blacklisted_time_intervals_.contains(day_time))
      {
            ...
            static_cast<std::uint32_t>(CampaignSvcs::US_OPTIN))
        {
          request_params.common_info.user_status =
            static_cast<std::uint32_t>(CampaignSvcs::US_BLACKLISTED);
        }
