...
namespace AdServer::Bidding::Grpc
{
  ...
  bool GoogleBidRequestTask::read_request() noexcept
  {
  ...
  bool GoogleBidRequestTask::write_response(
      ...
      campaign_match_result) noexcept
  {
    try
    {
      ...
      assert(
        campaign_match_result.ad_slots().size() == static_cast<int>(ad_slots_context_.size()));

      const auto& campaign_ad_slots = campaign_match_result.ad_slots();
      const std::size_t campaign_ad_slots_size = campaign_ad_slots.size();
      for(std::size_t ad_slot_i = 0;
          ad_slot_i < campaign_ad_slots_size;
          ++ad_slot_i)
      {
        const auto& ad_slot_result = campaign_ad_slots[ad_slot_i];
        const auto& adslot = bid_request_.adslot(ad_slot_i);
        ...
        if(selected_creatives.size() > 0)
        {
          ...
          for(const auto& publisher_account : request_info_.publisher_account_ids)
          {
            auto account_it = bid_frontend_->account_traits_.find(publisher_account);
          ...
          {
            // buyer_creative_id
            const auto& creative = ad_slot_result.selected_creatives()[0];
  ...
  void GoogleBidRequestTask::write_empty_response(unsigned int code) noexcept
  {
    FrontendCommons::HttpResponse_var response = bid_frontend_->create_response();
    if(code < 300)
    {
  ...
  void GoogleBidRequestTask::print_request(std::ostream& /*out*/) const noexcept
  {
    //out << bid_request__.DebugString();
  }

  void GoogleBidRequestTask::clear() noexcept
  {
