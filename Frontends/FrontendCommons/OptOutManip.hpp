...
namespace AdServer
{
  class OptInDays
  {
    ...
    static const char* save_opt_in_days(const Generics::Time& val, std::string& buf)
      noexcept;
  };
}
...
namespace AdServer
{
    ...
    Generics::Time& val) /*throw(InvalidParam)*/
  {
    ...
    {
      throw InvalidParam("FrontendCommons::load_opt_in_days(): " "noncorrect input buffer length.");
    }
  ...
  inline
  const char* OptInDays::save_opt_in_days(const Generics::Time& val, std::string& buf)
    noexcept
  {
