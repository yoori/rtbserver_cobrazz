...
namespace AdServer::Frontends
{
      ...
      common_module_(new CommonModule(logger_))
  {
    ...
    grpc_coalesce_runner_ = std::make_shared<AdServer::Commons::BoostAsioContextRunActiveObject>(
      callback_,
  ...
  FrontendsPool::handle_request(
    ...
    noexcept
  {
    for (auto frontend_it = frontends_.begin(); frontend_it != frontends_.end(); frontend_it++)
    {
    ...
    /*throw(eh::Exception)*/
  {
    try
    {
      ...
      for (const auto& module : modules_)
      {
        ...
        {
          interrupt_threads = fe_config.BidFeConfiguration()->interrupt_threads();
          break;
        }
      }
      ...
      for (auto module_it = modules_.begin(); module_it != modules_.end(); ++module_it)
      {
        ...
        {
          init_frontend_<Bidding::Frontend>(
            "bidding",
        ...
        {
          init_frontend_<PubPixel::Frontend>(
            "pubpixel",
        ...
        {
          init_frontend_<ContentFrontend>(
            "content",
        ...
        {
          init_frontend_<DirectoryModule>(
            "directory",
            fe_config.ContentFeConfiguration(),
            logger_,
            request_workers_);
        ...
        {
          init_frontend_<WebStat::Frontend>(
            "webstat",
        ...
        {
          init_frontend_<Action::Frontend>(
            "action",
        ...
        {
          init_frontend_<UserBindFrontend>(
            "userbind",
        ...
        {
          init_frontend_<Passback::Frontend>(
            "passback",
        ...
        {
          init_frontend_<PassbackPixel::Frontend>(
            "passbackpixel",
        ...
        {
          init_frontend_<Instantiate::Frontend>(
            "adinst",
        ...
        {
          init_frontend_<ImprTrack::Frontend>(
            "imprtrack",
  ...
  FrontendsPool::init_frontends_()
  {
      ...
      threads.emplace_back([frontend, name, &errors, i]()
        {
          try
          {
            const std::string begin_label = std::string("FrontendsPool ") + name + " init begin";
            trace_startup(begin_label.c_str());
            frontend->init();
            const std::string end_label = std::string("FrontendsPool ") + name + " init end";
    ...
    for (std::size_t i = 0; i < errors.size(); ++i)
    {
      ...
      {
        Stream::Error ostr;
        ostr << "Frontend '" << frontend_names_[i] << "' init failed: " << ex.what();
        throw Exception(ostr);
      }
      catch (...)
      {
        Stream::Error ostr;
        ostr << "Frontend '" << frontend_names_[i] << "' init failed: unknown exception";
        throw Exception(ostr);
      }
    }
  }

  template <class FrontendType, typename ConfigType, typename ...ArgsType>
  void
  FrontendsPool::init_frontend_(const char* name, const ConfigType& cfg, ArgsType&&... params)
  {
    if (cfg.present())
    {
      frontend_names_.emplace_back(name);
      frontends_.emplace_back(new FrontendType(config_, std::forward<ArgsType>(params)...));
    }
  }
}
