...
namespace AdServer::RequestInfoSvcs
{
  ...
  protected:
    ...
    using RequestOperationProxy_var = ReferenceCounting::SmartPtr<RequestOperationProxy>;
  ...
  protected:
    ...
    void
    delegate_processing_(const RequestProcessDelegate& request_process_gelegate)
      /*throw(Exception)*/;

    AdServer::Commons::Awaitable<void>
    co_delegate_processing_(const RequestProcessDelegate& request_process_gelegate);
    ...
    virtual void
    process_impression_(const ImpressionInfo& impression_info, bool move_enabled)
      /*throw(RequestContainerProcessor::Exception)*/;

    virtual AdServer::Commons::Awaitable<void>
    co_process_impression_(const ImpressionInfo& impression_info, bool move_enabled);
    ...
    get_profile_(Transaction* transaction);

    static AdServer::Commons::Awaitable<Generics::ConstSmartMemBuf_var>
    co_get_profile_(Transaction* transaction);
    ...
    Transaction_var
    get_transaction_(const AdServer::Commons::RequestId& request_id);

    AdServer::Commons::Awaitable<Transaction_var>
    co_get_transaction_(const AdServer::Commons::RequestId& request_id);
  ...
  using RequestInfoContainer_var = ReferenceCounting::SmartPtr<RequestInfoContainer>;
} // namespace AdServer::RequestInfoSvcs
...
namespace AdServer::RequestInfoSvcs
{
    ...
    /*throw(eh::Exception, RevenueDecimal::Overflow)*/
  {
    ...
    if (error_message)
    {
      logger_->sstream(Logging::Logger::ERROR, Aspect::REQUEST_INFO_CONTAINER) << error_message <<
