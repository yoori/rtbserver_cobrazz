...
#include <RequestInfoSvcs/RequestInfoManager/Compatibility/RequestProfile_v362.hpp>
#include <RequestInfoSvcs/RequestInfoManager/Compatibility/RequestProfile_v363.hpp>
...
namespace AdServer::RequestInfoSvcs
{
  namespace
  {
    ...
    namespace RequestInfoSvcs_v351::RequestInfoSvcsTarget
    {
    ...
    namespace RequestInfoSvcs_v351
    {
          ...
          cmp_channels_Container& reader)
      {
        for (AdServer::RequestInfoSvcs_v351::RequestInfoProfileReader::
              cmp_channels_Container::const_iterator it = reader.begin();
            it != reader.end(); ++it)
        {
    ...
    namespace RequestInfoSvcs_v356::RequestInfoSvcsTarget
    {
    ...
    namespace RequestInfoSvcs_v360::RequestInfoSvcsTarget
    {
    ...
    namespace RequestInfoSvcs_v361::RequestInfoSvcsTarget
    {
      using namespace ::AdServer::RequestInfoSvcs_v362;
    }
    ...
    namespace RequestInfoSvcs_v361
    {
      ...
      convert_to_v362(const Generics::MemBuf& membuf)
      {
        try
        {
          ...
          convert_revenue_(request_writer.orig_adv_revenue(), old_request_reader.adv_revenue());
          convert_revenue_(request_writer.orig_adv_comm_revenue(), old_request_reader.adv_comm_revenue());
          convert_revenue_(request_writer.orig_adv_payable_comm_amount(), old_request_reader.adv_payable_comm_amount());
          convert_revenue_(request_writer.orig_pub_revenue(), old_request_reader.pub_revenue());
          convert_revenue_(request_writer.orig_pub_comm_revenue(), old_request_reader.pub_comm_revenue());
          convert_revenue_(request_writer.orig_isp_revenue(), old_request_reader.isp_revenue());
    ...
    namespace RequestInfoSvcs_v362::RequestInfoSvcsTarget
    {
      using namespace ::AdServer::RequestInfoSvcs_v363;
    }

    namespace RequestInfoSvcs_v362
    {
      void
      convert_revenue_(
        RequestInfoSvcsTarget::RequestInfoRevenueWriter& writer,
        const AdServer::RequestInfoSvcs_v362::RequestInfoRevenueReader& reader)
      {
        writer.rate_id() = reader.rate_id();
        writer.impression() = reader.impression();
        writer.click() = reader.click();
      ...
      convert_cmp_channels_(
        RequestInfoSvcsTarget::RequestInfoProfileWriter::
          cmp_channels_Container& writer,
        const AdServer::RequestInfoSvcs_v362::RequestInfoProfileReader::
          cmp_channels_Container& reader)
      {
        for (AdServer::RequestInfoSvcs_v362::RequestInfoProfileReader::
              cmp_channels_Container::const_iterator it = reader.begin();
            it != reader.end(); ++it)
        {
          RequestInfoSvcsTarget::ChannelRevenueWriter rec_writer;
      ...
      template<typename WriterContainerType, typename ReaderContainerType>
      void
      convert_custom_actions_(WriterContainerType& writer, const ReaderContainerType& reader)
      {
        for (typename ReaderContainerType::const_iterator it = reader.begin();
            it != reader.end(); ++it)
        {
          RequestInfoSvcsTarget::NonconsideredCustomActionWriter rec_writer;
          rec_writer.action_id() = (*it).action_id();
          rec_writer.action_request_id() = (*it).action_request_id();
          rec_writer.time() = (*it).time();
          rec_writer.referer() = (*it).referer();
          rec_writer.order_id() = (*it).order_id();
          rec_writer.action_value() = (*it).action_value();
          writer.push_back(rec_writer);
        }
      }

      Generics::ConstSmartMemBuf_var
      convert_to_v363(const Generics::MemBuf& membuf)
      {
        try
        {
          RequestInfoSvcsTarget::RequestInfoProfileWriter request_writer;
          ...
          request_writer.notice_non_considered() = old_request_reader.notice_non_considered();
          request_writer.notice_pub_revenue_type() = old_request_reader.notice_pub_revenue_type();
          request_writer.notice_imp_revenue() = old_request_reader.notice_imp_revenue();
          request_writer.impression_time() = old_request_reader.impression_time();
          request_writer.impression_verified() = old_request_reader.impression_verified();
          request_writer.impression_non_considered() = old_request_reader.impression_non_considered();
          request_writer.impression_pub_revenue_type() = old_request_reader.impression_pub_revenue_type();
          request_writer.impression_imp_revenue() = old_request_reader.impression_imp_revenue();
          request_writer.impression_user_id() = old_request_reader.impression_user_id();
          request_writer.click_time() = old_request_reader.click_time();
          request_writer.click_done() = old_request_reader.click_done();
          request_writer.click_non_considered() = old_request_reader.click_non_considered();
          request_writer.action_time() = old_request_reader.action_time();
          ...
          request_writer.fraud() = old_request_reader.fraud();
          request_writer.auction_type() = old_request_reader.auction_type();
          request_writer.viewability() = old_request_reader.viewability();

          convert_custom_actions_(
            request_writer.custom_actions_imp_non_considered_(),
            old_request_reader.custom_actions_imp_non_considered_());
          convert_custom_actions_(
            request_writer.custom_actions_click_non_considered_(),
            old_request_reader.custom_actions_click_non_considered_());
          ...
          convert_revenue_(request_writer.delta_adv_revenue(), old_request_reader.delta_adv_revenue());
          ...
          convert_revenue_(request_writer.orig_adv_revenue(), old_request_reader.orig_adv_revenue());
          convert_revenue_(request_writer.orig_adv_comm_revenue(), old_request_reader.orig_adv_comm_revenue());
          convert_revenue_(request_writer.orig_adv_payable_comm_amount(), old_request_reader.orig_adv_payable_comm_amount());
          convert_revenue_(request_writer.orig_pub_revenue(), old_request_reader.orig_pub_revenue());
          convert_revenue_(request_writer.orig_pub_comm_revenue(), old_request_reader.orig_pub_comm_revenue());
          convert_revenue_(request_writer.orig_isp_revenue(), old_request_reader.orig_isp_revenue());

          request_writer.additional_info() = "";
        ...
        {
          Stream::Error ostr;
          ostr << "Can't adapt profile with version = 362: " << ex.what();
          throw RequestProfileAdapter::Exception(ostr);
        }
      }
    }

    namespace RequestInfoSvcs_v363::RequestInfoSvcsTarget
    {
      using namespace ::AdServer::RequestInfoSvcs;
    }

    namespace RequestInfoSvcs_v363
    {
      void
      convert_revenue_(
        RequestInfoSvcsTarget::RequestInfoRevenueWriter& writer,
        const AdServer::RequestInfoSvcs_v363::RequestInfoRevenueReader& reader)
      {
        writer.rate_id() = reader.rate_id();
        writer.impression() = reader.impression();
        writer.click() = reader.click();
      ...
      void
      convert_cmp_channels_(
        RequestInfoSvcsTarget::RequestInfoProfileWriter::cmp_channels_Container& writer,
        const AdServer::RequestInfoSvcs_v363::RequestInfoProfileReader::
          cmp_channels_Container& reader)
      {
      ...
      Generics::ConstSmartMemBuf_var
      convert_to_v364(const Generics::MemBuf& membuf)
      {
        try
        {
          RequestInfoSvcsTarget::RequestInfoProfileWriter request_writer;
          const AdServer::RequestInfoSvcs_v363::RequestInfoProfileReader old_request_reader(
            membuf.data(),
            membuf.size());

          request_writer.version() = 364;
          request_writer.request_done() = old_request_reader.request_done();
          request_writer.time() = old_request_reader.time();
          request_writer.isp_time() = old_request_reader.isp_time();
          request_writer.pub_time() = old_request_reader.pub_time();
          ...
          request_writer.walled_garden() = old_request_reader.walled_garden();
          request_writer.hid_profile() = old_request_reader.hid_profile();
          request_writer.disable_fraud_detection() =
            old_request_reader.disable_fraud_detection();
          request_writer.adv_account_id() = old_request_reader.adv_account_id();
          request_writer.advertiser_id() = old_request_reader.advertiser_id();
          request_writer.campaign_id() = old_request_reader.campaign_id();
          request_writer.ccg_id() = old_request_reader.ccg_id();
          ...
          request_writer.colo_id() = old_request_reader.colo_id();
          request_writer.currency_exchange_id() = old_request_reader.currency_exchange_id();
          request_writer.tag_delivery_threshold() =
            old_request_reader.tag_delivery_threshold();
          request_writer.tag_visibility() = old_request_reader.tag_visibility();
          request_writer.tag_top_offset() = old_request_reader.tag_top_offset();
          request_writer.tag_left_offset() = old_request_reader.tag_left_offset();
          request_writer.ccg_keyword_id() = old_request_reader.ccg_keyword_id();
          ...
          convert_revenue_(request_writer.orig_adv_revenue(),
            old_request_reader.orig_adv_revenue());
          convert_revenue_(request_writer.orig_adv_comm_revenue(),
            old_request_reader.orig_adv_comm_revenue());
          convert_revenue_(request_writer.orig_adv_payable_comm_amount(),
            old_request_reader.orig_adv_payable_comm_amount());
          convert_revenue_(request_writer.orig_pub_revenue(),
            old_request_reader.orig_pub_revenue());
          convert_revenue_(request_writer.orig_pub_comm_revenue(),
            old_request_reader.orig_pub_comm_revenue());
          convert_revenue_(request_writer.orig_isp_revenue(),
            old_request_reader.orig_isp_revenue());
          convert_revenue_(request_writer.adv_revenue(), old_request_reader.adv_revenue());
          convert_revenue_(request_writer.adv_comm_revenue(),
            old_request_reader.adv_comm_revenue());
          convert_revenue_(request_writer.adv_payable_comm_amount(),
            old_request_reader.adv_payable_comm_amount());
          convert_revenue_(request_writer.pub_revenue(), old_request_reader.pub_revenue());
          request_writer.pub_bid_cost() = old_request_reader.pub_bid_cost();
          request_writer.pub_floor_cost() = old_request_reader.pub_floor_cost();
          convert_revenue_(request_writer.pub_comm_revenue(),
            old_request_reader.pub_comm_revenue());
          request_writer.pub_commission() = old_request_reader.pub_commission();
          convert_revenue_(request_writer.isp_revenue(), old_request_reader.isp_revenue());
          request_writer.isp_revenue_share() = old_request_reader.isp_revenue_share();
          request_writer.adv_currency_rate() = old_request_reader.adv_currency_rate();
          request_writer.pub_currency_rate() = old_request_reader.pub_currency_rate();
          request_writer.isp_currency_rate() = old_request_reader.isp_currency_rate();
          convert_revenue_(request_writer.delta_adv_revenue(),
            old_request_reader.delta_adv_revenue());

          std::copy(old_request_reader.channels().begin(),
            old_request_reader.channels().end(),
            std::back_inserter(request_writer.channels()));
          request_writer.geo_channel_id() = old_request_reader.geo_channel_id();
          request_writer.device_channel_id() = old_request_reader.device_channel_id();
          ...
          request_writer.enabled_impression_tracking() =
            old_request_reader.enabled_impression_tracking();
          request_writer.enabled_action_tracking() =
            old_request_reader.enabled_action_tracking();
          request_writer.notice_received() = old_request_reader.notice_received();
          request_writer.notice_non_considered() = old_request_reader.notice_non_considered();
          request_writer.notice_pub_revenue_type() =
            old_request_reader.notice_pub_revenue_type();
          request_writer.notice_imp_revenue() = old_request_reader.notice_imp_revenue();
          request_writer.impression_time() = old_request_reader.impression_time();
          request_writer.impression_verified() = old_request_reader.impression_verified();
          ...
          request_writer.impression_pub_revenue_type() =
            old_request_reader.impression_pub_revenue_type();
          request_writer.impression_imp_revenue() =
            old_request_reader.impression_imp_revenue();
          request_writer.impression_user_id() = old_request_reader.impression_user_id();
          request_writer.click_time() = old_request_reader.click_time();
          request_writer.click_done() = old_request_reader.click_done();
          request_writer.click_non_considered() = old_request_reader.click_non_considered();
          ...
          convert_custom_actions_(request_writer.custom_actions_imp_non_considered_(),
            old_request_reader.custom_actions_imp_non_considered_());
          convert_custom_actions_(request_writer.custom_actions_click_non_considered_(),
            old_request_reader.custom_actions_click_non_considered_());
          convert_cmp_channels_(request_writer.cmp_channels(), old_request_reader.cmp_channels());
          request_writer.auction_type() = old_request_reader.auction_type();
          request_writer.viewability() = old_request_reader.viewability();
          ...
          for (const auto& action_name : old_request_reader.post_impression_actions())
          {
            if (old_request_reader.impression_verified())
            {
              request_writer.post_imp_done_actions().push_back(action_name);
            }
            else
            {
              RequestInfoSvcsTarget::PostActionWriter action_writer;
              action_writer.name() = action_name;
              // Legacy profiles did not store the event time.
              action_writer.time() = 0;
              action_writer.value() = "";
              request_writer.post_imp_pending_actions().push_back(action_writer);
            }
          }
          ...
          std::copy(old_request_reader.model_ctrs().begin(),
            old_request_reader.model_ctrs().end(),
            std::back_inserter(request_writer.model_ctrs()));
          request_writer.conv_rate_algorithm_id() =
            old_request_reader.conv_rate_algorithm_id();
          request_writer.conv_rate() = old_request_reader.conv_rate();
          std::copy(old_request_reader.model_conv_rates().begin(),
            old_request_reader.model_conv_rates().end(),
            std::back_inserter(request_writer.model_conv_rates()));
          request_writer.self_service_commission() =
            old_request_reader.self_service_commission();
          request_writer.adv_commission() = old_request_reader.adv_commission();
          request_writer.pub_cost_coef() = old_request_reader.pub_cost_coef();
          request_writer.at_flags() = old_request_reader.at_flags();
          request_writer.additional_info() = old_request_reader.additional_info();
        ...
        {
          Stream::Error ostr;
          ostr << "Can't adapt profile with version = 363: " << ex.what();
          throw RequestProfileAdapter::Exception(ostr);
        }
      }
    }
...
namespace AdServer::RequestInfoSvcs
{
    ...
    const Generics::ConstSmartMemBuf* mem_buf) /*throw(Exception)*/
  {
    ...
    const unsigned long version_head_size = RequestInfoProfileVersionReader::FIXED_SIZE;
    ...
    Generics::ConstSmartMemBuf_var result_mem_buf = ReferenceCounting::add_ref(mem_buf);
    ...
    try
    {
      ...
      if (version_reader.version() != AdServer::RequestInfoSvcs::CURRENT_REQUEST_PROFILE_VERSION)
      {
        ...
        if (current_version == 362)
        {
          result_mem_buf = RequestInfoSvcs_v362::convert_to_v363(result_mem_buf->membuf());
          current_version = 363;
        }

        if (current_version == 363)
        {
          result_mem_buf = RequestInfoSvcs_v363::convert_to_v364(result_mem_buf->membuf());
          current_version = 364;
        }
