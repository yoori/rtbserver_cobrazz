...
namespace AdServer::RequestInfoSvcs
{
  ...
  using SiteReachProcessor_var = ReferenceCounting::SmartPtr<SiteReachProcessor>;
  ...
  public:
    ...
    virtual void process_tag_request(const TagRequestInfo& request_info)
      /*throw(TagRequestProcessor::Exception)*/;

    virtual AdServer::Commons::Awaitable<void>
    co_process_tag_request(const TagRequestInfo& request_info);
  ...
  private:
    ...
    using UserSiteReachMap_var = ReferenceCounting::SmartPtr<UserSiteReachMap>;
  ...
  private:
    ...
    AdServer::Commons::Awaitable<void>
    co_process_tag_request_(const TagRequestInfo& request_info);
  ...
  using UserSiteReachContainer_var = ReferenceCounting::SmartPtr<UserSiteReachContainer>;
...
namespace AdServer::RequestInfoSvcs
{
    ...
    const SiteReachProcessor::SiteReachInfo& right) const
  {
    return
      ...
      std::equal(appearance_list.begin(), appearance_list.end(), right.appearance_list.begin());
  }

  inline
  void
  SiteReachProcessor::SiteReachInfo::print(std::ostream& ostr, const char* offset) const
  {
