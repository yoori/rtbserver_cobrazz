...
namespace AdServer::RequestInfoSvcs
{
  ...
  struct RequestPool: public ReferenceCounting::AtomicImpl
  {
    ...
    struct Bank
    {
      Bank(const Generics::Time& start_time_val, const Generics::Time& end_time_val)
        noexcept
    ...
    RequestPool(const Generics::Time& bank_period_val, unsigned long max_request_pool_size_val)
  ...
  namespace
  {
    ...
    private:
      ...
      protected:
        ...
        void
        send_request_(Context& context, const Request& request)
          /*throw(Sender::Exception)*/;

        template<typename IntType>
        static
        void
        init_int_message_(zmq::message_t& message, const IntType& value)
          noexcept;
        ...
        static
        bool
        zmq_send_(zmq::socket_t& sock, zmq::message_t& msg, int flags, const char* part_name)
          /*throw(eh::Exception)*/;
    ...
    private:
      static Context_var
      init_context_(const char* uid_private_key, const TagRequestProfiler::AddressList& addresses)
        /*throw(Exception)*/;
    };
  }
  ...
  namespace
  {
      ...
      : sock(zmq_context, ZMQ_PUSH)
    {
      ...
      sock.setsockopt(ZMQ_RECONNECT_IVL, &reconnect_interval, sizeof(reconnect_interval));
      ...
      const Generics::Time& now)
    {
      ...
      const Generics::Time up_time_bound = request_pool_->time_bank_round(now - window_size_);
...
#     endif
      ...
      {
        // check expired banks
        RequestPool::BankList::iterator bank_it = request_pool_->banks.begin();

        while (bank_it != request_pool_->banks.end() && bank_it->start_time <= up_time_bound)
        {
...
#       endif
        ...
        request_pool_->banks.erase(request_pool_->banks.begin(), bank_it);
...
#         endif
          ...
          for (; bank_it != request_pool_->banks.end(); ++bank_it)
          {
            if (bank_it->requests_size != 0)
            {
...
#             ifdef DEBUG_OUTPUT
              std::cout << "CBRT: cbtr = " << cur_bank_target_requests <<
                ...
                ", su.i = " << (bank_it->start_time - up_time_bound).tv_sec << std::endl;
#             endif
            }
          }
        ...
        request_pool_->last_request_time = std::max(request_pool_->last_request_time, now);

        // eval next sending period by cur_bank_target_requests
        // allow to adapt speed for additional processing time
        if (request_pool_->cur_bank_target_requests > request_pool_->cur_bank_sent_requests)
        {
#         ifdef DEBUG_OUTPUT
      ...
      {
        {
          ...
          Sync::ConditionalGuard guard(request_pool_->cond, request_pool_->lock);
        ...
        get_requests_to_process_(next_request_time, process_requests, now);
...
#         endif
          ...
          // send requests
          for (RequestPool::RequestList::const_iterator it = process_requests.begin();
    ...
    template<typename IntType>
    void
    Sender::Job::init_int_message_(zmq::message_t& message, const IntType& value)
      noexcept
    {
      char BUF[std::numeric_limits<IntType>::digits10 + 3];
      size_t res_size = String::StringManip::int_to_str(value, BUF, sizeof(BUF));
      assert(res_size > 0);
    ...
    void
    Sender::Job::send_request_(Context& context, const Request& request)
      /*throw(Sender::Exception)*/
    {
      ...
      try
      {
        ...
        {
          const std::string signed_user_id_str = context.uid_generator->sign(request.user_id).str();
          user_id_message.rebuild(signed_user_id_str.size());
          ::memcpy(user_id_message.data(), signed_user_id_str.data(), signed_user_id_str.size());
        }
      ...
      /*throw(Exception)*/
    {
      ...
      try
      {
        ...
        context->uid_generator.reset(new Generics::SignedUuidGenerator(uid_private_key));

        for (TagRequestProfiler::AddressList::const_iterator addr_it = addresses.begin();
  ...
  RequestPool::Bank::filter_requests(
    ...
    const noexcept
  {
    for (RequestList::iterator req_it = requests.begin(); req_it != requests.end();)
    {
      RequestFilterMap::const_iterator f_it = request_filters.find(req_it->filter_hash);
      if (f_it != request_filters.end())
      {
        RequestFilter::TimeSet::const_iterator time_it = f_it->second.times.lower_bound(time_start);
  ...
  RequestPool::time_bank_round(const Generics::Time& time) const
    noexcept
  {
    Generics::Time res((time.tv_sec / bank_period.tv_sec + 1) * bank_period.tv_sec);
  ...
  RequestPool::print(std::ostream& ostr) const noexcept
  {
    ostr <<
      ...
      ", cbsr = " << cur_bank_sent_requests << ", cbtr = " << cur_bank_target_requests << ":";
    for (BankList::const_iterator bank_it = banks.begin(); bank_it != banks.end(); ++bank_it)
    {
      ostr << "[s=" << bank_it->start_time.get_gm_time().format("%F %T") <<
        ",e=" << bank_it->end_time.get_gm_time().format("%F %T") <<
        ",sz=" << bank_it->requests_size << ",rsz=" << bank_it->requests.size() << "]";
    }
  }
  ...
  void
  TagRequestProfiler::process_tag_request(const TagRequestInfo& tag_request_info)
    /*throw(TagRequestProcessor::Exception)*/
  {
    ...
    try
    {
         ...
         tag_request_info.profile_referer)
      {
        ...
        for (auto it = tag_request_info.urls.begin(); it != tag_request_info.urls.end(); ++it)
        {
          req_list.push_back(
            Request(
              tag_request_info.time,
        ...
        RequestPool::BankList::iterator bank_it = request_pool_->banks.end();
        if (request_pool_->banks.empty() || request_pool_->banks.rbegin()->end_time < round_now)
        {
        ...
        if (create_new_bank)
        {
          RequestPool::Bank new_bank(round_now - request_pool_->bank_period, round_now);
        ...
        if (!req_list.empty())
        {
          ...
          {
            bank_it->request_filters[req_it->filter_hash].times.insert(tag_request_info.time);
          }
          ...
          if (request_pool_->sum_requests_size > request_pool_->max_request_pool_size)
          {
    ...
    {
      logger_->sstream(Logging::Logger::ERROR) << FUN << ": got eh::Exception: " << ex.what();
    }
  }
}
