...
namespace AdServer::RequestInfoSvcs
{
  ...
  UserColoReachContainer::co_process_request(const RequestInfo& request_info)
  {
    ...
    try
    {
      ...
      catch(const eh::Exception& ex)
      {
        ...
        ostr << FUN << ": on read, for user_id = " <<
          request_info.user_id.to_string() << "' caught eh::Exception: " << ex.what();
        throw Exception(ostr);
      }
      ...
      if (mem_buf.in())
      {
        ...
        bool changed_isp_create_time = old_isp_time_found && (isp_time < old_isp_time);
      ...
      if (save_colo || colo_appeared || isp_colo_appeared)
      {
