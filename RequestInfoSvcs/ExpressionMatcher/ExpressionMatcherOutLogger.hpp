...
namespace AdServer::RequestInfoSvcs
{
  ...
  public:
    ...
    void
    process_match_request(const MatchRequestProcessor::MatchInfo& request_info)
  ...
  private:
    ...
    using ChannelHitStatLogger_var = ReferenceCounting::AssertPtr<ChannelHitStatLogger>::Ptr;
    ...
    protected:
      ...
      void
      process_colo_reach(const ColoReachInfo& reach_info)
        /*throw(ColoReachProcessor::Exception)*/
      {
        if (reach_info.household)
        {
              ...
              it != reach_info.colocations.end(); ++it)
          {
            ...
            data.add(inner_key, typename CollectorT::DataT::DataT(0, 0, 0, it->counter));
        ...
        else
        {
              ...
              it != reach_info.colocations.end(); ++it)
          {
            typename CollectorT::DataT::KeyT inner_key(
              reach_info.create_time, it->last_appearance_date);
            typename CollectorT::DataT data;
            data.add(inner_key, typename CollectorT::DataT::DataT(it->counter, 0, 0, 0));
            this->add_record(typename CollectorT::KeyT(it->date, it->id), data);
          }
              ...
              it != reach_info.ad_colocations.end(); ++it)
          {
            typename CollectorT::DataT::KeyT inner_key(
              reach_info.create_time, it->last_appearance_date);
            typename CollectorT::DataT data;
            data.add(inner_key, typename CollectorT::DataT::DataT(0, it->counter, 0, 0));
            this->add_record(typename CollectorT::KeyT(it->date, it->id), data);
          }

          for (IdAppearanceList::const_iterator it = reach_info.merge_colocations.begin();
              it != reach_info.merge_colocations.end(); ++it)
          {
            typename CollectorT::DataT::KeyT inner_key(
              reach_info.create_time, it->last_appearance_date);
            typename CollectorT::DataT data;
            data.add(inner_key, typename CollectorT::DataT::DataT(0, 0, it->counter, 0));
            this->add_record(typename CollectorT::KeyT(it->date, it->id), data);
          }
        }
      }
