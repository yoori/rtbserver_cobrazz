...
namespace AdServer::RequestInfoSvcs
{
  ...
  struct SizeChannel
  {
    SizeChannel(const Commons::ImmutableString& size_val, unsigned long channel_id_val)
      : size(size_val),
  ...
  struct ChannelLevel
  {
    ...
    {
      return channel_id == right.channel_id && level == right.level;
    }
  ...
  public:
    virtual void process_match_request(const MatchInfo& request_info)
      /*throw(Exception)*/ = 0;
  ...
  public:
    ...
    private:
      static
      std::ostream&
      print_channels_(std::ostream& ostr, const ChannelIdList& lst)
        noexcept;

      static
      std::ostream&
      print_channels_(std::ostream& ostr, const ChannelIdArray& lst)
        noexcept;

      std::ostream&
      print_channel_counter_map_(std::ostream& ostr, const ChannelImpCounterVector& chs) const
        noexcept;

      std::ostream&
      print_channel_avg_revenue_map_(std::ostream& ostr, const ChannelAvgRevenueVector& chs) const
        noexcept;
    };
...
namespace AdServer::RequestInfoSvcs
{
  namespace
  {
    ...
    inline
    bool compare_seq_(const ContainerType& left, const ContainerType& right) noexcept
    {
  ...
  inline
  void
  MatchRequestProcessor::MatchInfo::print(std::ostream& ostr, const char* offset) const
    noexcept
  {
    ...
    for (AdBidSlotList::const_iterator sit = text_ads.begin(); sit != text_ads.end(); ++sit)
    {
  ...
  {
    return size == right.size && channel_id == right.channel_id;
  }
  ...
  {
    return SizeChannel::operator==(right) && ecpm == right.ecpm;
  }

  inline
  bool
  InventoryActionProcessor::InventoryInfo::
  ChannelImpInfo::operator==(const ChannelImpInfo& right) const noexcept
  {
  ...
  {
    return imp_channels.empty() && imp_other_channels.empty() && impop_no_imp_channels.empty();
  }

  inline
  bool
  InventoryActionProcessor::InventoryInfo::
  ChannelImpAppearInfo::operator==(const ChannelImpAppearInfo& right) const noexcept
  {
  ...
  ChannelImpAppearInfo::empty() const noexcept
  {
    return impop_appear_channels.empty() &&
      ...
      imp_other_appear_channels.empty() && impop_no_imp_appear_channels.empty();
  }
    ...
    std::ostream& ostr, const char* offset) const noexcept
  {
    ...
    for (SizeChannelArray::const_iterator size_channel_it = appear_channel_ecpms.begin();
        size_channel_it != appear_channel_ecpms.end();
        ++size_channel_it)
    {
      ostr << "[" << size_channel_it->size << ", " << size_channel_it->channel_id << "] ";
    }

    ostr << std::endl;
    ostr << offset << "disappear channel ecpms: ";

    for (ChannelECPMArray::const_iterator ecpm_it = disappear_channel_ecpms.begin();
        ecpm_it != disappear_channel_ecpms.end();
        ++ecpm_it)
    {
      ostr << "[" << ecpm_it->size << ", " << ecpm_it->ecpm << ", " << ecpm_it->channel_id << "] ";
    }
  ...
  inline
  bool
  InventoryActionProcessor::InventoryInfo::operator==(const InventoryInfo& right) const noexcept
  {
    return
      time == right.time &&
