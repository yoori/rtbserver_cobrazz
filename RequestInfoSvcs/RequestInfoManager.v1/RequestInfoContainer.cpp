...
namespace RequestInfoSvcs {
    ...
    const RequestInfo::Revenue& orig_pub_revenue)
  {
    ...
    std::cerr << "apply pub_cost_coef on " << impression_info.request_id << ": " <<
...

    {
      ...
      convert_revenue_reader_to_revenue(pub_revenue, request_writer.pub_revenue());
    ...
    /*throw(RequestContainerProcessor::Exception)*/
  {
    ...
    try
    {
      ...
      if(request_process_delegate.move_operation &&
         request_operation_processor_.in())
      {
        assert(request_operation_processor_);

        ImpressionInfo move_impression_info(impression_info);
        move_impression_info.user_id =
          request_process_delegate.move_request_user_id;
        request_operation_processor_->process_impression(
          move_impression_info);
      }
    }
    catch(const eh::Exception& ex)
    {
...
#       endif
        ...
        unsigned long i_val = request_reader.impression_non_considered();
        bool request_done = request_reader.request_done();
        unsigned long fraud_val = request_reader.fraud();
        if(!request_done &&
          fraud_val != RequestInfo::RS_MOVED)
        {
...
#       endif
        ...
        if(request_reader.fraud() == RequestInfo::RS_MOVED)
        {
          if(move_enabled)
          {
            ...
            request_process_delegate.move_operation = true;
            request_process_delegate.move_request_user_id =
              AdServer::Commons::UserId(request_reader.user_id());
          }
...
#       endif
        ...
        if(request_reader.fraud() != RequestInfo::RS_MOVED &&
          ...
          request_reader.impression_non_considered() == 0)
        {
#         ifdef DEBUG_OUTPUT_
          std::cerr << "RequestInfoContainer::process_impression_buf_(" <<
            impression_info.request_id << "): this = " << this <<
