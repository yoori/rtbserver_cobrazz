          ...
          if (numeric_warn)
          {
              ...
              it_part->part.data() + it_part->part.length())
            {
              std::cerr << "The word '" << it_part->part
                << "' in the trigger '" << it->second << "' contains only numbers" << std::endl;
              numeric_warn = false;
            }
          }
          ...
          {//over covering
                ...
                res_it != data.trigger_ids[ChannelSvcs::CT_PAGE].end(); ++res_it)
            {
              if (best != *res_it && triggers_.find(*res_it) != triggers_.end())
              {
                ...
                Language::Trigger::normalize(triggers_[*res_it], trigger_view2, 0);
                ...
                else if (cmp == 2)
                {
                  std::cerr << "The trigger '" << triggers_[*res_it]
                    << "' is more common, remove trigger '" << triggers_[best] << "'" << std::endl;
                ...
                {
                  std::cerr << "The trigger '" << triggers_[*res_it]
                    << "' is dublicate '" << triggers_[best] << "', remove it" << std::endl;
                  if (*res_it != cur)
                  {
                    triggers_.erase(*res_it);
                  }
          ...
          else
          {
            if (data.trigger_ids[ChannelSvcs::CT_PAGE].front() != it->first)
            {
              std::cerr << "Expect matching '" << it->second << "', got '"
                << triggers_[data.trigger_ids[ChannelSvcs::CT_PAGE].front()] << "'" << std::endl;
            }
            ++it;
          }
        }
