...
namespace AdServer::ChannelSvcs::TriggerParser
{

  const unsigned long TriggerParser::MAX_TRIGGER_LENGTH = Commons::MAX_WORD_LENGTH;
    ...
    /*throw(Exception)*/
  {
    ...
    atom.id = id;
    atom.lang = lang;
    for (TriggerList::const_iterator it = triggers.begin(); it != triggers.end(); ++it)
    {
      if (it->type == 'U')
      {
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      try
      {
        Language::Trigger::normalize(trigger, trigger_view, segmentor);
      }
      catch(const eh::Exception& e)
      {
        if (logger)
        {
          logger->sstream(Logging::Logger::WARNING, "TraffickingProblem", "ADS-TF-5")
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      while (splitter.get_token(token))
      {
        ...
        catch(const eh::Exception& e)
        {
          if (logger)
          {
            logger->sstream(Logging::Logger::WARNING, "TraffickingProblem", "ADS-TF-6")
    ...
    {
      Stream::Error err;
      err << __func__ << ": error on parsing channel " << channel_id << " :"  << e.what();
      throw Exception(err);
    }
  }
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      Parts parts(2);
      bool exact;
      if (get_url_view(trigger, allow_ports, exact, trigger_view) && !trigger_view.empty())
      {
        ...
        Serialization::serialize(parts, 'U', exact, negative, url_words.back().trigger);
      }
    }
    catch(const eh::Exception& e)
    {
