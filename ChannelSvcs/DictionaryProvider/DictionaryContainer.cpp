...
namespace AdServer::ChannelSvcs
{
  ...
  Dictionary::add_lexeme(const Forms& forms) /*throw(eh::Exception)*/
  {
    ...
    for (Forms::const_iterator ci = forms.begin(); ci != forms.end(); ++ci)
    {
      if (!ci->empty())
      {
  ...
  {
    buffer_.insert(buffer_.end(), dict.buffer_.begin() + 1, dict.buffer_.end());
  ...
  Dictionary::make_lexeme_(const char* begin, Forms& forms) const /*throw(eh::Exception)*/
  {
    ...
    while (*word_end != '\n')
    {
      if (*word_end == '\0')
      {
        forms.insert(String::SubString(word_begin, word_end - word_begin));
      ...
      LexemeItemVector& res) const /*throw(eh::Exception)*/
  {
    ...
    {
      const Dictionaries::const_iterator dict_it = dictionaries_.find(lang_name);
  ...
  DictionaryContainer::get_statistic(std::ostream& ostr) const /*throw(eh::Exception)*/
  {
    ...
    for (Dictionaries::const_iterator it = dictionaries_.begin(); it != dictionaries_.end(); ++it)
    {
