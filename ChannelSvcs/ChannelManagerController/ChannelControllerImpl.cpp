...
namespace AdServer::ChannelSvcs
{
      ...
      check_sum_(0)
  {
    try
    {
      ...
      const xsd::AdServer::Configuration::ChannelSourceType& type = config->ChannelSource();

      if (type.local_groups().present())
      {
        parse_groups_(type.local_groups().get(), connection_.local_groups);

        Generics::CRC32Hash hash(check_sum_, check_sum_);
        hash.add(type.local_groups().get().data(), type.local_groups().get().size());
      }
      ...
      if (type.ProxySource().present() && type.ProxySource().get().ProxyRefs().present())
      {
        calc_proxy_sums_(
      ...
      if (logger()->log_level() >= Logging::Logger::TRACE)
      {
        ...
        for (auto i = it->begin(); i != it->end(); ++i)
        {
          log_chunks(ostr, (*i)->chunks, count_chunks_) << "; check sums "
            << i - it->begin() << "->" << (*i)->check_sum[0] << ':' << (*i)->check_sum[1] << ' ';
        }
    ...
    /*throw(Exception)*/
  {
    ...
    {
      unsigned long num;
      if (String::StringManip::str_to_int(token, num) && num < connection_.servers.size())
      {
      ...
      /*throw(Exception)*/
  {
    try
    {
      read_list_refs_(proxy_config.CampaignServerCorbaRef(), connection_.campaign_server_refs);

      add_refs_to_sum(check_sum_, proxy_config.CampaignServerCorbaRef());

      if (proxy_config.ProxyRefs().present())
      {
        read_list_refs_(proxy_config.ProxyRefs().get(), connection_.proxy_refs);
      ...
      /*throw(Exception)*/
  {
    try
    {
      read_list_refs_(regular_config.CampaignServerCorbaRef(), connection_.campaign_server_refs);

      connection_.pg_connection = regular_config.PGConnection().connection_string();
  ...
  void ChannelControllerImpl::init_corba_refs_(const ChannelControllerConfig* config)
      /*throw(Exception)*/
  {
    ...
    try
    {
      ...
      for (j = config->ControlGroup().begin(); j != config->ControlGroup().end(); j++)
      {
        if (j->ChannelHost().size())
        {
          ...
          xsd::AdServer::Configuration::ControlGroupType::ChannelHost_sequence::const_iterator i;

          CSInfo_var info;
          for (i = j->ChannelHost().begin(); i != j->ChannelHost().end(); ++i)
          {
            ...
            info->control_ref = AdServer::ChannelSvcs::ChannelServerControl::_narrow(obj.in());
            ...
            Config::CorbaConfigReader::read_corba_ref(i->ChannelServerProcRef(), corba_object_ref);
            ...
            info->proccontrol_ref = CORBACommons::IProcessControl::_narrow(obj.in());
            ...
            Config::CorbaConfigReader::read_corba_ref(i->ChannelServerRef(), corba_object_ref);
            ...
            info->server_ref = AdServer::ChannelSvcs::ChannelServer::_narrow(obj.in());
            ...
            Config::CorbaConfigReader::read_corba_ref(i->ChannelUpdateRef(), corba_object_ref);
            ...
            info->update_ref = AdServer::ChannelSvcs::ChannelUpdate::_narrow(obj.in());
    ...
    catch(const CORBA::Exception& e)
    {
      ...
      ostr << "ChannelControllerImpl::init_corba_refs_: "
        << "Catch CORBA::Exception on resolving " << corba_object_ref.object_ref << ". : " << e;
      logger()->log(ostr.str(), Logging::Logger::ERROR, ASPECT, "ADS-IMPL-25");
      throw Exception(ostr);
    }
    catch(const eh::Exception& e)
    {
      Stream::Error ostr;
      ostr << "ChannelControllerImpl::init_corba_refs_: "
        << "Catch CORBA::SystemException. : " << e.what();
      logger()->log(ostr.str(), Logging::Logger::ERROR, ASPECT, "ADS-IMPL-25");
      throw Exception(ostr);
    }
    catch(...)
    {
      Stream::Error ostr;
      ostr << "ChannelControllerImpl::init_corba_refs_: " << "unknown Exception." ;
      logger()->log(ostr.str(), Logging::Logger::ERROR, ASPECT, "ADS-IMPL-25");
      throw Exception(ostr);
    }
  }
          ...
          if (control_servers_)
          {
            if ((*i)->check_sum[1] != check_sum)
            {
              if (((*i)->check_sum[0] == check_sum))
              {
                ...
                if ((*i)->proccontrol_ref->is_alive() == CORBACommons::AProcessControl::AS_READY)
                {
              ...
              if (check_sum)
              {
                logger()->sstream(Logging::Logger::NOTICE, ASPECT, "ADS-IMPL-21") <<
                  "Change configuration for server " <<
                  (i - it->begin()) <<
                  ", old sum = " << check_sum << ", new sum = " << (*i)->check_sum[0];
              }
              ...
              try
              {
                ...
                else
                {
                  ...
                  pack_ref_list_(connection_.campaign_server_refs, db_info.campaign_refs);
              ...
              catch(const AdServer::ChannelSvcs::ImplementationException& e)
              {
                ...
                logger()->log(ostr.str(), Logging::Logger::CRITICAL, ASPECT, "ADS-IMPL-22");
              }
              catch(const CORBA::SystemException& e)
              {
                Stream::Error ostr;
  ...
  ChannelControllerImpl::get_channel_session()
  {
    ...
    try
    {
          ...
          it != connection_.servers.end(); ++it,j++)
      {
        ...
        for (CSConnection::CSInfoVector::const_iterator i = it->begin(); i != it->end(); ++i,k++)
        {
    ...
    /*throw(eh::Exception)*/
  {
    ...
    for (auto i = info.begin(); i != info.end(); i++, k++)
    {
      ...
      std::copy((*i)->chunks.begin(), (*i)->chunks.end(), descr[k].chunks.get_buffer());
    ...
    /*throw(AdServer::ChannelSvcs::ChannelClusterControl::ImplementationException)*/
  {
    try
    {
      ...
      for (CSConnection::CSGroups::const_iterator it = connection_.servers.begin();
          it != connection_.servers.end(); ++it)
      {
        for (CSConnection::CSInfoVector::const_iterator i = it->begin(); i != it->end(); ++i,j++)
        {
    ...
    catch(const eh::Exception& e)
    {
      ...
      CORBACommons::throw_desc<
        ChannelSvcs::ChannelClusterControl::ImplementationException>(ostr.str());
    }
    return 0; // never reach
  }

  ChannelClusterControlImpl::ChannelClusterControlImpl(ChannelControllerImpl* controller) noexcept:
  ...
  ChannelClusterControlImpl::is_alive() noexcept
  {
    try
    {
      ::AdServer::ChannelSvcs::ChannelClusterSession_var session = get_control_session();
      return session->is_alive();
    }
    catch(const AdServer::ChannelSvcs::ChannelClusterControl::ImplementationException&)
    {
  ...
  ChannelClusterControlImpl::comment() /*throw(CORBACommons::OutOfMemory)*/
  {
    ...
    catch(const AdServer::ChannelSvcs::ChannelClusterControl::ImplementationException& e)
    {
      Stream::Error ostr;
      ostr << "AdServer::ChannelSvcs::"
        "ChannelClusterControlImpl::ImplementationException:" << e.description;
