...
namespace AdServer::ChannelSvcs
{
  ...
  {
    CORBACommons::CorbaClientAdapter_var c_adapter;
    ChannelLoadSessionFactoryImpl_var load_session_factory;
    ChannelUpdateStatLogger_var update_stat_logger;

    ChannelServerCoreState()
      : c_adapter(new CORBACommons::CorbaClientAdapter())
    {}
  };
      ...
      queries_counter_(0)
  {
    AdServer::ChannelSvcs::ChannelServerSessionFactory::init(
      *state_holder_->c_adapter,
      0,
      &state_holder_->load_session_factory,
      callback_,
      0,
      0,
      10 // threads
    ...
    try
    {
      ...
      if(server_config->DictionaryRefs().present())
      {
        ...
        dict_matcher_.reset(new DictionaryMatcher(
            *state_holder_->c_adapter, dictionary_server_refs, logger()));
      }
      ...
      schedule_update_task_(1, nullptr);
    }
    catch(const eh::Exception& e)
    {
      Stream::Error ostr;
  ...
  ChannelServerCore::~ChannelServerCore() noexcept
  {
  }
    ...
    const ChannelServerConfig* config) noexcept
  {
    try
    {
      ...
      if(config->UpdateStatLogger().present())
      {
        ...
        state_holder_->update_stat_logger = new ChannelUpdateStatLogger(
          size,
          statistic.period(),
          path.c_str());
      }
  ...
  void ChannelServerCore::init_update_(
    ...
    try
    {
      ...
      switch(update_data->state)
      {
        ...
        case UpdateData::US_CHECKED:
          ...
          variant_server->update_ccg(update_data.get(), ccg_limit_);
          {
  ...
  void ChannelServerCore::schedule_update_task_(
    unsigned long period, UpdateDataPtr update_data) noexcept
  {
    try
    {
      if(!update_data)
      ...
      ReferenceCounting::SmartPtr<TaskBase> msg =
        new UpdateTask(this, std::move(update_data), task_runner_);
  ...
  void ChannelServerCore::check(
    const ChannelServerCore::CheckQuery& query,
    ChannelServerCore::CheckData& data)
  {
    ...
    try
    {
      ...
      Generics::Time old_master = query.master_stamp;

      CheckInformation::CheckMap ids;
      force_id.insert(query.new_ids.begin(), query.new_ids.end());
      ...
      data.first_stamp = info.first_master;
      data.master_stamp = info.master;
  ...
  void ChannelServerCore::log_update_() noexcept
  {
  ...
  void ChannelServerCore::update_triggers(
    const std::vector<unsigned long>& ids,
    ChannelServerCore::UpdateDataResult& result)
  {
    static const char* FN = "ChannelServerCore::update_triggers";
            ...
            buffer.begin(); i != buffer.end(); ++i)
        {
          ...
          if(info)
          {
                ...
                tr_it != info->triggers.end(); ++tr_it)
            {
              ...
              trigger_info.channel_trigger_id = tr_it->channel_trigger_id;
              trigger_info.trigger = trigger;
              allocated_size += trigger.size();
              add.words.push_back(trigger_info);
            }
            add.stamp = info->stamp;
  ...
  void ChannelServerCore::update_all_ccg(
    const ChannelServerCore::CCGQuery& query,
    ChannelServerCore::PosCCGResult& result)
  {
    ...
    try
    {
      ...
      get_id.insert(query.channel_ids.begin(), query.channel_ids.end());
          ...
          it != buffer.end(); ++it)
      {
        ...
        CCGKeyword out;
        out.ccg_keyword_id = keyword.ccg_keyword_id;
        out.ccg_id = keyword.ccg_id;
        out.channel_id = keyword.channel_id;
        out.max_cpc = keyword.max_cpc;
        out.ctr = keyword.ctr;
        out.click_url = keyword.click_url;
        out.original_keyword = keyword.original_keyword;
      ...
      for(const auto& deleted_item : deleted)
      {
        ...
        version.stamp = deleted_item.second;
        result.deleted.push_back(version);
      }
  ...
  unsigned long ChannelServerCore::get_count_chunks() noexcept
  {
    return count_chunks_;
  }

  void ChannelServerCore::match(
    ...
    ChannelServerCore::MatchResult& result)
  {
    try
    {
      ...
      TriggerMatchRes res;
      const Generics::Uuid uid = query.uid;
      ...
      result.match_time = timer.elapsed_time();
      queries_counter_.fetch_add(1, std::memory_order_relaxed);
    }
    catch(const ChannelContainer::Exception& e)
    {
      Stream::Error ostr;
  ...
  void ChannelServerCore::get_ccg_traits(
    const std::vector<unsigned long>& ids,
        ...
        for(const auto id : ids)
        {
          ...
          if(channel)
          {
                ...
                it_id != channel->ccg_keywords.end(); ++it_id)
            {
              ...
              {
                ChannelServerCore::CCGKeyword out;
                out.ccg_keyword_id = keyword.ccg_keyword_id;
                out.ccg_id = keyword.ccg_id;
                out.channel_id = keyword.channel_id;
                out.max_cpc = keyword.max_cpc;
                ...
                out.click_url = keyword.click_url;
                out.original_keyword = keyword.original_keyword;
                result.ccg_keywords.push_back(out);
              }
            }
          }
        }
  ...
  void ChannelServerCore::set_sources(
    ...
    try
    {
      ...
      if(!db_info.campaign_refs.empty())
      {
        ...
        for(size_t i = 0; i < db_info.campaign_refs.size(); i++)
        {
          ...
          mes << db_info.campaign_refs[i];
          pool_config.iors_list.emplace_back(db_info.campaign_refs[i].c_str());
        }
        logger()->log(mes.str(), Logging::Logger::TRACE, ASPECT);
      }

      new_variant_server_ = std::make_shared<ChannelServerDB>(
        db_in,
        ports_,
        vec_sources,
        count_chunks_,
        db_info.colo,
        db_info.version.c_str(),
        pool_config,
        SERVICE_INDEX_,
        segmentors_,
        logger(),
  ...
  std::string ChannelServerCore::comment() /*throw(eh::Exception)*/
  {
    ...
    return ready_ ? std::string("ready") : message.str().str();
  }
  ...
  void ChannelServerCore::add_channels_(
  ...
  void ChannelServerCore::log_parsed_input_(
  ...
  void ChannelServerCore::fill_result_(
  ...
  void ChannelServerCore::log_result_(
  ...
  void ChannelServerCore::UpdateTask::execute() noexcept
  {
    server_impl_->update_task(update_data_);
  }

  void ChannelServerCore::deactivate_object()
