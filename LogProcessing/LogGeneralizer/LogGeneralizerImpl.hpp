...
namespace AdServer::LogProcessing
{
  ...
  private:
    ...
    void
    init_log_proc_info_(const xsd::cxx::tree::optional<LogProcessingParams>& log_proc_params)
      /*throw(Exception, eh::Exception)*/;

    template <typename LogProcTraits>
    void
    init_hf_log_proc_info_(const LogProcessingParamsHFTypeOptional& log_proc_params)
      /*throw(Exception, eh::Exception)*/;
    ...
    public:
      ...
      execute() noexcept
      {
        ...
        try
        {
          schedule(Generics::Time::get_time_of_day() + log_proc_thread_info_->CHECK_LOGS_PERIOD);
        }
  ...
  {
    return log_proc_params.present() ? log_proc_params.get().max_upload_task_count() + 1 : 0;
  }
  ...
  LogGeneralizerImpl::make_hf_log_processor_(
    ...
    /*throw(Exception, eh::Exception)*/
  {
    const HitsFilteringParamsType& hf_params = log_proc_params.get().HitsFiltering();
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    params.log_proc_thread_info->log_processor = db_enabled() ?
      make_log_processor_<LogProcTraits>(params, log_proc_params) :
      make_log_processor_<typename LogProcTraits::Inactive, LogProcessingParams>(
        params, log_proc_params);

    PlannerTask_var check_logs_task(new CheckLogsTask(this, params.log_proc_thread_info));
    ...
    /*throw(Exception, eh::Exception)*/
  {
    ...
    params.log_proc_thread_info->log_processor = db_enabled() ?
      ...
      make_hf_log_processor_<typename LogProcTraits::Inactive>(params, log_proc_params);
