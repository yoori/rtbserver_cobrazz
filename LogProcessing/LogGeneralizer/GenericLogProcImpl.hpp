...
namespace AdServer::LogProcessing
{
  ...
  public:
    ...
    :
      ...
      log_version_manager_(new LogVersionManagerT(context, out_dir, bundle_params))
    {
    ...
    :
      LogProcessorImplBase(in_dir, context->logger),
      proc_stat_(proc_stat_impl),
      log_version_manager_(new LogVersionManagerT(context, out_dir, bundle_params, hits_filter))
    {
      create_file_receiver_(fr_interrupter);
    }
  ...
  GenericLogProcessorImpl<L_TRAITS_, LVM_>::check_and_load()
  {
    ...
    while (fr_interrupter_->active() && (file = file_receiver_->get_eldest(tmp)))
    {
      ...
      {
        proc_stat_->template add<Generics::Values::UnsignedInt>(proc_stat_.input_error_count, 1);
      ...
      if (file_processing_max_time < file_time)
      {
        ...
        proc_stat_->set_if_gt_time(proc_stat_.input_time_per_file_max, file_time);
      }
    }
