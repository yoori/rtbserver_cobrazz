...
namespace AdServer::LogProcessing
{
    ...
    /*throw(eh::Exception)*/
  {
    if (version < "2.6")
    {
      ConversionFacility &conv_facility = ConversionFacilitySingleton::instance();

      ConversionFacility::KeyT
        conv_type(std::string(FromLogTraits::log_base_name()) + " V" + version,
          ToLogTraits::log_base_name());

      const ConversionFacility::ConvData &conv_data = conv_facility.get_conversion(conv_type);
  ...
  ColoUsersExtTraits::make_conversions(
    ...
    /*throw(eh::Exception)*/
  {
    Aux_::ConversionFacility &conv_facility = Aux_::ConversionFacilitySingleton::instance();
    ...
    const Aux_::ConversionFacility::ConvData &conv_data = conv_facility.get_conversion(conv_type);
  ...
  void
  CampaignUserStatExtTraits::add_conversion_support(LogVersionManager* log_version_manager)
    /*throw(eh::Exception)*/
  {
    // For old (V1.1) CampaignStat logs
    log_version_manager->template add_conversion<
      typename ConvTraits::CollectorType>(ConvTraits::current_version());
    // For old (V1.0) CampaignStat logs
    log_version_manager->template add_conversion<CampaignStatCollector_V_1_0>("1.0");
  }

  template <typename LogVersionManager>
  void
  SiteUserStatExtTraits::add_conversion_support(LogVersionManager* log_version_manager)
    /*throw(eh::Exception)*/
  {
    log_version_manager->template add_conversion<
      typename ConvTraits::CollectorType>(ConvTraits::current_version());
  ...
  template <typename LogVersionManager>
  void
  CcgCcStatToCcgCcUserStatConversion::add_conversion_support(LogVersionManager* log_version_manager)
    /*throw(eh::Exception)*/
  {
    // For old CCGStat logs
    ConversionAdder<LogVersionManager> convert_versions(log_version_manager);
    LogVersionManager::Traits::ConvTraits::for_each_old(convert_versions);
  }
    ...
    /*throw(eh::Exception)*/
  {
    ...
    try
    {
      ...
      upd_conv_data.proxy2 = log_version_manager->get_proxy(LogTraits::current_version());
    ...
    catch (const Aux_::ConversionFacility::Exception&)
    {
      ...
      conv_data.proxy = log_version_manager->get_proxy(LogTraits::current_version());
  ...
  public:
    ...
    add_conversion(const char* version) /*throw(eh::Exception)*/
    {
      ...
      conv_data.loader = LogExtTraits::template make_conversion_loader<OldCollector>(bundle_);
      conv_data.proxy = get_proxy(LogExtTraits::current_version());
      ConversionFacilitySingleton::instance().register_conversion(conv_type, conv_data);
    }
  ...
  protected:
      ...
      /*throw(eh::Exception)*/
    {
      ...
      LOG_CONV_RESULT result = LogTraits::make_conversions(is, file_handle, version);
      switch (result)
      {
        case LCR_PARTIAL:
      ...
      /*throw(eh::Exception)*/
    {
      ...
      bundle = typename OtherLogExtTraits::CollectorBundlePtrType(
        new typename OtherLogExtTraits::CollectorBundleType(bundle_params, thread_saver));
      ...
      CollectorBundleProxy_var proxy = new CollectorBundleProxyImpl<BundlePtrT>(bundle);
  ...
  class LogVersionManager2 : public LogVersionManagerBaseImpl<Traits>
  {
    ...
    )
    {
      ...
      typename ThreadLogSaverImpl<Traits, typename Traits::DbSaverType>::SmartPtr
        thread_saver(new ThreadLogSaverImpl<Traits, typename Traits::DbSaverType>(context, saver));

      BaseType::bundle_ = new typename Traits::CollectorBundleType(bundle_params, thread_saver);
      ...
      CollectorBundleProxy_var proxy = new CollectorBundleProxyImpl<BundlePtrT>(BaseType::bundle_);
  ...
  public:
    ...
    {
      add_current_version_support(context, bundle_params, lgsm_bundle, pg_conn_factory);
  ...
  public:
    ...
    {
      add_current_version_support(context, bundle_params, dest, lgsm_bundle, pg_conn_factory);
  ...
  class LogVersionManager3 : public LogVersionManagerBaseImpl<Traits>
  {
      ...
      const std::string& dest)
    {
      ...
      SaverType* saver(new SaverType(dest,
        typename Traits::CollectorFilterType::SmartPtr(new typename Traits::CollectorFilterType)));
  ...
  class LogVersionManager8: public LogVersionManagerBaseImpl<Traits>
  {
    ...
    )
    {
      ...
      typename ThreadLogSaverImpl<Traits, SaverType>::SmartPtr
         thread_saver(new ThreadLogSaverImpl<Traits, SaverType>(context, saver));
