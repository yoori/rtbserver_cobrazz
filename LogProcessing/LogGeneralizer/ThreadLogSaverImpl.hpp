...
namespace AdServer::LogProcessing
{
  ...
  namespace
  {
    ...
    inline
    void
    trace_msg_fun(const Traits&, const Param& p)
       noexcept
    {
...
#define trace_msg(a, b)
        ...
        try
        {
          if (PERIODIC)
          {
            ...
            trace_msg(LogExtTraits(),
              ...
              << ", occupy slot, tasks slots=" << context_->available_dump_task_count);
          }
          ...
          if (data_->collector_size)
          {
            Generics::Timer dump_timer;
            {
              ...
              trace_msg(LogExtTraits(),
                ...
                data_->collector_size << ", tasks slots=" << context_->available_dump_task_count);
  ...
  public:
    ...
    virtual void
    dump_start(const Spillover_var& data, CollectorBundleType* spillovers_owner = 0) noexcept
    {
      try
      {
        PlannerTask_var task(new DumpLogsTask(data, context_, save_processor_, spillovers_owner));
        task->deliver();
      }
      catch (const eh::Exception &ex)
      {
    ...
    {
      {
        ...
        trace_msg(LogExtTraits(),
          "Urgent task with data size=" << data->collector_size
            << " will be push into runner, tasks slots=" << context_->available_dump_task_count);
      }
  ...
  ThreadLogSaverImpl<LogExtTraits, LogSaverType>::DbDumpTimeoutMsg::deliver()
    noexcept
  {
    unsigned long secs = (Generics::Time::get_time_of_day() - START_TIME_).tv_sec;
