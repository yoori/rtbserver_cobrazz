...
namespace AdServer::LogProcessing
{
  ...
  class ProcStatImpl : public Generics::Values
  {
    ...
    typedef const Floating& (*FloatFunction)(const Floating&, const Floating&);
    typedef const UnsignedInt& (*UIntFunction)(const UnsignedInt&, const UnsignedInt&);
  ...
  public:
    ...
    stat_num_entries(Generics::Timer& dump_timer, size_t num_entries)
    {
      ...
      double total_elapsed = ProcStatsValues::total_dump_elapsed_time_.as_double();

      UnsignedInt total_dumps_count =
        func_or_set_(ProcStatsValues::output_success_count, 1ul, std::plus<UnsignedInt>());

      set_(ProcStatsValues::output_time_avg, total_elapsed / total_dumps_count);

      func_or_set_<FloatFunction>(ProcStatsValues::output_time_max, elapsed, std::max);

      set_(ProcStatsValues::output_time_total, total_elapsed);

      UnsignedInt total_entries_count =
        func_or_set_(ProcStatsValues::output_records_total, num_entries, std::plus<UnsignedInt>());

      set_(ProcStatsValues::output_records_avg, double(total_entries_count) / total_dumps_count);

      func_or_set_<UIntFunction>(ProcStatsValues::output_records_max, num_entries, std::max);
    }
    ...
    {
      Generics::Time elapsed_time = Generics::Time::get_time_of_day() - start_time;
      ...
      set_(ProcStatsValues::input_time_avg, total_elapsed / ++num_input_ops);

      set_(ProcStatsValues::input_time_per_file_avg,
        total_file_proc_elapsed / total_processed_files_count);

      func_or_set_<FloatFunction>(ProcStatsValues::input_time_max, elapsed, std::max);
  ...
  public:
    ...
    stat_num_entries(Generics::Timer& dump_timer, size_t num_entries)
    {
      process_stats_values_->
        stat_num_entries<ProcStatsValues<LOG_TYPE_TRAITS_> >(dump_timer, num_entries);
    }
  ...
  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::input_time_avg = L_T_T_::snmp_friendly_name() + ".inputTimeAvg";

  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::input_time_max = L_T_T_::snmp_friendly_name() + ".inputTimeMax";

  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::input_time_total = L_T_T_::snmp_friendly_name() + ".inputTimeTotal";
  ...
  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::input_error_count = L_T_T_::snmp_friendly_name() + ".inputErrorCount";

  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::output_time_avg = L_T_T_::snmp_friendly_name() + ".outputTimeAvg";

  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::output_time_max = L_T_T_::snmp_friendly_name() + ".outputTimeMax";

  template <class L_T_T_> const typename ProcStatsValues<L_T_T_>::Key
    ProcStatsValues<L_T_T_>::output_time_total = L_T_T_::snmp_friendly_name() + ".outputTimeTotal";
