...
namespace AdServer::LogProcessing
{
  ...
  template <typename LogExtTraits>
  void
  LogSaveProcessor<LogExtTraits>::dump(Generics::Timer& dump_timer, const Spillover_var& data)
  {
    try
    {
      saver_->save(data);
    ...
    {
      handle_save_exception_(data, "Exception", DEFAULT_ERROR_DIR, ex.what(), ex.code());
    }
    ...
    noexcept
  {
    ...
    {
      logger_->sstream(Logging::Logger::TRACE)
        << FNT << "Trying to save files [" << data->files() << "] in " << error_folder_name << "/";
    }
    try
    {
      // Must move files to Error or Deferred folder
      LogExtTraits::reject_files(data->files(), in_dir_, error_folder_name, data->save_time);
      if (!data->files().empty())
      {
        data->clear();
        proc_stat_->template add<Generics::Values::UnsignedInt>(ProcStatT::output_error_count, 1);
      }
    }
  ...
  template <typename LogExtTraits>
  void
  LogSaveProcessor<LogExtTraits>::unlink_files_(const CollectorBundleFileList& file_list)
  {
