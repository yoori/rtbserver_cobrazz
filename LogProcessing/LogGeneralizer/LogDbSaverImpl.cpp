...
namespace AdServer::LogProcessing
{
  ...
  const char DT_POSTCLICKSTATPGRECORD[] = "adserver.snapshot_post_click_t";
  ...
  CreativeStatPgRecord::readSQL(Commons::Postgres::SqlStream& /* stream */)
    /*throw(eh::Exception, Commons::Postgres::SqlException)*/
  {
    throw Commons::Postgres::SqlException("CreativeStatPgRecord::readSQL(): " "not supported");
  }
  ...
  class PostClickStatPgRecord: public Commons::Postgres::Object
  {
  public:
    PostClickStatPgRecord(
      const PostClickStatTraits::CollectorType::KeyT& outer_key,
      const PostClickStatTraits::CollectorType::DataT::KeyT& key,
      const PostClickStatTraits::CollectorType::DataT::DataT& data) noexcept
      : outer_key_(outer_key),
        key_(key),
        data_(data)
    {}

    const char* getSQLTypeName() const override
    {
      return DT_POSTCLICKSTATPGRECORD;
    }

    void writeSQL(Commons::Postgres::SqlStream& stream) override
    {
      stream.set_timestamp(Generics::ExtendedTime(outer_key_.sdate().time().get_gm_time()));
      stream.set_timestamp(Generics::ExtendedTime(outer_key_.adv_sdate().time().get_gm_time()));
      const auto& creative_key = key_.creative_key();
      stream.set_value(creative_key.colo_id());
      stream.set_value(creative_key.publisher_account_id());
      stream.set_value(creative_key.tag_id());
      stream.set_value(creative_key.size_id().present() ? creative_key.size_id().get() : 0);
      stream.set_value(creative_key.country_code());
      stream.set_value(creative_key.adv_account_id());
      stream.set_value(creative_key.campaign_id());
      stream.set_value(creative_key.ccg_id());
      stream.set_value(creative_key.cc_id());
      stream.set_value(creative_key.ccg_rate_id());
      stream.set_value(creative_key.colo_rate_id());
      stream.set_value(creative_key.site_rate_id());
      stream.set_value(creative_key.currency_exchange_id());
      stream.set_decimal(FixedNumber(creative_key.delivery_threshold()));
      stream.set_value(creative_key.num_shown());
      stream.set_value(creative_key.position());
      stream.set_value(creative_key.test());
      stream.set_value(creative_key.fraud());
      stream.set_value(creative_key.walled_garden());
      stream.set_value(std::string(1, creative_key.user_status()));
      stream.set_value(
        creative_key.geo_channel_id().present() ? creative_key.geo_channel_id().get() : 0);
      stream.set_value(
        creative_key.device_channel_id().present() ?
          creative_key.device_channel_id().get() : 0);
      stream.set_value(creative_key.ctr_reset_id());
      stream.set_value(creative_key.hid_profile());
      stream.set_value(creative_key.viewability());
      stream.set_value(data_.visits());
      stream.set_value(data_.visits_with_bounce());
      stream.set_value(data_.session_time_sum());
      stream.set_value(data_.page_views());
      stream.set_value(data_.new_user_visits());
      stream.set_value(key_.yandex_ref_id());
      stream.set_date(key_.yandex_event_date().time());
      stream.set_value(data_.reporting_visits());
    }

    void readSQL(Commons::Postgres::SqlStream&) override
    {
      throw Commons::Postgres::SqlException(
        "PostClickStatPgRecord::readSQL(): not supported");
    }

  private:
    const PostClickStatTraits::CollectorType::KeyT& outer_key_;
    const PostClickStatTraits::CollectorType::DataT::KeyT& key_;
    const PostClickStatTraits::CollectorType::DataT::DataT& data_;
  };
  ...
  CmpStatPgRecord::readSQL(Commons::Postgres::SqlStream& /* stream */)
    /*throw(eh::Exception, Commons::Postgres::SqlException)*/
  {
    throw Commons::Postgres::SqlException("CmpStatPgRecord::readSQL(): " "not supported");
  }
  ...
  )
  {
    ...
    for (CollectorT::const_iterator it = collector.begin(); it != collector.end(); ++it)
    {
        ...
        data_it != data.end(); ++data_it)
      {
        ...
        {
          cmp_value.adv_account_amount -= data_it->second.pub_advcurrency_amount();
        }

        cmp_value.adv_amount = data_it->second.adv_amount();
        cmp_value.adv_comm_amount = data_it->second.adv_comm_amount();
        cmp_value.adv_payable_comm_amount = data_it->second.adv_payable_comm_amount();
        ...
        if (data_it->first.publisher_account_id())
        {
          ...
          cmp_value.publisher_amounts[pub_acc_id] += data_it->second.adv_amount();
        }
        ...
        {
          cmp_value.tag_amounts[data_it->first.tag_id()] += cmp_tag_stat_value;
        }
  ...
  )
  {
    ...
    for (CollectorT::DataT::const_iterator it = collector.begin(); it != collector.end(); ++it)
    {
      if (!it->first.test())
      {
    ...
    {
      const char sql_stmt[] = "SELECT adserver.fill_snapshot_rsh($1::adserver.snapshot_rsh_t[])";

      Commons::Postgres::Statement_var stmt = new Commons::Postgres::Statement(sql_stmt);

      stmt->set_array(1, records_to_db);

      AdServer::Commons::Postgres::ResultSet_var res = connection->execute_statement(stmt, true);
    }
  ...
  void
  CreativeStatDbSaver::save_i_(CollectorT& collector, Generics::Time& outtime)
  {
    Commons::Postgres::Connection_var connection;
    ...
    try
    {
      ...
      {
        Stream::Error es;
        es << __PRETTY_FUNCTION__ << ": DB is not active. Operation will be deferred.";
  ...
  static bool
  do_post_click_stat_save_(
    Commons::Postgres::Connection_var& connection,
    const PostClickStatTraits::CollectorType::KeyT& outer_key,
    const PostClickStatTraits::CollectorType::DataT& collector)
  {
    if (collector.empty())
    {
      return true;
    ...
    Commons::Postgres::ObjectVector records;
    records.reserve(collector.size());
    for (const auto& [key, data] : collector)
    {
      if (!key.creative_key().test())
      {
        records.push_back(new PostClickStatPgRecord(outer_key, key, data));
      }
    }
    ...
    if (!records.empty())
    {
      const char statement_text[] =
        "SELECT adserver.fill_snapshot_post_click($1::adserver.snapshot_post_click_t[])";
      Commons::Postgres::Statement_var statement =
        new Commons::Postgres::Statement(statement_text);
      statement->set_array(1, records);
      connection->execute_statement(statement, true);
    }
  ...
  void
  PostClickStatDbSaver::save_i_(CollectorT& collector, Generics::Time& outtime)
  {
    Commons::Postgres::Connection_var connection;
    try
    {
      ...
      {
        throw OperationDeferred("PostClickStat database is not active");
      }

      for (const auto& [outer_key, data] : collector)
      {
        do_save_(connection, outer_key, data);
      }
    }
    catch (const Exception&)
    {
    ...
    catch (const AdServer::Commons::Postgres::Exception& ex)
    {
      ...
      Stream::Error error;
      error << __PRETTY_FUNCTION__ << ": PostgreSQL exception: " << ex.what();
      throw Exception(error, LOG_GEN_DB_ERR_CODE_1);
    }
    catch (const eh::Exception& ex)
    {
      Stream::Error error;
      error << __PRETTY_FUNCTION__ << ": exception: " << ex.what();
      throw Exception(error, LOG_GEN_IMPL_ERR_CODE_3);
    }
  }

  bool
  PostClickStatDbSaver::do_save_(
    Commons::Postgres::Connection_var& connection,
    const CollectorT::KeyT& outer_key,
    const CollectorT::DataT& collector)
  {
    return do_post_click_stat_save_(connection, outer_key, collector);
  }

  void
  DeferredPostClickStatDbSaver::save_i_(
    CollectorT& collector,
    const Generics::Time&,
    Generics::Time& outtime)
  {
  ...
  bool
  DeferredPostClickStatDbSaver::do_save_(
    Commons::Postgres::Connection_var& connection,
    const CollectorT::KeyT& outer_key,
    const CollectorT::DataT& collector)
  {
  ...
  void
  CreativeStatPgCsvSaver::pg_save_i_(CollectorT& collector, Generics::Time& outtime)
  {
    Commons::Postgres::Connection_var connection;
  ...
  )
  {
    ...
    const char sql_stmt[] = "SELECT adserver.fill_snapshot_cmp($1::adserver.snapshot_cmp_t[])";
    ...
    for (CollectorT::DataT::const_iterator it = collector.begin(); it != collector.end(); ++it)
    {
      CmpStatPgRecord *cmp_stat_pg_record = new CmpStatPgRecord(outer_key, it->first, it->second);
  ...
  void
  CmpStatDbSaver::save_i_(CollectorT& collector, Generics::Time& outtime)
  {
    Commons::Postgres::Connection_var connection;
  ...
  void
  CmpStatPgCsvSaver::pg_save_i_(CollectorT& collector, Generics::Time& outtime)
  {
    Commons::Postgres::Connection_var connection;
