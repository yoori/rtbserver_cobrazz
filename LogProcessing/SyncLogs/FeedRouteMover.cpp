...
namespace AdServer::LogProcessing
{
  ...
  bool
  FeedRouteMover::move(const char* src_path, const char* src_file, std::string* dst_host)
    /*throw(Utils::UnlinkException)*/
  {
    ...
    file_route_params.dst_file_name = dst_dir_ + file_route_params.src_file_name;
        ...
        ++try_number)
    {
      ...
      try
      {
        if (error_logger_->log_level() >= TraceLevel::HIGH)
        {
          ...
          ostr << FUN << ": preparing to send the '" <<
            file_route_params.src_file_path << "' to '" << file_route_params.dst_file_name << "':";
          error_logger_->log(ostr.str(), TraceLevel::HIGH, Aspect::SYNC_LOGS);
        }

        bool local_moving = host_checker_.check_host_name(file_route_params.dst_host);
      ...
      catch (const eh::Exception& ex)
      {
        ...
        ostr << "Can't sync '" << file_route_params.src_file_path <<
          ...
          " '" << file_route_params.dst_file_name << "' unavailable: " << ex.what();
