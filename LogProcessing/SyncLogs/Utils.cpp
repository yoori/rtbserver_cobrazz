...
namespace
{
    ...
    /*throw(eh::Exception)*/
  {
    oss << "SyncLogsImpl::run_cmd:" << (add_info ? add_info : "") <<
      "Status: " << status << ", while trying to execute the command: " << command;
  }
...
namespace AdServer::LogProcessing::Utils
{
    ...
    InterruptCallback* interrupter) /*throw(eh::Exception)*/
  {
    ...
    catch (const ArgvTokenizer::Exception& ex)
    {
      ...
      token_oss << "Auxiliary::StringTokenizer::Exception caught: " << ex.what() << ' ';
    ...
    {
      arg_str_vec[0].erase(arg_str_vec[0].begin(), arg_str_vec[0].begin() + pos + 1);
    }
    ...
    {
      Stream::Error ostr;
      form_run_cmd_error_string(ostr, " child finished abnormally.", cmdline, status);
      throw Exception(ostr);
    }
  ...
  unsigned int find_host_num(const char *file_name, unsigned int dest_hosts_size)
    /*throw(Exception, eh::Exception)*/
  {
    ...
    if (pos1 + 4 > pos2 || !(pos1 < pos_point && pos_point < pos2) || pos2 == std::string::npos)
    {
      Stream::Error ostr;
      ostr << FUN << ": file name '" << file_name << "' does not match the requested pattern.";
      throw Exception(ostr);
    }
    ...
    if (num == 0 || num > from_num)
    {
      Stream::Error ostr;
      ostr << FUN << ": "
        "the number of destination host in the '" << file_name << "' file name is not valid.";
      throw Exception(ostr);
    }
  ...
  void unlink_file(const char* file) /*throw(NotFound, UnlinkException)*/
  {
    if (::unlink(file) == -1)
    {
      if (errno == ENOENT)
      {
        eh::throw_errno_exception<NotFound>("Utils::unlink_file(", file, ") failed.");
      }
      else
      {
