...
namespace AdServer::LogProcessing
{
  ...
  private:
    void schedule_route_processing_task_(RouteProcessor *processor, unsigned long timeout) noexcept;

    static SchedType get_feed_type_(const char* feed_type_name)
      /*throw(Exception)*/;

    const char* adapt_path_(const char* dir, std::string& res) const
      /*throw(Exception)*/;
...
namespace AdServer::LogProcessing
{
  ...
  void SyncLogsImpl::ProcessRouteTask::execute() noexcept
  {
    ...
    sync_logs_impl_->schedule_route_processing_task_(route_processor_, check_period_);
  }
}
