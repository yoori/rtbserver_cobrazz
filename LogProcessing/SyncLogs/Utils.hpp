...
namespace AdServer::LogProcessing
{
  ...
  namespace Utils
  {
    ...
    public:
       ...
       */
      LogsRouteProcessorFlusher(ErrorPool* error_logger)
        noexcept;
...
namespace AdServer::LogProcessing::Utils
{
  ...
  MessageOut::log(
    ...
    const char* /*code*/)
  {
    logger->stream(severity, aspect, key->code().c_str())
      << "Aggregate " << errors_count << " errors, descriptions: "
      << buffer << (buffer.size() < max_size ? "" : "...[cut]...");
  }
  ...
  CellsKey::operator ==(const CellsKey& right) const noexcept
  {
    return src_host_ == right.src_host_ &&
      dst_host_ == right.dst_host_ && src_path_ == right.src_path_ && code_ == right.code_;
  }
...
namespace AdServer::LogProcessing
{
  ...
  namespace Utils
  {
    ...
    //
    inline
    LogsRouteProcessorFlusher::LogsRouteProcessorFlusher(ErrorPool* error_logger)
      noexcept
