...
namespace AdServer::Commons
{
  ...
  public:
     ...
     */
    void
    encrypt(const AesEncryptKey& key, unsigned char *out) const
      noexcept;
  ...
  public:
     ...
     */
    SecKeyStorage(const std::vector<std::string>& keys);
     ...
     */
    const Key&
    get_key(const Generics::Time& now, std::size_t& key_id) const;
  ...
  public:
     ...
     * @param config
     */
    SecTokenGenerator(const Config& config)
      /*throw(eh::Exception)*/;
     ...
     */
    std::string
    get_text_token(in_addr_t client_address) const
      /*throw(eh::Exception)*/;
  ...
  public:
     ...
     * @param key string vector
     */
    SecTokenValidator(const std::vector<std::string>&  keys);
