...
namespace Algs
{
  ...
  unsigned long
  hex_encode(Generics::ArrayAutoPtr<unsigned char>& res, const String::SubString& src)
    /*throw(eh::Exception)*/
  {
    ...
    for (String::SubString::ConstPointer it = src.begin(); it != src.end(); ++it, ++res_it)
    {
      ...
      *res_it = (high_val << 4) +
        static_cast<unsigned char>(String::AsciiStringManip::hex_to_int(*it));
    }
