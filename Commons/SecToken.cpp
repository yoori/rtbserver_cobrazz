...
namespace AdServer::Commons
{
  ...
  void
  PlainTextToken::encrypt(const AesEncryptKey& key, unsigned char *out) const
    noexcept
  {
  ...
  // SecTokenGenerator
  //
  SecTokenGenerator::SecTokenGenerator(const Config& config)
    /*throw(eh::Exception)*/  :
  ...
  // SecTokenValidator
  //
  SecTokenValidator::SecTokenValidator(const std::vector<std::string>&  keys) :
    keys_(keys)
  { }
    ...
    const Generics::Time& timeout)
  {
    ...
    try
    {
      String::StringManip::base64mod_decode(encrypted_buf, token, false);
    }
    ...
    uint32_t crc = Generics::CRC::quick(0, buf, PlainTextToken::size - sizeof(uint32_t));
