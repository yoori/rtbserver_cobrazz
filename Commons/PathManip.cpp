...
namespace AdServer::PathManip
{
  bool
  relative_path(const char* from_path, const char* to_path, std::string& result)
    noexcept
  {
    result.clear();
    ...
    for (; *from_path == *to_path && *from_path; ++from_path, ++to_path)
    {
    }
  ...
  normalize_path(std::string& path) noexcept
  {
    ...
    while (end_pos != 0)
    {
         ...
         // ignore '//', '/./' appearance
      {
        ...
        {
          result_parts.push_front(String::SubString(path.c_str() + start_pos, end_pos - start_pos));
    ...
    for (PartList::const_iterator part_it = result_parts.begin();
  ...
  void
  split_path(const char* path_name, std::string* path, std::string* name, bool strip_slash)
    noexcept
  {
    ...
    if (pos != std::string::npos && pos != path_pattern_s.size() - 1)
    {
      ...
      if (name)
      {
        name->assign(path_pattern_s, pos + 1, path_pattern_s.size() - pos - 1);
      }
    }
    else
    {
