...
namespace AdServer
{
  ...
  //
  XsltFileUpdateStrategy::XsltFileUpdateStrategy(const XsltTextTemplateArgs* args, const char* file)
    /*throw(eh::Exception)*/
    ...
    /*throw(Generics::CacheExceptions::CacheException, eh::Exception)*/
  {
    ...
    try
    {
      ...
      if (args_.in())
      {
        ...
        xsl_transformer_.transformer.reset(new XslTransformer(xsl_content, file_name_.c_str()));
      }
      else
      {
        xsl_transformer_.transformer.reset(new XslTransformer(file_name_.c_str()));
      }
    ...
    {
      Stream::Error ostr;
      ostr << FUN << ": Can't init Xslt template by '" << file_name_ << "': " << ex.what();
      throw Generics::CacheExceptions::CacheException(ostr);
    }
  ...
  //
  XsltFileCacheFactory::XsltFileCacheFactory(const XsltTextTemplateArgs* args)
    : args_(ReferenceCounting::add_ref(args))
  {}

  XsltFileCacheFactory::XsltFileCacheFactory(const XsltFileCacheFactory& init)
    : args_(init.args_)
  {}
  ...
  XsltFileCacheManager::XsltFileCacheManager(
    ...
    : Generics::CacheManager<
        ...
        XsltFileCacheFactory>(XsltFileCacheFactory(args), threshold_timeout, bound_limit)
  {}
}
