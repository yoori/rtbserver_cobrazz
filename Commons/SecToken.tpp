...
namespace AdServer::Commons
{
  ...
  SecKeyStorage<Key>::SecKeyStorage(const std::vector<std::string>&  keys)
  {
    auto make_key = std::bind(&SecKeyStorage::make_key_, this, std::placeholders::_1);

    keys_.resize(keys.size());

    std::transform(keys.begin(), keys.end(), keys_.begin(), make_key);
  }


  template <typename Key>
  const Key&
  SecKeyStorage<Key>::get_key(const Generics::Time& now, std::size_t& key_id) const
  {
  ...
  template <typename Key>
  Key
  SecKeyStorage<Key>::make_key_(std::string base64_buf)
    /*throw (Exception, eh::Exception)*/
  {
    ...
    return Key(reinterpret_cast<const unsigned char*>(key.data()), key.size());
  }
