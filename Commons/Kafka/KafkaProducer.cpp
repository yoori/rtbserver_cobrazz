...
namespace AdServer::Commons::Kafka
{
  ...
  // Producer::EventCb
  Producer::EventCallback::EventCallback(ProducerHandler* handler)
    noexcept :
    handler_(handler)
  {}

  void
  Producer::EventCallback::event_cb(RdKafka::Event& event)
  {
  ...
  Producer::DeliveryReportCallback::DeliveryReportCallback(ProducerHandler* handler)
    noexcept :
    handler_(handler)
  { }
      ...
      topic_conf_(RdKafka::Conf::create(RdKafka::Conf::CONF_TOPIC))
  {
    ...
    try
    {
      ...
      topic_conf_->set("partitioner_cb", &partition_callback_, error_str);
  ...
  int
  Producer::ProducerHandler::poll(int timeout_ms)
  {
    return producer_->poll(timeout_ms);
  }

  int
  Producer::ProducerHandler::flush(int timeout_ms)
  {
    ...
    size_t len)
  {
    ...
    {
      owner_->push_data(*key, std::string(static_cast<const char*>(payload), len));
    }
  }

  // class StatsObject
  Producer::StatsObject::StatsObject(Producer* owner, Generics::ActiveObjectCallback* callback) :
    ...
    {
      {
        ...
        ostr << FUN << ": '" << owner_->topic_name_ <<
          ...
          "  status: " << (disconnected? "Disconnected": "Connected") << std::endl <<
          "  sent: " << sent << std::endl <<
  ...
  Producer::work_() noexcept
  {
    ...
    while (active())
    {
...

      {
        ...
        reconnect_cond_.timed_wait(reconnect_lock_, &RECONNECT_TIMEOUT, true);
      }
  ...
  void
  Producer::process_error_(const char* context, const char* error)
  {
    if (!stats_->disconnected)
    {
      stats_->error_exception += 1;
      Stream::Error ostr;
      ostr << context << ": '" << topic_name_ << "' producer error: " << error;
  ...
  void Producer::push_data(const std::string& key, const std::string& data) noexcept
  {
