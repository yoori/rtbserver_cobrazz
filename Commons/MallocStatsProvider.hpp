...
namespace AdServer::Commons
{
  ...
  public:
    struct MemoryInfo
    {
      MemoryInfo(const std::uint64_t total_memory, const std::uint64_t in_use_memory)
  ...
  public:
    static MemoryInfo get_momory_info()
    {
      ...
      {
        Stream::Error ostr;
        ostr << __PRETTY_FUNCTION__ << " : Reason : " << "open_memstream is failed";
      ...
      {
        Stream::Error ostr;
        ostr << __PRETTY_FUNCTION__ << " : Reason : " << "malloc_info is failed";
        throw Exception(ostr);
      }
      ...
      {
        Stream::Error ostr;
        ostr << __PRETTY_FUNCTION__ << " : Reason : " << "buffer is empty";
        throw Exception(ostr);
      }
      ...
      catch (const boost::exception& exc)
      {
        ...
        Stream::Error ostr;
        ostr << __PRETTY_FUNCTION__ << " : Reason : " << error_info;
        throw Exception(ostr);
      }
    }
  };
