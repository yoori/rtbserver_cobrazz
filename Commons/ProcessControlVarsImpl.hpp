...
namespace AdServer::Commons
{
    ...
    bool set_value(const char *value, std::string& result) /*throw(Exception)*/
    {
      ...
      try
      {
        if (num_value == -1)
        {
          try
          {
            result = DbStateChanger::print_db_state(db_state_changer_->get_db_state());
          }
  ...
  public:
    ...
    {
      add_var_processor(LogLevelProcessor::VAR_NAME, new LogLevelProcessor(logger));
    }
...
namespace AdServer::Commons
{
  ...
  inline char*
  ProcessControlVarsLoggerImpl::control(const char* param_name, const char* param_value)
    noexcept
  {
    try
    {
      if (!proc_ctrl_vars_impl_.in())
      {
        ...
        const char* aspect = dynamic_cast<Logging::ActiveObjectCallbackImpl*>(callback())->aspect();
        ostr << (aspect ? aspect : "Service") << " is not ready.";
        return CORBA::string_dup(ostr.str().str().c_str());
      }
      return proc_ctrl_vars_impl_->control(param_name, param_value);
    }
    catch (const eh::Exception &ex)
    {
