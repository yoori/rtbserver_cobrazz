...
namespace AdServer::Commons::Postgres
{
    ...
    /*throw(SqlException, Postgres::Exception)*/
  {
    ...
    if (status != PGRES_TUPLES_OK && status != PGRES_COMMAND_OK)
    {
      ...
      err << "Query to database failed: ("
        << PQresStatus(status) << ") " << PQresultErrorMessage(res_pg.get());
  ...
  void Connection::create_cursor_(Statement* statement, int result_format)
    /*throw(Postgres::Exception)*/
  {
    ...
    try
    {
      ...
      if (PQsendQuery(conn_, "BEGIN;"))
      {
        ...
        {
          Stream::Error err;
          err << '(' << PQresStatus(status) << ") " << PQresultErrorMessage(pg_res.get());
      ...
      {
        Stream::Error err;
        err << '(' << PQresStatus(status) << ") " << PQresultErrorMessage(pg_res);
        throw Postgres::Exception(err);
      }
      //execute_query_("COMMIT;");
    }
    ...
    /*throw(Postgres::Exception, SqlException)*/
  {
    if (!end_cursor_)
    {
      ...
      if (status != PGRES_TUPLES_OK && status != PGRES_COMMAND_OK)
      {
        Stream::Error err;
        err << "Query to database failed: ("
          << PQresStatus(status) << ") " << PQresultErrorMessage(res_pg);
        throw SqlException(err);
      }
  ...
  ResultSet_var
  Connection::execute_statement(Statement* statement, bool sync, DATA_FORMATS format)
    /*throw(SqlException, Postgres::Exception)*/
  {
    clear_result_();
    const std::vector<std::string>& params = statement->params();
    ...
    std::vector<int> params_lens;
    prepare_params_(statement, params_values, params_lens);
    if (prepared_statements_.find(statement->name()) == prepared_statements_.end())
    {
    ...
    if (sync)
    {
      ...
      if (status != PGRES_TUPLES_OK && status != PGRES_SINGLE_TUPLE && status != PGRES_COMMAND_OK)
      {
        ...
        err << __func__ << ": Execution statement '" << statement->name()
          << "' failed: (" << PQresStatus(status) << ") " << PQresultErrorMessage(pg_res);
        throw SqlException(err);
      }
      ...
      {//save first error
        if (status != PGRES_TUPLES_OK)
        {
          ...
          err << __func__ << ": error on fetching row ("
            << PQresStatus(status) << ") : " << PQresultErrorMessage(res.get());
  ...
  Connection::delete_this_() const noexcept
  {
    ...
    if (connection_owner_.in())
    {
      ...
      delete_this = non_const_this->connection_owner_->destroy_connection(non_const_this);
    }
