...
namespace AdServer::Commons::Postgres
{
  ...
  void Statement::prepare_query(PGconn* conn, int count_params)
    /*throw(SqlException)*/
  {
    PGresult* res = PQprepare(conn, name_.c_str(), query_.c_str(), count_params, NULL);
    ExecStatusType status = PQresultStatus(res);
    PQclear(res);
    if (status != PGRES_COMMAND_OK)
    {
      Stream::Error err;
      err << "Preparing statement '" << name_ << "' failed: " << PQresultErrorMessage(res);
  ...
  void Statement::set_time(unsigned int param_num, const Generics::Time& time)
    /*throw(eh::Exception)*/
  {
    add_param_(time.get_gm_time().format("%H:%M:%S.%q"), param_num - 1);
  }

  void Statement::set_timestamp(unsigned int param_num, const Generics::Time& timestamp)
    /*throw(eh::Exception)*/
  {
  ...
  void Statement::set_array(unsigned int param_num, const ObjectVector& objs)
    /*throw(Exception)*/
  {
