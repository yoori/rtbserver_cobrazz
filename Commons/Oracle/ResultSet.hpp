...
namespace AdServer::Commons::Oracle
{
  ...
  public:
    ...
    std::string get_number_as_string(unsigned int col_index) const
    ...
    /*throw(typename DecimalType::Overflow)*/
  {
         ...
         DecimalType::FRACTION_RANK))
    {
      ...
      ostr << FUN << ": can't convert number '" << str <<
        "' to DecimalType<" << DecimalType::TOTAL_RANK << ", " << DecimalType::FRACTION_RANK << ">";
      throw typename DecimalType::Overflow(ostr);
    }

    size_t res_len = digits_count + sign + (
      exp < 0 ? -exp + 1 : (digits_count > exp ? 0 : exp + 1 - digits_count) + 1);
    ...
    long i = 0;
    for (; *pos != 'E' && frac_i < static_cast<long>(DecimalType::FRACTION_RANK); ++pos)
    {
      if (*pos != '.')
      {
        *decimal_pos++ = *pos;
